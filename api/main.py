...
from app.presentation.api.dependencies import get_event_bus, init_ws_event_handlers
...
class MaintenanceMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Allow app static assets through during maintenance
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # interval thereafter. The scheduled-workflow feature builds on this
        # same primitive (see app/infrastructure/scheduling/schedule_task.py).
        ...
        from app.infrastructure.scheduling.schedule_task import (
            build_schedule_callback,
        )
        ...
        # Schedule-trigger tick: fires workflows whose RRULE next_run_at is due.
        scheduler.register(
            ScheduledTask(
                ...
                initial_delay_seconds=float(settings.SCHEDULE_TICK_INTERVAL_SECONDS),
            )
        )
...
def create_app() -> FastAPI:
    ...
    # Workspace files are served only through authenticated routes (org_files,
    # RLS-scoped) and the public avatars route (GET /api/v1/public/avatars/...,
    # hardcoded to the avatars subdir). The workspace root is never statically mounted.
    ...
    # Webhooks mount under /api/v1 like every other router so inbound provider
    # callbacks hit /api/v1/webhooks/incoming/{token} (the convention clients use).
