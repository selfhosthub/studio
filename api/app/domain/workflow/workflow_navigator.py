...
class WorkflowNavigator:
    ...
    @staticmethod
    def get_dependency_closure(steps: Dict[str, Any], step_id: str) -> Set[str]:
        """The transitive `depends_on` ancestors of step_id (its upstream
        closure), excluding step_id itself. A step may reference only the
        outputs of steps it transitively depends on - forward steps and
        unrelated siblings are not in the closure. Cycle-safe.
        """
        closure: Set[str] = set()
        to_visit = [step_id]
        while to_visit:
            current = to_visit.pop()
            config = steps.get(current)
            deps = (
                config.get("depends_on", []) or []
