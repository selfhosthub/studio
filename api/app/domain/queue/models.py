...
class Queue(AggregateRoot):
    ...
    max_concurrency: int = Field(
        default_factory=lambda: get_settings().QUEUE_DEFAULT_MAX_CONCURRENCY, ge=1, le=1000
    )
    max_pending_jobs: int = Field(
        default_factory=lambda: get_settings().QUEUE_DEFAULT_MAX_PENDING, ge=1, le=100000
    )
    default_timeout_seconds: int = Field(
        default_factory=lambda: get_settings().QUEUE_DEFAULT_TIMEOUT, ge=1, le=86400
    )
    ...
    def create(
        ...
        queue = cls(
            ...
            max_concurrency=max_concurrency if max_concurrency is not None else s.QUEUE_DEFAULT_MAX_CONCURRENCY,
            max_pending_jobs=max_pending_jobs if max_pending_jobs is not None else s.QUEUE_DEFAULT_MAX_PENDING,
            default_timeout_seconds=default_timeout_seconds if default_timeout_seconds is not None else s.QUEUE_DEFAULT_TIMEOUT,
...
class Worker(AggregateRoot):
    ...
    # "local" iff the worker shares the API's /workspace mount. Re-checked
    # by the worker on each heartbeat - may flip if the mount comes/goes.
...
class QueuedJob(AggregateRoot):
    ...
    retry_count: int = 0
    max_retries: int = Field(default_factory=lambda: get_settings().DEFAULT_MAX_RETRIES)
    ...
    def create(
        ...
        job = cls(
            ...
            max_retries=max_retries if max_retries is not None else get_settings().DEFAULT_MAX_RETRIES,
            tags=tags or [],
            client_metadata=client_metadata or {},
