...
class StepExecution(AggregateRoot):
    ...
    # NOTE: iteration_count is NOT cached here - IterationExecution rows are the
    # canonical source (I-15). Derive via _resolve_iteration_metadata_from_rows.
    ...
    def create(
        ...
        *,
        ...
        return cls(
            ...
            status=status,
            max_retries=(
                max_retries if max_retries is not None else get_settings().DEFAULT_MAX_RETRIES
            ),
    ...
    def wait_for_webhook(self) -> None:
        """Park awaiting a provider callback. Idempotent.

        Clears the once-per-step "still awaiting" notify flag so a fresh entry
        (e.g. after a rerun re-parks the step) can notify again."""
        if self.status == StepExecutionStatus.WAITING_FOR_WEBHOOK:
            return
        self._validate_transition(StepExecutionStatus.WAITING_FOR_WEBHOOK)
        self.status = StepExecutionStatus.WAITING_FOR_WEBHOOK
        self.execution_data.pop("webhook_notified_at", None)
        self.updated_at = datetime.now(UTC)
