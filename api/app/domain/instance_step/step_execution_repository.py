...
class StepExecutionRepository(ABC):
    ...
    async def get_by_instance_and_key(
        ...
        *,
        ...
        for_update locks the row to serialize concurrent iteration-result
        transactions racing the all-iterations-done count.
        """
    ...
    async def list_webhook_steps_awaiting_reconcile(
        ...
        """WAITING_FOR_WEBHOOK steps idle past the notify grace period (notified once, never failed on a timer)."""

    @abstractmethod
    async def touch_updated_at(self, step_execution_ids: List[uuid.UUID]) -> int:
        """Bump updated_at without changing status (WFW reconcile re-poll throttle)."""
