...
class Instance(AggregateRoot):
    ...
    def wait_for_webhook(self, step_id: str) -> None:
        """Transition to WAITING_FOR_WEBHOOK after a step fires its async request(s)
        and releases its worker. Idempotent so callback/sweep handlers can re-enter
        the target state. The inbound callback resumes via transition_to_processing.
        """
        if self.status == InstanceStatus.WAITING_FOR_WEBHOOK:
            return
        if self.status != InstanceStatus.PROCESSING:
            raise InvalidStateTransition(
                message=f"Cannot wait for webhook from status {self.status}",
                code="INVALID_WAIT_FOR_WEBHOOK",
                context={
                    "instance_id": str(self.id),
                    "current_status": self.status,
                    "step_id": step_id,
        ...
        old_status = self.status
        self.status = InstanceStatus.WAITING_FOR_WEBHOOK
        self.updated_at = datetime.now(UTC)
