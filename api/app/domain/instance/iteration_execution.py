...
class IterationExecution(Entity):
    ...
    # Provider's id for the generation this iteration fired (e.g. Leonardo
    # generation_id). Nullable: set after the worker fires in webhook mode;
    # never populated in get/poll mode. The match key for inbound callbacks
    # (one row per generation_id) - see get_by_external_id.
