...
class Visibility(str, Enum):
    """Cross-org marketplace visibility of a catalog package.

    Orthogonal to ``scope`` (intra-org), ``status``/``is_enabled`` (operational
    enable), and ``publish_status`` (the org-user->org-admin approval flow). Do
    not fold any of those into this axis.

    - ``private``: super-org only. Default on create/import.
    - ``staging``: visible to orgs carrying ``is_staging``.
    - ``public``: visible to all orgs in the marketplace.
    """
...
class OperationalStatus(str, Enum):
    """Whether a catalog package can be used/run at all.

    Orthogonal to ``Visibility``: a ``public`` + ``inactive`` package stays
    listed (installed orgs see why runs paused) but starts no new work.

    Intentionally ``active | inactive`` only - no reserved ``deprecated``.
    Adding an enum value later is a cheap ``ALTER TYPE ADD VALUE``; reserving an
    undefined state now would force every switch to handle it.
    """
...
class ResourceSpecification(BaseModel):
    ...
    max_retry_count: Optional[int] = Field(
        default_factory=lambda: get_settings().DEFAULT_MAX_RETRIES
    )
...
class JobConfig(BaseModel):
    ...
    retry_count: Optional[int] = Field(
        default_factory=lambda: get_settings().STEP_DEFAULT_RETRY_COUNT
    )
    retry_delay_seconds: Optional[int] = Field(
        default_factory=lambda: get_settings().DEFAULT_RETRY_DELAY_SECONDS
    )
