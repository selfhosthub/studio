...
def _sync_local(repo_root: str = "/app") -> bool:
    ...
    if catalog_src.exists():
        # Source repo's catalog is authoritative - copy verbatim (don't re-derive providers).
        with open(catalog_src, "r", encoding="utf-8") as f:
            catalog = json.load(f)
        _write_catalog(catalog)
        logger.info(
            f"Docs synced from {COMMUNITY_SOURCE}: "
            f"{len(catalog.get('providers', []))} provider docs"
        )
