...
class Settings(BaseSettings):
    ...
    # WAITING_FOR_WEBHOOK notify window - default grace period before a parked
    # WFW step fires its once-per-step "still awaiting provider callback"
    # notification. No timer ever fails the step (I-13); this only nudges the
    # operator. Per-provider override lives in the catalog
    # (client_metadata.webhook_completion.notify_minutes); a provider whose
    # generations finish fast can set a shorter window. 60 is conservative for
    # providers like Leonardo whose healthy renders run 15+ min.
    WEBHOOK_NOTIFY_MINUTES: int = Field(default=60)

    # Periodic cleanup interval - how often the in-process scheduler fires the
    # cleanup cycle (worker deregister + orphan requeue + webhook notify sweep +
    # dead-letter replay). Lower → faster detection, more DB chatter. Higher →
    # cheaper, longer operator wait before stuck instances surface.
    PERIODIC_CLEANUP_INTERVAL_SECONDS: int = Field(default=60)

    # How often the schedule-trigger tick checks for due RRULE workflows. 60s
    # means a scheduled run can lag up to a minute past its nominal time.
    SCHEDULE_TICK_INTERVAL_SECONDS: int = Field(default=60)
...
def __getattr__(name: str):
    """Back-compat: `from app.config.settings import settings` still works.

    The first attribute access on the returned proxy triggers `get_settings()`.
    Modules that import the proxy but never read from it pay nothing.
    """
    if name == "settings":
        return get_settings()
