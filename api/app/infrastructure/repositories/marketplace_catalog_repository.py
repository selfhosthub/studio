...
class SQLAlchemyMarketplaceCatalogRepository(MarketplaceCatalogRepository):
    ...
    async def upsert_active(
        ...
        # One row per type: reuse the existing row and drop any extras left by
        # the prior insert-per-refresh behavior. This is a cache, not a ledger -
        # install history lives in package_versions (invariants #6).
        result = await self.session.execute(
            select(MarketplaceCatalogModel)
            .where(MarketplaceCatalogModel.catalog_type == catalog_type)
            .order_by(MarketplaceCatalogModel.is_active.desc())
        )
        ...
        if model is None:
            model = MarketplaceCatalogModel(
                id=uuid.uuid4(), catalog_type=catalog_type
            )
            self.session.add(model)
        model.catalog_data = catalog_data
