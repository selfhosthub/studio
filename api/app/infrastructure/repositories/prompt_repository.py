...
class SQLAlchemyPromptRepository(PromptRepository):
    ...
    async def create(self, prompt: Prompt) -> Prompt:
        model = PromptModel(
            ...
            visibility=prompt.visibility,  # type: ignore[assignment]  - domain enum assigned to SA column
    ...
    async def update(self, prompt: Prompt) -> Prompt:
        ...
        model.visibility = prompt.visibility  # type: ignore[assignment]
    ...
    async def list_all_by_slug(self, slug: str) -> List[Prompt]:
        stmt = select(PromptModel).where(
            PromptModel.slug == slug,
