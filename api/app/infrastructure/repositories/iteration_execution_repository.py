...
class SQLAlchemyIterationExecutionRepository(IterationExecutionRepository):
    ...
    async def get_by_step_and_index(
        ...
        *,
        ...
        if for_update:
            # Serialize concurrent/duplicate provider callbacks racing the same
            # (step, iteration) row's read-modify-write (the external_id stamp).
            # populate_existing refreshes the identity-map copy so the post-lock
            # read reflects committed state, not a stale cached attribute.
    ...
    async def get_by_external_id(
        ...
        # .first() (not scalar_one_or_none): a stale retained external_id from a
        # regen could collide, and a duplicate must not 500 the inbound callback.
        stmt = (
            select(IterationExecutionModel)
            .where(IterationExecutionModel.external_id == external_id)
            .order_by(IterationExecutionModel.updated_at.desc())
        )
        result = await self.session.execute(stmt)
    ...
    async def update(
        ...
        model.external_id = iteration.external_id
    ...
    async def delete_by_step_id(
        self, step_id: uuid.UUID, *, commit: bool = True
    ) -> int:
        stmt = delete(IterationExecutionModel).where(
            IterationExecutionModel.step_id == step_id
        )
    ...
    def _to_model(self, iteration: IterationExecution) -> IterationExecutionModel:
        return IterationExecutionModel(
            ...
            external_id=iteration.external_id,
