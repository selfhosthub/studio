...
class SQLAlchemyProviderRepository(ProviderRepository):
    ...
    async def update(self, provider: Provider) -> Provider:
        ...
        provider_model.visibility = provider.visibility
        provider_model.operational_status = provider.operational_status
    ...
    async def list_all(
        ...
        if operational_status is not None:
            stmt = stmt.where(ProviderModel.operational_status == operational_status)
        if provider_type is not None:
            stmt = stmt.where(ProviderModel.provider_type == provider_type)
        if visibilities is not None:
            stmt = stmt.where(ProviderModel.visibility.in_(visibilities))
...
class SQLAlchemyProviderServiceRepository(ProviderServiceRepository):
    ...
    def _to_domain(self, model: ProviderServiceModel) -> ProviderService:
        return ProviderService(
            ...
            custom_ui=model.custom_ui,
            custom_output=model.custom_output,
            parameter_title=model.parameter_title,
            client_metadata=model.client_metadata,
            created_by=model.created_by,
            created_at=model.created_at,
            updated_at=model.updated_at,
    ...
    async def create(self, service: ProviderService) -> ProviderService:
        ...
        service_model = ProviderServiceModel(
            ...
            custom_ui=service.custom_ui,
            custom_output=service.custom_output,
            parameter_title=service.parameter_title,
    ...
    async def update(self, service: ProviderService) -> ProviderService:
        ...
        service_model.custom_ui = service.custom_ui
        service_model.custom_output = service.custom_output
        service_model.parameter_title = service.parameter_title
...
class SQLAlchemyProviderCredentialRepository(ProviderCredentialRepository):
    ...
    def __init__(self, session: AsyncSession):
        self.session = session
        # Secret-class data MUST be encrypted at rest (CLAUDE.md hard rule). If the
        # encryption key is missing/invalid, get_credential_encryption() raises ValueError
        # and construction fails closed - credentials are never stored in plaintext.
    ...
    def _to_domain(self, model: ProviderCredentialModel) -> ProviderCredential:
        ...
        return ProviderCredential(
            ...
            created_by=model.created_by,
            client_metadata={},
            webhook_callback_token=model.webhook_callback_token,
            created_at=model.created_at,
            updated_at=model.updated_at,
        )
    ...
    async def create(self, credential: ProviderCredential) -> ProviderCredential:
        ...
        credential_model = ProviderCredentialModel(
            ...
            webhook_callback_token=credential.webhook_callback_token,
        )
    ...
    async def update(self, credential: ProviderCredential) -> ProviderCredential:
        ...
        credential_model.webhook_callback_token = credential.webhook_callback_token
    ...
    async def get_by_webhook_callback_token(
        self, token: str
    ) -> Optional[ProviderCredential]:
        """Resolve the credential that owns this inbound-callback routing token.

        The token is unique per credential; the inbound webhook handler uses it
        as the auth-gate key before demultiplexing the callback by generation id.
        """
        stmt = select(ProviderCredentialModel).where(
            ProviderCredentialModel.webhook_callback_token == token
        )
        result = await self.session.execute(stmt)
        credential_model = result.scalars().first()
