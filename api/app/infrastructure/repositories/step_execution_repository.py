...
class SQLAlchemyStepExecutionRepository(StepExecutionRepository):
    ...
    async def get_by_id(self, step_execution_id: uuid.UUID) -> Optional[StepExecution]:
    ...
    async def list_webhook_steps_awaiting_reconcile(
        self, timeout_minutes: int
    ) -> List[StepExecution]:
        """WAITING_FOR_WEBHOOK steps idle past the notify grace period.

        Drives the once-per-step "still awaiting provider callback" notification
        (I-13: WFW is never failed on a timer; this only notifies, never fails).
        """
        ...
        stmt = (
            ...
            .where(
                ...
                == StepExecutionStatus.WAITING_FOR_WEBHOOK.value,
    ...
    async def touch_updated_at(self, step_execution_ids: List[uuid.UUID]) -> int:
        """Bump updated_at without changing status.

        The WFW reconcile sweep uses this to throttle re-polling: after it
        enqueues poll jobs for a parked step, the step drops out of the
        reconcile window for another grace period instead of being re-polled
        every cleanup cycle. Status is untouched, so a late provider callback
        still resolves the still-PENDING iteration rows normally.
        """
        from sqlalchemy import update
