...
class GenericHTTPAdapter(BaseProviderAdapter):
    ...
    def build_request(
        ...
        ``system_extra`` merges extra keys into the Jinja ``system`` namespace
        (e.g. a per-iteration ``callback_url`` for webhook-completion injection).
        """
        endpoint_template = (
        ...
        # callback_url is always present (None unless a webhook-completion caller
        # supplies it) so templates can reference `system.callback_url` under
        # StrictUndefined without erroring in non-webhook modes.
        ...
        if system_extra:
            system_ctx.update(system_extra)
        ...
        if method in ("GET", "DELETE", "HEAD"):
            ...
            if body_transform:
                # request_transform.body overlays the mapped body; transform
                # keys win. With no parameter_mapping the transform is the
                # whole body.
                mapped_body = {
                    k: v
                ...
                rendered = self._render_template_dict(body_transform, context)
                rendered = self._strip_omit_sentinel(rendered)
                body = {**mapped_body, **rendered}
                if not body:
                    body = None
                if query_transform:
                    rendered_q = self._render_template_dict(query_transform, context)
    ...
    def _apply_auth(
        ...
        # No auth block declared -> nothing to apply (e.g. self-hosted providers
        # with requires_credentials=false). When a block IS declared, its type is
        # required by the provider schema and never guessed.
        if not self.auth_config:
            return built
        auth_type = self.auth_config.get("type")
        if not auth_type:
            raise ValueError(
                "provider auth config is present but required 'auth.type' is missing"
            )
        ...
        if auth_type == "bearer":
        ...
        elif auth_type in ("bearer_header", "api_key", "oauth2"):
            # Bearer-style: the access token / api key rides in the Authorization
            ...
            raise ValueError(
                f"unknown auth.type '{auth_type}'; declare a supported type "
                "(bearer, bearer_header, api_key, oauth2, custom, query_param, "
                "custom_header, header, none)"
            )
