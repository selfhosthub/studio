...
class ProviderLoader:
    ...
    async def load_provider(
        ...
        result = await session.execute(
            select(ProviderModel).where(
                ProviderModel.name == provider_name,
                ProviderModel.operational_status == "ACTIVE",
            )
        )
    ...
    async def load_all_providers(self, session: AsyncSession) -> int:
        ...
        result = await session.execute(
            select(ProviderModel).where(ProviderModel.operational_status == "ACTIVE")
        )
...
async def initialize_providers(session: AsyncSession, registry: AdapterRegistry) -> int:
    """Load DB providers at startup. There are no built-in adapters: core services
    execute inline and declarative providers come from the database."""
    loader = ProviderLoader(registry)
    count = await loader.load_all_providers(session)

    logger.info(f"Provider initialization complete: {count} providers loaded")
    return count
