...
class OrgFileCreator:
    ...
    async def create_from_downloaded_files(
        ...
        await self._enforce_single_selected_per_slot(
            resource_repo, job_execution_id, created
        )
    ...
    async def _enforce_single_selected_per_slot(
        self,
        resource_repo: SQLAlchemyOrgFileRepository,
        job_execution_id: UUID,
        created: List[OrgFile],
    ) -> None:
        """Exactly one is_selected per (job, iteration_index) slot just touched: the
        primary just-created variant; all other variants (batch siblings + prior
        regenerations) become deselected candidates."""
        if not created:
            return
        existing = await resource_repo.list_by_job(job_execution_id)
        fresh_ids = {r.id for r in created}
        for resource in OrgFile.enforce_single_selected_per_slot(existing, fresh_ids):
    ...
    def _build_resource(
        ...
        # `display_order` is the per-iteration position of this file (0..M-1).
        # The worker's `file_info["index"]` already carries that scale, so we
        # use it directly. `metadata["index"]` remains for informational use
        # (and stays the source of truth for the backfill migration).
        index_raw = file_info.get("index", 0)
        try:
            display_order = max(0, int(index_raw))
