...
class PackageVersionService:
    ...
    @staticmethod
    def _ensure_namespaced(slug: str) -> str:
        """Ensure slug has a namespace prefix to satisfy ck_package_versions_slug_namespaced.

        Catalog IDs may arrive flat from legacy URLs; storage requires
        `<namespace>/<slug>`. Default to `shs/` (the public community
        namespace) when missing. Idempotent on already-namespaced values.
        """
        if "/" in slug:
            return slug
        return f"shs/{slug}"
    ...
    async def record_version(
        ...
        """
        slug = PackageVersionService._ensure_namespaced(slug)
