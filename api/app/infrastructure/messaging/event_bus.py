...
class BaseEventBus(EventBus):
    ...
    # High-frequency, handler-less events whose publish is never actionable.
    # Logging them at DEBUG drowns out real publishes (6+ workers × heartbeat).
    _SILENT_PUBLISH_EVENTS = {"worker.heartbeat"}

    async def _publish_to_handlers(self, event: DomainEvent) -> None:
        event_type = event.event_type
        if event_type not in self._SILENT_PUBLISH_EVENTS:
            self._logger.debug(
