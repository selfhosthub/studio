...
class ResultProcessor:
    ...
    @staticmethod
    def _reproject_declared_outputs(
        result_data: Dict[str, Any],
        workflow_snapshot: Optional[Dict[str, Any]],
        step_id: str,
        output_extractor: OutputExtractor,
    ) -> None:
        """Re-project declared outputs from result_data after files are rebuilt.

        Output projection first runs before OrgFile resources exist, so derived
        paths into downloaded_files (e.g. video_url -> downloaded_files[0].url)
        resolve empty. Once downloaded_files is rebuilt from live resources, the
        declared outputs are re-projected in place, replacing result_data so
        undeclared keys do not leak downstream.
        """
        outputs_config = (
            (workflow_snapshot or {}).get("steps", {}).get(step_id, {}).get("outputs")
        )
        if not outputs_config:
            return
        projected = output_extractor.extract_from_data(result_data, outputs_config)
        result_data.clear()
        result_data.update(projected)
    ...
    def __init__(
        ...
        self.notifier = InstanceNotifier(
            ...
            # Chain EVENT-trigger workflows when an instance finishes. Only wired
            # when an event_bus is supplied (skipped in lightweight test setups).
            terminal_hook=self._fire_event_triggers if event_bus is not None else None,
        )
        ...
        # Serializes result events for one step so PROCESSING commits started_at
        # before COMPLETED reads the row; without it the two fire-and-forget tasks
        # race on separate sessions and a fast step lands started_at == completed_at.
    ...
    def _step_lock(self, step_id: str) -> asyncio.Lock:
        ...
        if lock is None:
            lock = asyncio.Lock()
            self._step_locks[step_id] = lock
        return lock

    async def _fire_event_triggers(
        self, instance: Any, action_type: str, session: AsyncSession
    ) -> None:
        """Notifier terminal hook: fan out to EVENT-trigger workflows bound to
        this instance's workflow. Lazy import avoids an import cycle."""
        from app.application.services.trigger_runtime import fire_event_triggers

        # Hook is only wired when an event_bus was supplied (see __init__).
    ...
    async def process_result(self, payload: Dict[str, Any]) -> None:
        ...
        if existing is not None:
            # Inline downstream steps reuse the outer session and already hold the
            # parent's lock; they have different step_ids so they never self-block.
    ...
    async def _process_with_session(
        ...
        if downloaded_files:
            ...
            # Keep-variants: the just-produced rows default is_selected=True; make
            # exactly one selected per slot (the fresh primary), deselecting prior
            # generations so only one variant flows downstream (I-15 / §4.9).
            for changed in OrgFile.enforce_single_selected_per_slot(
                await resource_repo.list_by_job(outcome.job.id), fresh_ids
            ):
                await resource_repo.update(changed)
            ...
            if (
                ...
                self._reproject_declared_outputs(
                    outcome.job.extracted_outputs,
                    (outcome.instance.workflow_snapshot or {}),
                    outcome.step_id,
                    self.processing_service.output_extractor,
                )
        ...
        # webhook_pending results park the step, not complete it - no step_completed.
        if parsed.is_iteration_job and not parsed.webhook_pending:
            await self.notifier.announce_state_change(
                instance=outcome.instance,
        ...
        if op is not None:
            ...
            # Keep-variants: regen no longer deletes originals; they were
            # deselected at creation (OrgFileCreator). Nothing to delete here.
