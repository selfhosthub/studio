...
from app.application.services.mapping_resolver import UnresolvedMappingError
...
from app.application.services.instance.helpers import cancel_non_terminal_steps
...
class ActionExecutor:
    ...
    async def execute(
        ...
        match action.action:
            ...
            case OrchestrationAction.WAIT_WEBHOOK:
                return await self._handle_wait_webhook(action, context)
    ...
    async def _cancel_remaining_non_terminal_steps(
        ...
        Thin wrapper over the canonical helper (I-13 companion). Without this,
        steps that were PENDING at instance creation but never enqueued (because
        the failure short-circuited orchestration) stay PENDING forever.
        """
        await cancel_non_terminal_steps(
            ctx.step_execution_repo,
            ctx.instance_id,
            context_label="instance fail sweep",
        )
    ...
    async def _handle_wait_manual(
        ...
        # All ready manual steps arm together; step_ids is the full set, with
        # waiting_step_id as the back-compat first entry.
        manual_step_ids = action.step_ids or (
            [action.waiting_step_id] if action.waiting_step_id else []
        )
        assert manual_step_ids, "WAIT_MANUAL action requires at least one manual step"

        # Instance transition is idempotent across the loop.
        ctx.instance.wait_for_manual_trigger(manual_step_ids[0])
        ...
        for manual_step_id in manual_step_ids:
        ...
        logger.info(f"Waiting for manual trigger: steps={manual_step_ids}")
    ...
    async def _handle_wait_webhook(
        self,
        action: NextAction,
        ctx: ActionContext,
    ) -> bool:
        """Park the instance + step in WAITING_FOR_WEBHOOK. Idempotent on re-entry.

        An iterated webhook step fires N times (one WAIT_WEBHOOK per iteration);
        the instance/step transitions are idempotent, but the parking
        announcement must fire ONCE - on the first entry into the waiting state -
        otherwise the user gets one "waiting for webhook" toast per iteration.
        """
        webhook_step_id = action.waiting_step_id
        assert (
            webhook_step_id is not None
        ), "WAIT_WEBHOOK action requires waiting_step_id"

        # First entry vs. an idempotent re-entry from a sibling iteration.
        already_waiting = (
            ctx.instance.status == InstanceStatus.WAITING_FOR_WEBHOOK
        )

        ctx.instance.wait_for_webhook(webhook_step_id)

        ctx.instance.output_data["pending_webhook"] = {
            "step_id": webhook_step_id,
            "waiting_since": datetime.now(UTC).isoformat(),
        }

        webhook_step_entity = await ctx.step_execution_repo.get_by_instance_and_key(
            ctx.instance_id, webhook_step_id
        )
        if webhook_step_entity:
            webhook_step_entity.wait_for_webhook()
            await ctx.step_execution_repo.update(webhook_step_entity)
        else:
            logger.error(
                f"StepExecution entity not found for webhook step "
                f"{webhook_step_id} on instance {ctx.instance_id}"
            )

        await ctx.instance_repo.update(ctx.instance)

        # Announce once: skip the re-entry announcements that each later
        # iteration would otherwise emit (one toast per iteration). Still commit
        # so each iteration's staged external_id (the callback match key) persists
        # - the announce path commits on first entry, but re-entries skip it.
        if already_waiting:
            await ctx.session.commit()
            return True

        logger.info(f"Waiting for webhook callback: step={webhook_step_id}")

        await self.notifier.announce_state_change(
            instance=ctx.instance,
            action_type="waiting_webhook",
            step_id=webhook_step_id,
            result_data={"waiting_for_webhook": True},
            session=ctx.session,
        )
