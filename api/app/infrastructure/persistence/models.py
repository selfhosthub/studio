...
# Shared PG enum types for the catalog-visibility axes. One CREATE TYPE each,
# reused across all four catalog package tables (workflows / prompts /
# comfyui_workflows / providers) - reusing the *same* Enum instance is what
# makes SQLAlchemy emit a single CREATE TYPE rather than one per table. Native
# enum labels are the Python member NAMES (uppercase): PRIVATE/STAGING/PUBLIC,
# ACTIVE/INACTIVE (invariant #1).
...
class OrganizationModel(Base):
    ...
    # Staging-tenant flag: orgs carrying this see staging-visibility catalog
    # packages. super_admin-settable only. Default false on the org-creation path.
    ...
    activated_by: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey(
            "users.id", use_alter=True, name="fk_organizations_activated_by_users"
        ),
        nullable=True,
    )
...
class WorkflowModel(Base):
    ...
    has_unresolved_refs: Mapped[bool] = mapped_column(
    ...
    schedule_dtstart: Mapped[Optional[datetime]] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True
    )
    schedule_rrule: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    schedule_timezone: Mapped[str] = mapped_column(String(64), default="UTC")
    schedule_enabled: Mapped[bool] = mapped_column(default=False)
    schedule_next_run_at: Mapped[Optional[datetime]] = mapped_column(
    ...
    # --- API trigger ---
    # The bearer key lives (encrypted, recoverable) in the referenced
    # OrganizationSecret; auth resolves the workflow from the URL {workflow_id} and
    # constant-time compares the presented key to the stored one.
    # Reference to the OrganizationSecret holding this workflow's recoverable
    # trigger creds. Non-unique (workflows may share a secret); ON DELETE SET NULL
    # so deleting the secret unlinks referencing workflows without cascading.
    trigger_secret_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True),
    ...
    event_source_workflow_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), nullable=True, index=True
    )
    event_on: Mapped[str] = mapped_column(String(20), default="completed")
    ...
    # Cross-org marketplace visibility. Orthogonal to scope/status/publish_status.
    ...
    # Reserved/unwired for workflows (column lands now; only providers wire it).
    ...
    __table_args__ = (
        ...
        Index(
            "ix_workflows_visibility_operational",
            "visibility",
            "operational_status",
        ),
    )
...
class StepExecutionModel(Base):
    ...
    max_retries: Mapped[int] = mapped_column(
        Integer, default=lambda: get_settings().DEFAULT_MAX_RETRIES
    )
...
class IterationExecutionModel(Base):
    ...
    # Provider generation id; nullable (webhook mode only). Indexed for the
    # inbound-callback match path (get_by_external_id). Non-unique: get-mode
    # rows leave it NULL and Postgres allows many NULLs.
...
class ProviderModel(Base):
    ...
    # Cross-org visibility. Providers are not org-scoped: existing rows map to
    # public (both ACTIVE/INACTIVE were always catalog-listed - see plan data
    # map); private/staging are net-new behaviour.
    visibility: Mapped[Visibility] = mapped_column(
        VISIBILITY_ENUM,
        nullable=False,
        default=Visibility.PUBLIC,
        server_default=Visibility.PUBLIC.name,
    )
    # Wired for providers in Phase 8 (migrated from the overloaded ProviderStatus).
    operational_status: Mapped[OperationalStatus] = mapped_column(
        OPERATIONAL_STATUS_ENUM,
        nullable=False,
        default=OperationalStatus.ACTIVE,
...
class ProviderServiceModel(Base):
    ...
    custom_ui: Mapped[bool] = mapped_column(
        Boolean, default=False, server_default="false", nullable=False
    )
    # Service manages its own output; UI hides the output schema section.
    custom_output: Mapped[bool] = mapped_column(
        Boolean, default=False, server_default="false", nullable=False
    )
    # Custom parameter-section title; UI falls back to "Service Parameters".
    parameter_title: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    client_metadata: Mapped[Dict[str, Any]] = mapped_column(JSON, default=dict)
...
class ComfyUIWorkflowModel(Base):
    ...
    # Cross-org visibility (default private). operational_status reserved/unwired.
    visibility: Mapped[Visibility] = mapped_column(
        VISIBILITY_ENUM,
        nullable=False,
        default=Visibility.PRIVATE,
    ...
    __table_args__ = (
        ...
        Index(
            "ix_comfyui_workflows_visibility_operational",
            "visibility",
            "operational_status",
        ),
    )
...
class ProviderCredentialModel(Base):
    ...
    # (/webhooks/incoming/{token}). Plaintext capability, not auth material - the
    # inbound bearer secret is webhook_callback_api_key inside secret_data. Lazily
    # minted on first save of a webhook_callback_api_key; immortal once set. NULL
    # for poll-only credentials.
    webhook_callback_token: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True, unique=True, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), default=lambda: datetime.now(UTC)
...
class PackageVersionModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "package_type",
            "version",
            name="uix_package_versions_slug_type_version",
        ),
        CheckConstraint(
            "slug LIKE '%/%' AND slug NOT LIKE '/%' AND slug NOT LIKE '%/'",
...
class WorkerModel(Base):
    ...
    # "local" if the worker shares the API's /workspace volume (writes results
    # directly); "remote" otherwise (uploads via HTTP). Reported on register
    # and every heartbeat; may flip if a network mount goes down/comes back.
...
class OrgFileModel(Base):
    ...
    # Active variant for its (step, iteration_index) slot; downstream uses only selected.
...
class PromptModel(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("organization_id", "slug", name="uix_prompt_org_slug"),
        CheckConstraint(
            "slug LIKE '%/%' AND slug NOT LIKE '/%' AND slug NOT LIKE '%/'",
            name="ck_prompt_slug_namespaced",
        ),
        Index(
            "ix_prompts_slug_pattern",
            text("slug text_pattern_ops"),
        ),
        Index(
            "ix_prompts_visibility_operational",
            "visibility",
            "operational_status",
        ),
    )
...
class DocumentationModel(Base):
    """Documentation page sourced from a marketplace source (community/plus).

    Global content (not org-scoped, no RLS - like providers/site_content).
    User and workflow docs are synced from the community source's
    docs-catalog.json; provider docs follow the provider install lifecycle
    (active toggles with install/uninstall).
    """
    ...
    # Namespaced for provider/workflow docs (e.g. 'shs/openai'), plain for
    # user docs (e.g. 'super-admin').
    slug: Mapped[str] = mapped_column(String(255))
    ...
    icon: Mapped[str] = mapped_column(String(100), default="box")
    # Markdown body. Payload-class data (invariant #4): never secret, never PII.
    ...
    # 'community' | 'plus' - which source to re-fetch from.
    source_tier: Mapped[str] = mapped_column(String(50), default="community")
    ...
    __table_args__ = (
        UniqueConstraint("slug", "doc_type", name="uix_documentation_slug_doc_type"),
        Index("ix_documentation_doc_type_active", "doc_type", "active"),
    )
