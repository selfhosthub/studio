...
async def websocket_endpoint(
    ...
    async def _reject(code: int, reason: str) -> None:
        """Release the IP slot, accept (RFC 6455 requires a subprotocol echo), then close."""
        await manager.release_ip(client_ip)
        try:
            if subprotocol:
                await websocket.accept(subprotocol=subprotocol)
    ...
    if organization_id:
        try:
            ...
            await _reject(status.WS_1003_UNSUPPORTED_DATA, "Invalid organization ID")
            return
        # Client-supplied org must match the authed user's org (strict: no super_admin
        # cross-org for operational feeds), matching the /ws/organization/{id} route.
        try:
            await validate_organization_access(str(org_id), user_typed)
        except HTTPException:
            await _reject(status.WS_1008_POLICY_VIOLATION, "Access denied")
            return
    ...
    if instance_id:
        try:
            ...
            await _reject(status.WS_1003_UNSUPPORTED_DATA, "Invalid instance ID")
            return
        # Derive the org from the instance row and validate ownership; never trust the
        # client to scope by an instance it doesn't own.
        try:
            instance = await instance_service.get_instance(inst_id)
        except Exception:
            instance = None
        if not instance:
            await _reject(status.WS_1008_POLICY_VIOLATION, "Instance not found")
            return
        try:
            ...
            try:
                ...
                if message.get("action") == "subscribe":
                    ...
                    if subscription_type == "organization":
                        try:
                            await validate_organization_access(
                                str(entity_id), user_typed
                            )
                        except HTTPException:
                            await manager.send_personal_message(
                                {"error": "Access denied"}, websocket
                            )
                            continue
                        ...
                        try:
                            instance = await instance_service.get_instance(entity_id)
                        except Exception:
                            instance = None
                        if not instance:
                            await manager.send_personal_message(
                                {"error": "Instance not found"}, websocket
                            )
                            continue
                        try:
                            await validate_organization_access(
                        ...
                        # A user may only subscribe to their own feed.
