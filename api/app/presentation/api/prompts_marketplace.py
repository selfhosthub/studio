...
async def _org_copy_prompt(
    ...
    # slug must be namespaced to satisfy ck_prompt_slug_namespaced.
    # Legacy URLs may pass a flat slug; default to the public `shs/` namespace.
    namespaced_slug = prompt_slug if "/" in prompt_slug else f"shs/{prompt_slug}"
...
async def install_custom_prompt(
    ...
    the org-side slug to prevent duplicates.
    """
    org_id = get_effective_org_id(None, current_user)
    ...
    # Idempotent check - slug is the namespaced form of the source prompt id.
    namespaced_id = prompt_id if "/" in prompt_id else f"shs/{prompt_id}"
    existing = await prompt_repo.get_by_slug(
        slug=namespaced_id,
        organization_id=uuid.UUID(org_id),
    )
    ...
    # Copy prompt to the org. slug already namespaced above.
    new_prompt = Prompt.create(
        organization_id=uuid.UUID(org_id),
