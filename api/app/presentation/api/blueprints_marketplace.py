...
from app.config.sources import VALID_TIERS
...
async def fetch_remote_blueprints_catalog(
    ...
    """Fetch blueprints catalog from community catalog (+ plus catalog if token), merge."""
...
async def refresh_catalog_from_remote(
    ...
    source_url = cat_config.build_url(cat_config.REPO_COMMUNITY, cat_config.BLUEPRINTS)
...
async def get_blueprints_catalog(
    ...
    Access control:
    - super_admin: sees all blueprints (community + plus)
    - admin: sees community blueprints + plus blueprints only if requirements met
...
async def get_blueprint(
    ...
    - admin: can only view community blueprints, or plus blueprints if requirements met
    """
    # Get set of installed package IDs from database
    installed_ids = await get_installed_package_ids(provider_repo)
...
async def refresh_blueprints_catalog(
    ...
    """
    Re-fetch the blueprints catalog from community catalog (+ plus catalog if token) and store in database.
    """
    token = await get_entitlement_token(current_user["org_id"], secret_repo)
