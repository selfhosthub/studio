...
_NAMESPACED_HALF_RE = r"^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]$|^[a-zA-Z0-9]$"


def validate_safe_package_name(name: str) -> str:
    """Reject path-traversal patterns; accept legacy flat slugs and namespaced (`a/b`) form.

    Prevents path traversal attacks by rejecting `..`, `\\`, and null bytes. A
    single `/` is allowed as the namespace boundary; both halves must
    independently match the safe-character pattern.

    Defense-in-depth: even with two-segment routing constraining the path
    shape, a malformed namespace or slug segment still reaches the handler and
    is later joined into a filesystem path, so this stays load-bearing.
    """
    if not name:
        raise HTTPException(
    ...
    if len(parts) > 2:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid package name: at most one '/' allowed (namespace/slug)",
        )

    for part in parts:
        if not re.match(_NAMESPACED_HALF_RE, part):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
def namespaced_id(
    namespace: str = Path(..., description="Package namespace (e.g. `shs`)"),
    slug: str = Path(..., description="Package slug within the namespace"),
) -> str:
    ...
    receive one validated id instead of hand-rolling the reassembly.
    """
    return validate_safe_package_name(f"{namespace}/{slug}")


# Annotated alias so handlers can write `pkg_id: NamespacedId` instead of the
# full Depends(...) spelling.
NamespacedId = Annotated[str, Depends(namespaced_id)]
...
class CurrentUser(TypedDict):
    ...
    email: Optional[str]
    role: Optional[str]
    # Always present: get_current_user resolves it from the DB (the user->org
    # JOIN is required, so an authenticated user always has an org).
    org_id: str
...
async def get_documentation_repository(
    session: AsyncSession = Depends(get_db_session),
) -> AsyncGenerator[DocumentationRepository, None]:
    """documentation is NOT RLS-protected (global content)."""
    yield SQLAlchemyDocumentationRepository(session)
...
async def get_instance_service(
    ...
    resource_repo: OrgFileRepository = Depends(get_org_file_repository),
    event_bus: EventBus = Depends(get_event_bus),
    session: AsyncSession = Depends(get_db_session),
) -> InstanceService:
...
async def get_webhook_service_public(
    ...
    # so the seeded retrieve job shares this request's session.
    result_processor = getattr(request.app.state, "result_processor", None)
    ...
    return WebhookService(
        ...
        provider_credential_repository=SQLAlchemyProviderCredentialRepository(session),
        ...
        iteration_execution_repository=SQLAlchemyIterationExecutionRepository(session),
        job_enqueue_service=instance_service.job_enqueue_service,
        process_result_fn=(
            result_processor.process_result if result_processor else None
        ),
        form_field_resolver=FormFieldResolver(
            provider_service_repository=SQLAlchemyProviderServiceRepository(session),
            prompt_repository=SQLAlchemyPromptRepository(session),
        ),
        organization_secret_repository=SQLAlchemyOrganizationSecretRepository(session),
    )
