...
async def update_organization(
    ...
    """Requires admin privileges. Admins are restricted to their own org (super_admin may
    cross-org); branding and staging-tenant changes require super_admin."""
    ...
    # Staging-tenant flag is super_admin-only (catalog visibility gate)
    if (
        update_data.is_staging is not None
        and user.get("role") != "super_admin"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only super-admins can change staging-tenant status",
        )
    ...
    try:
        ...
        dto = OrganizationUpdateDTO(
            ...
            is_staging=update_data.is_staging,
            updated_by=current_user_id,
        )
        updated_org = await service.update_organization(
            organization_id, dto, current_user_id
        )
        ...
        if existing_org:
            ...
            if (
                ...
                and update_data.description != existing_org.description
            ):
            ...
            if update_data.settings is not None:
                ...
                if {k: v for k, v in old_settings.items() if k != "branding"} != {
                    k: v for k, v in new_settings.items() if k != "branding"
                }:
                    changes["settings"] = {"changed": True}
            if (
                update_data.is_active is not None
                and update_data.is_active != existing_org.is_active
            ):
                changes["is_active"] = {
                    "old": existing_org.is_active,
                    "new": update_data.is_active,
                }
            if (
                update_data.is_staging is not None
                and update_data.is_staging
                != existing_org.is_staging
            ):
                changes["is_staging"] = {
                    "old": existing_org.is_staging,
                    "new": update_data.is_staging,
                }

        # Determine severity based on what changed
        severity = AuditSeverity.INFO
        if "is_active" in changes or "is_staging" in changes:
...
async def update_user_as_admin(
    ...
    try:
        ...
        if existing_user:
            ...
            if (
                update_data.is_active is not None
                and update_data.is_active != existing_user.is_active
            ):
                changes["is_active"] = {
                    "old": existing_user.is_active,
                    "new": update_data.is_active,
                }
...
async def list_organization_credentials(
    ...
    try:
        ...
        logger.exception(
            "Failed to fetch credentials for organization %s", organization_id
