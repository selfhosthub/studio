...
async def get_provider_doc_content(slug: str) -> DocContent:
    """Public; reads catalog.providers[].file. Slug is namespaced (e.g. shs/openai)."""
    catalog = _load_catalog()
    ...
    provider_path = os.path.join(DOCS_PATH, provider_info.get("file", f"providers/{slug}.md"))
...
async def get_workflow_doc_content(slug: str) -> DocContent:
    """Public; reads catalog.workflows[].file. Slug is namespaced (e.g. shs/item-groups)."""
    catalog = _load_catalog()
    ...
    workflow_path = os.path.join(DOCS_PATH, workflow_info.get("file", f"workflows/{slug}.md"))
