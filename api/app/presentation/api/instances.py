...
async def create_instance(
    ...
    # Run-block: the system (super) org cannot run instances. Staging-visibility
    # packages must be validated through a real staging organization, not the super-org
    # that authors them (it sees everything, which would mask staging gaps).
    org_id = user.get("org_id")
    if org_id:
        caller_org = await organization_repo.get_by_id(UUID(org_id))
        if caller_org and (caller_org.settings or {}).get("is_system"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    "The system organization cannot run workflow instances. "
                    "Use a staging organization to test."
                ),
            )
...
async def trigger_step(
    ...
    # New shape carries every armed step; tolerate the legacy single-step shape.
    pending_step_ids = pending_trigger.get("step_ids")
    if pending_step_ids is None:
        legacy = pending_trigger.get("step_id")
        pending_step_ids = [legacy] if legacy else []

    # Sibling trigger while the instance is already PROCESSING but step still armed.
    armed_during_processing = (
        instance.status == "processing" and step_id in pending_step_ids
    )
    if instance.status != "waiting_for_manual_trigger" and not armed_during_processing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if step_id not in pending_step_ids:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Step {step_id} is not an armed manual-trigger step (armed: {pending_step_ids})",
        )
...
class CheckWebhookStatusResponse(BaseModel):
    """Result of a user-initiated provider poll on a parked webhook step."""
...
@router.post(
    "/{instance_id}/step_executions/{step_id}/check-webhook-status",
    response_model=CheckWebhookStatusResponse,
)
async def check_webhook_status(
    ...
    """User-initiated provider poll for a step awaiting a webhook callback.

    Not a fallback and not an auth bypass: it fetches the result already on the
    provider's side using the step's own credential (same data a rerun would
    pull), preserving instance state. Same permission as rerun.
    """
    instance = await service.get_instance(instance_id)
    if not instance:
        raise HTTPException(
    ...
    await _audit_instance_action(
        ...
        instance,
        resource_type=ResourceType.INSTANCE_STEP,
        metadata={"step_id": step_id, "mode": "check_webhook_status"},
    )

    logger.info(
        f"Step {step_id} for instance {instance_id} "
        f"check-webhook-status by user {user.get('id')} via API"
    )

    try:
        result = await service.check_webhook_status(instance_id, step_id)
    except DomainException as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=safe_error_message(e)
        )
    return CheckWebhookStatusResponse(**result)
...
async def upload_files_to_step(
    ...
    user: CurrentUser = Depends(require_tenant),
    service: InstanceService = Depends(get_instance_service),
    resource_service: OrgFileService = Depends(get_org_file_service),
