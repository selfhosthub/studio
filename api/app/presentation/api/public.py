...
from app.infrastructure.security.asset_signing import verify_asset
...
# Upload writes exactly {16 hex}.jpg; anything else cannot be an avatar and is
# rejected before any filesystem access, so path traversal is structurally impossible.
_AVATAR_FILENAME = re.compile(r"^[0-9a-f]{16}\.jpg$")


@router.get("/avatars/{org_id}/{filename}")
async def get_avatar(org_id: UUID, filename: str) -> FileResponse:
    """Serve a re-encoded avatar JPEG. Unauthenticated, hardcoded to the avatars dir only."""
    ...
    if not avatar_path.is_file():  # codeql[py/path-injection]
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")

    return FileResponse(
        path=str(avatar_path),  # codeql[py/path-injection]
...
@router.get("/assets/{org_id}/{rel_path:path}")
async def get_signed_asset(
    ...
    """Serve a workspace asset for external provider fetches. HMAC-signed + expiring, org-scoped."""
    if ".." in rel_path or rel_path.startswith("/"):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")

    if not verify_asset(str(org_id), rel_path, exp, sig):
        raise HTTPException(
    ...
    org_root = (Path(settings.WORKSPACE_ROOT) / "orgs" / str(org_id)).resolve()  # codeql[py/path-injection]
    asset_path = (org_root / rel_path).resolve()  # codeql[py/path-injection]
    # Defense in depth: the resolved path must stay inside the org's directory.
    if not asset_path.is_relative_to(org_root) or not asset_path.is_file():  # codeql[py/path-injection]
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found")

    media_type = mimetypes.guess_type(asset_path.name)[0] or "application/octet-stream"
    return FileResponse(
        path=str(asset_path),  # codeql[py/path-injection]
