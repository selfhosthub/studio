...
from app.infrastructure.logging.obfuscation import mask_id
...
def parse_token_with_expiration(token_value: str) -> Tuple[str, Optional[datetime]]:
    ...
    if match:
        ...
        try:
            ...
            logger.warning(f"Invalid date format in token suffix: {date_str}")  # codeql[py/clear-text-logging-sensitive-data]
...
class SecretUsageResponse(BaseModel):
    """How many workflows trigger off this secret (and a sample of their names),
    so the UI can warn before deletion that confirming resets them to manual."""
...
async def list_secrets(
    ...
    """Metadata-only listing. Admin-only. Protected secrets (e.g. ENTITLEMENT_TOKEN) are super_admin-only."""
    try:
        ...
        logger.error(f"Failed to list secrets: {safe_error_message(e)}")
        raise HTTPException(
...
async def get_secret(
    ...
    """Returns DECRYPTED secret_data. Admin-only. Logs a secret_revealed audit event. Protected secrets are super_admin-only."""
    try:
        org_id = user.get("org_id") or user.get("organization_id")
        user_role = user.get("role", "user")
        if not org_id:
        ...
        logger.error(f"Failed to get secret {mask_id(secret_id)}: {safe_error_message(e)}")  # codeql[py/clear-text-logging-sensitive-data]
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_secret(
    ...
    try:
        ...
        logger.error(f"Failed to update secret {mask_id(secret_id)}: {safe_error_message(e)}")  # codeql[py/clear-text-logging-sensitive-data]
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.get("/secrets/{secret_id}/usage", response_model=SecretUsageResponse)
async def get_secret_usage(
    secret_id: UUID4 = Path(..., description="Secret ID"),
    user: CurrentUser = Depends(require_admin),
    workflow_service: WorkflowService = Depends(get_workflow_service),
):
    """Admin only. How many workflows trigger off this secret — feeds the in-use
    warning before deletion. Deleting an in-use secret resets those workflows to a
    manual trigger."""
    usage = await workflow_service.get_trigger_secret_usage(uuid.UUID(str(secret_id)))
    return SecretUsageResponse(**usage)
...
async def delete_secret(
    ...
    """Admin only. Protected secrets return 403. Any workflows triggering off the
    secret are reset to a manual trigger first, so none is left referencing a
    deleted credential."""
    try:
        org_id = user.get("org_id") or user.get("organization_id")
        if not org_id:
        ...
        # Fall dependent workflows back to manual before the credential vanishes.
        # Skipped for protected secrets — repo.delete rejects those below anyway.
        if secret and secret.can_delete():
            await workflow_service.reset_workflows_for_trigger_secret(
                uuid.UUID(str(secret_id))
            )
        ...
        logger.error(f"Failed to delete secret {mask_id(secret_id)}: {safe_error_message(e)}")  # codeql[py/clear-text-logging-sensitive-data]
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
