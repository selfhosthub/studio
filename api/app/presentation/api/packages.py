...
async def upload_package(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # `/` in namespaced slugs would otherwise create temp subdirectories.
        json_path = temp_path / f"{provider_slug.replace('/', '__')}.json"
        ...
        visibility_on_insert = (
            Visibility.PUBLIC if is_reserved else Visibility.PRIVATE
        )
        try:
...
async def install_from_path(
    ...
    # Files mirror the namespaced id at <type>/<namespace>/<slug>.json,
    # so `request.package_id` (e.g. `shs/foo`) is the relative path under
    # the providers source dir.
    ...
    # Bundled first-party install path — authorized to write `shs/`.
...
async def download_provider_package(
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            # Namespaced package ids (e.g. "shs/core") embed a `/`, so the
            # dest path may include subdirectories that don't exist yet.
...
async def refresh_package(
    ...
    """
    # Files mirror the namespaced id at <type>/<namespace>/<slug>.json, so
    # package_name (e.g. "shs/core") is the relative path under the source dir.
    # Try community source, then plus source.
...
async def install_all_from_catalog(
    ...
    for package in catalog.packages:
        ...
        # Files mirror the namespaced id; `package.path` carries the relative
        # path under the providers source dir (e.g. `shs/foo`).
        source_dir = _providers_source_for_tier(package.tier)
        local_source_path: Path | None = None
        if source_dir is not None:
            file_path = package.path or package_id
            candidate = source_dir / f"{file_path}.json"
            if candidate.exists():
        ...
        try:
            ...
            if local_source_path is not None:
                # Bundled first-party source mount; authorized to write `shs/`.
