...
async def get_workflow_form_schema(
    ...
    # `workflow.steps` comes from a JSONB column whose key order is not preserved,
    # so iterate in topological (depends_on) order to match the flow editor.
    ordered_step_ids = order_steps_topologically({"steps": workflow.steps})
    step_order = 0
