...
"""OAuth2 endpoints. Each credential supplies its own client_id/client_secret (stored in the DB).
...
async def oauth_authorize(
    ...
    credential_id: str = Query(
        ..., description="Credential to authorize (must have client_id/client_secret)"
    ),
    ...
    """Start OAuth flow for a credential. The credential must have client_id/client_secret in the DB. Re-authorize uses the same endpoint."""
    ...
    logger.info(
        ...
        f"using credential {credential_id}"
    )
...
async def oauth_callback(
    ...
    if error:
        ...
        return RedirectResponse(
            url=_build_oauth_redirect_url(  # codeql[py/url-redirection]
    ...
    try:
        # Preserve client_id/client_secret on the credential; add/refresh tokens.
        ...
        logger.info(
            f"OAuth tokens stored in credential {credential_id} "
...
async def oauth_refresh(
    ...
    if not client_id or not client_secret:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Credential is missing client_id/client_secret. Re-create it with client credentials.",
        )
    ...
    # Preserve client_id/client_secret; update tokens.
    credential.credentials = {
        "client_id": client_id,
        "client_secret": client_secret,
        "access_token": tokens.get("access_token"),
...
async def list_oauth_providers(
    ...
    """Returns providers grouped by oauth_provider with merged scopes."""
