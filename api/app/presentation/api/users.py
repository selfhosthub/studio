...
async def upload_user_avatar(
    ...
    try:
        ...
        # The filename is deterministic (same path every upload), so browsers
        # would serve a stale cached image. A content-derived ?v= token changes
        # whenever the image changes, busting the cache wherever avatar_url is rendered.
        version = hashlib.sha256(processed_image).hexdigest()[:8]
        avatar_url = (
            f"/api/v1/public/avatars/{user.organization_id}/{filename}?v={version}"
        )
...
async def delete_user_avatar(
    ...
    try:
        ...
        # Empty string clears the column; None means "no change" (see
        # User.update_profile). Must use "" here or the avatar_url would persist
        # after the file is unlinked, leaving a broken link in the UI.
        update_command = UserUpdate(
            avatar_url="",
        )
        await org_service.update_user(
