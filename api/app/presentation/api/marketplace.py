...
from app.application.services.catalog_merge_service import (
    caller_is_staging,
    merge_providers_with_marketplace,
)
...
async def _caller_is_staging(db: AsyncSession, org_id: uuid.UUID) -> bool:
    return await caller_is_staging(db, org_id)
...
async def fetch_catalog_from_remote(
    ...
    """Fetch providers catalog from community catalog (+ plus catalog if token), merge."""
...
async def refresh_catalog_from_remote(
    ...
    """Fetch catalog from community catalog (+ plus catalog if token) and store in database."""
    remote_catalog = await fetch_catalog_from_remote(token=token)
    source_url = cat_config.build_url(cat_config.REPO_COMMUNITY, cat_config.PROVIDERS)
...
async def get_catalog(
    ...
    # Auto-refresh if stale (only community catalog - no token on GET)
    ...
    merged_providers = await merge_providers_with_marketplace(
        db, caller_is_staging=caller_staging
    )
    entitled_slugs = {e.id for e in merged_providers}

    active_all = await provider_repo.list_all(
        skip=0,
        limit=settings.DEFAULT_FETCH_LIMIT,
        operational_status=OperationalStatus.ACTIVE,
    )
    db_providers = [p for p in active_all if p.slug in entitled_slugs]
    ...
    inactive_all = await provider_repo.list_all(
        skip=0,
        limit=settings.DEFAULT_FETCH_LIMIT,
        operational_status=OperationalStatus.INACTIVE,
    )
    inactive_providers = [p for p in inactive_all if p.slug in entitled_slugs]
    ...
    for pv in active_provider_pvs:
        ...
        if pkg_slug in all_packages:
            continue
        if pkg_slug not in entitled_slugs:
            continue
...
@router.get(
    "/packages/{namespace}/{slug}",
    response_model=MarketplacePackage,
    operation_id="get_marketplace_package",
)
async def get_package(
    ...
    """Get details for a specific package by namespaced id (`<namespace>/<slug>`).
    ...
    if provider and package_id in entitled_slugs:
        ...
        return MarketplacePackage(
            ...
            or metadata.get("tier", DEFAULT_TIER),
...
@router.post(
    ...
    description="Fetch the latest catalog from community catalog (+ plus catalog if token) and store in database. Super admin only.",
)
async def refresh_catalog(
    ...
    Fetches from community catalog and plus catalog (, if ENTITLEMENT_TOKEN
