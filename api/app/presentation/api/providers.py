...
def validate_safe_slug(slug: str, field_name: str = "slug") -> str:
    """Reject slugs with path traversal patterns before they reach filesystem paths.

    Accepts both legacy flat slugs (`openai`) and namespaced slugs
    (`shs/openai`). Both halves of a namespaced slug must individually
    pass the same character rules; `..`, `\\`, and null bytes are
    rejected outright in either form.
    """
    if not slug:
        raise HTTPException(
    ...
    if len(parts) > 2:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid {field_name}: at most one '/' allowed (namespace/slug)",
        )
    half_re = r"^[a-zA-Z0-9][a-zA-Z0-9._-]*[a-zA-Z0-9]$|^[a-zA-Z0-9]$"
    for part in parts:
        if not re.match(half_re, part):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
from app.application.services.catalog_merge_service import (
    ...
    caller_is_staging as _caller_is_staging,
)
...
from app.domain.provider.models import CredentialType, PackageType
...
from app.domain.audit.models import AuditAction, AuditActorType, ResourceType
...
@router.get("/", response_model=Union[List[ProviderRead], List[ProviderSlim]])
async def list_providers(
    skip: int = Query(0, ge=0),
    limit: int = Query(settings.API_PAGE_LIMIT_DEFAULT, ge=1, le=settings.API_PAGE_MAX),
    slim: bool = Query(
        False,
        description="Return only {id, slug} for every provider (no services, "
        "no pagination). For identity-only callers like workflow readiness.",
    ),
    ...
    Super-admins see every provider. Non-super-admins are gated to
    operationally-active providers whose visibility is allowed for their org
    (public always; staging only for staging-flagged orgs; private never).

    ``slim=true`` returns the full set as {id, slug} only, skipping the
    per-provider services hydration - the same visibility gating applies.
    """
    is_super_admin = user.get("role") == "super_admin"

    if is_super_admin:
        if slim:
            providers = await service.list_providers_slim()
            return [ProviderSlim.model_validate(p) for p in providers]
        return await service.list_providers(skip=skip, limit=limit)

    caller_org_id = uuid.UUID(user["org_id"])
    caller_staging = await _caller_is_staging(db, caller_org_id)
    allowed_visibilities = _allowed_visibilities(caller_staging)
...
async def update_provider(
    ...
    """Requires SUPER_ADMIN role."""
    try:
        old = await service.get_provider(provider_id)
        if old is None:
        ...
        result = await service.update_provider(provider_id, provider_update)
    except EntityNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=safe_error_message(e))

    if (
        provider_update.visibility is not None
        and old.visibility != result.visibility
    ):
        await audit_service.log_event(
            ...
            changes={
                "visibility": {
                    "old": old.visibility.value,
                    "new": result.visibility.value,
                }
            },
        )
...
async def get_provider_package_defaults(
    ...
    client_metadata = {
        ...
        "slug": provider_slug,
        "tier": provider_data.get("tier", DEFAULT_TIER),
        # category is schema-required and enforced at install - read it directly.
...
async def create_credential(
    ...
    try:
        ...
        command = ProviderCredentialCreate(
            ...
            webhook_callback_token=credential_data.webhook_callback_token,
        )
