...
class WorkflowStepPreview(BaseModel):
    """One step in the pre-install workflow preview."""
    ...
    type: str = ""
    depends_on: List[str] = []
    service: Optional[str] = None  # provider_id/service_id binding it uses


class WorkflowConnectionPreview(BaseModel):
    """A provider/service connection the workflow's steps require."""

    provider: str  # provider slug, e.g. "shs/openai"
    services: List[str] = []  # service ids used from that provider


class WorkflowIOField(BaseModel):
    """A run-form input or step output exposed by the workflow."""
...
class MarketplaceWorkflowDetail(MarketplaceWorkflow):
    """Full pre-install detail for one workflow (super-admin)."""

    steps: List[WorkflowStepPreview] = []
    connections: List[WorkflowConnectionPreview] = []
    credentials: List[str] = []  # distinct provider slugs needing credentials
    inputs: List[WorkflowIOField] = []
    outputs: List[str] = []
...
async def fetch_remote_workflows_catalog(
    ...
    """Fetch workflows catalog from community catalog (+ plus catalog if token), merge."""
...
async def refresh_catalog_from_remote(
    ...
    source_url = cat_config.build_url(cat_config.REPO_COMMUNITY, cat_config.WORKFLOWS)
...
def build_workflow_preview(
    entry: MarketplaceWorkflow,
    workflow_data: Dict[str, Any],
) -> MarketplaceWorkflowDetail:
    """Derive a pre-install preview (steps, connections, credentials, I/O) from
    the raw workflow file. Pure transform - no DB, no network."""
    raw_steps = workflow_data.get("steps", {})
    steps: List[WorkflowStepPreview] = []
    # provider slug -> set of service ids it contributes
    conn_map: Dict[str, set[str]] = {}

    items = raw_steps.items() if isinstance(raw_steps, dict) else []
    for step_id, sd in items:
        if not isinstance(sd, dict):
            continue
        job = sd.get("job", {}) if isinstance(sd.get("job"), dict) else {}
        # Service routing fields live on the step or nested in job.
        provider = sd.get("provider_id") or job.get("provider_id")
        service = sd.get("service_id") or job.get("service_id")
        service_label = (
            f"{provider}/{service}" if provider and service else (service or provider)
        )
        if provider:
            conn_map.setdefault(provider, set())
            if service:
                conn_map[provider].add(service)
    ...
    connections = [
        WorkflowConnectionPreview(provider=p, services=sorted(s))
        for p, s in sorted(conn_map.items())
    ]

    # Run-form inputs come from the trigger input schema (JSON Schema props).
    inputs: List[WorkflowIOField] = []
    schema = workflow_data.get("trigger_input_schema") or {}
    ...
    # Outputs: distinct output keys declared across steps.
    outputs: List[str] = []
    for _sid, sd in items:
        if not isinstance(sd, dict):
            continue
        step_outputs = sd.get("outputs", {})
        if isinstance(step_outputs, dict):
            outputs.extend(step_outputs.keys())
        elif isinstance(step_outputs, list):
            outputs.extend(o for o in step_outputs if isinstance(o, str))
    outputs = sorted(set(outputs))

    return MarketplaceWorkflowDetail(
        **entry.model_dump(),
        steps=steps,
        connections=connections,
        credentials=sorted(conn_map.keys()),
        inputs=inputs,
        outputs=outputs,
    )
...
async def _org_admin_catalog_workflows(
    ...
    for pv in active:
        ...
        wf_tier = entry.get("tier", DEFAULT_TIER)
    ...
    return WorkflowsCatalogResponse(
        ...
        workflows=workflows,
        filter_options={
            "tier": sorted(tiers_set) if tiers_set else list(VALID_TIERS),
...
@router.get("/catalog/{namespace}/{slug}", response_model=MarketplaceWorkflowDetail)
async def get_workflow_detail(
    ...
    """Full pre-install detail for one catalog workflow (super-admin).

    The catalog list ships metadata only; the step DAG, required connections,
    credentials, and I/O are derived on demand from the per-item workflow file
    so the list stays lean at hundreds of workflows.
    """
    ...
    entry = next((w for w in catalog.workflows if w.id == workflow_id), None)
    if not entry:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    installed_ids = await get_installed_package_ids(provider_repo)
    installed_prompt_slugs = await get_platform_installed_prompt_slugs(db)
    prompt_slugs = extract_prompt_slugs(entry.model_dump())
    missing = [req for req in entry.requires if req not in installed_ids]
    missing_prompts = [s for s in prompt_slugs if s not in installed_prompt_slugs]

    market_entry = MarketplaceWorkflow(
        id=entry.id,
    ...
    workflow_data = await _load_workflow_data(
        entry, current_user["org_id"], secret_repo
    )
    return build_workflow_preview(market_entry, workflow_data)
    ...
    merged_metadata.update(
        {
            ...
            "marketplace_tier": catalog_entry.get("tier", DEFAULT_TIER),
...
async def refresh_workflows_catalog(
    ...
    """
    Re-fetch the workflows catalog from community catalog (+ plus catalog if token) and store in database.
    """
