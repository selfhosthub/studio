...
from app.domain.provider.models import ProviderType, ServiceType
...
class ProviderRead(ProviderBase):
    ...
    # Operational axis (active/inactive); serialized alias for operational_status.
    status: OperationalStatus
    # Cross-org marketplace visibility (private/staging/public).
    ...
    tier: Optional[str] = Field(
        default=None, description="Provider tier: 'community' or 'plus'"
    )
...
class ProviderSlim(BaseModel):
    """Minimal provider projection: id + slug only.

    For callers that only need provider identity (e.g. workflow-readiness
    checks that match step provider references against installed providers).
    Skips the heavy services/config/metadata hydration of ProviderRead.
    """
...
class CredentialCreate(CredentialBase):
    ...
    webhook_callback_token: Optional[str] = Field(
        default=None,
        description="Client-generated callback routing token (the random string in the inbound callback URL); persisted as-is when provided",
    )
...
class CredentialRead(BaseModel):
    ...
    webhook_callback_token: Optional[str] = Field(
        default=None,
        description="Stable routing key for the inbound callback URL; null until first webhook save",
    )
...
class ProviderServiceRead(ProviderServiceBase):
    ...
    id: UUID
    provider_id: UUID
    # Declarative UI flags (real columns).
    custom_ui: bool = False
    custom_output: bool = False
    parameter_title: Optional[str] = None
    # Lifted from client_metadata so the UI reads them without name-matching.
