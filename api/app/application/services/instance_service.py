...
class InstanceService:
    def __init__(
        ...
        self._state_transition = StateTransitionService(
            ...
            notifier=notifier,
            iteration_execution_repo=iteration_execution_repository,
        )
            ...
            if result == "STOP":
                ...
                if self._notifier:
                    await self._notifier.announce_state_change(
                        ...
                        session=getattr(self.instance_repository, "session", None),
                    )
            elif result == "SKIP":
                workflow_snapshot = instance.workflow_snapshot
                    ...
                    if next_jobs == ["STOP"]:
                        ...
                        if self._notifier:
                            await self._notifier.announce_state_change(
                                ...
                                action_type="completed",
                                session=getattr(
                                    self.instance_repository, "session", None
                                ),
                            )
                    elif not next_jobs:
                        instance.status = InstanceStatus.COMPLETED
    ...
    async def check_webhook_status(
        ...
        """User-initiated, one-shot provider poll for a parked WFW step.

        Not a fallback and not an auth bypass (see docs/plans/no-timer-fails-a-step.md):
        it is the user doing what a rerun would do, but cheaper - it preserves
        instance state and just fetches the result already on the provider's side,
        authenticated by the step's own credential. Enqueues one single-shot poll
        per still-pending iteration; the result flows through the normal completion
        path. Returns {polls_enqueued, iterations_pending} so the UI can show
        "checking N generations" vs. "still generating".
        """
        if not self.job_enqueue_service or not self.iteration_execution_repository:
            raise BusinessRuleViolation(
                "Check Status is unavailable: poll plumbing not configured."
            )
        ...
        if step is None:
            raise EntityNotFoundError(
                ...
                entity_id=step_id,
                code=f"Step {step_id} not found on instance {instance_id}",
            )
        if step.status != StepExecutionStatus.WAITING_FOR_WEBHOOK:
            raise BusinessRuleViolation(
                "Check Status only applies to a step awaiting a provider callback."
            )

        workflow = await self.workflow_repository.get_by_id(instance.workflow_id)
        step_config = workflow.steps.get(step_id) if workflow else None
        job = step_config.job if step_config else None
        if job is None:
            raise BusinessRuleViolation(
                "Cannot resolve the step's provider configuration to poll."
            )
        ...
        all_rows = await self.iteration_execution_repository.list_by_step_id(step.id)
        pending = [
            r
            for r in all_rows
            if r.status == IterationExecutionStatus.PENDING and r.external_id
        ]

        polls_enqueued = 0
        for row in pending:
            group_total = sum(
                1 for r in all_rows if r.iteration_group_id == row.iteration_group_id
            )
            group_id = (
                str(row.iteration_group_id) if row.iteration_group_id else None
            )
            job_id = await self.job_enqueue_service.enqueue_webhook_poll_job(
                ...
                generation_id=str(row.external_id),
                iteration_index=row.iteration_index,
                iteration_count=group_total,
