...
class StepResultHandler:
    ...
    def handle_failed(
        ...
        # Update instance step via entity (source of truth). StepExecution.fail
        # is permissive (no source-state validation), so there is no concurrent-
        # cancel transition error to catch here - the CANCELLED guard above is
        # the only skip path.
        if instance_step:
    ...
    def handle_completed(
        ...
        # Capture the step's status BEFORE job.complete() mutates it - `job is
        # instance_step`, so completing the job flips instance_step to COMPLETED
        # and would wrongly trip the regeneration branch below, re-stamping
        # started_at and collapsing processing time to 0s.
        instance_step_status = instance_step.status if instance_step else None
        ...
        if instance_step:
            # Non-None whenever instance_step is (both captured together above).
            assert instance_step_status is not None
            pre_transition_status = instance_step_status
            try:
                # Handle various starting states
                if instance_step_status in (
                    StepExecutionStatus.PENDING,
                    StepExecutionStatus.WAITING_FOR_MANUAL_TRIGGER,
                ):
                    instance_step.start()
                elif instance_step_status in (
                    StepExecutionStatus.COMPLETED,
                    StepExecutionStatus.FAILED,
                ):
                    # Regeneration: step was already done, restart it
