...
def resources_to_downloaded_files(
    ...
    Returns:
        ...
        - display_order (per-iteration position; user-editable via reorder)
        - created_at descending (newest first for regenerated images sharing
          the same display_order slot during the staged-deletion window)
    """
    # Only the selected variant per slot flows downstream; deselected variants
    # (prior regenerations) are kept for the user but never consumed.
    resources = [r for r in resources if getattr(r, "is_selected", True)]

    # `display_order` is the single authority for within-iteration ordering.
    # It's assigned at creation from worker `file_info["index"]` and rewritten
    # by user reorder. `-created_at` is the tiebreaker so regen's newest
    # replacement wins while the original is still in the staged-deletion
    # window. See docs/plans/iteration-reorder-persistence.md §5.
    ...
    for resource in sorted_resources:
        ...
        # Emit the workspace-relative virtual_path as the `url`. Enqueue-time
        # parameter resolution signs it into an /api/v1/public/assets URL; the
        # old absolute /uploads URL pointed at a removed static mount and 404'd.
        url = resource.virtual_path or ""
