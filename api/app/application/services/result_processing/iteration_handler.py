...
a dedicated table don't collide, but the all-iterations-done count is a
read-modify-write across siblings: `track_iteration_via_rows` takes a
FOR UPDATE lock on the parent StepExecution row to serialize it (see there).
...
class IterationHandler:
    ...
    async def track_iteration_via_rows(
        ...
        *,
        ...
        Takes a FOR UPDATE lock on the parent StepExecution row to serialize the
        all-done count across concurrent sibling-result transactions (see step 1).
        ...
        #    FOR UPDATE serializes concurrent iteration-result transactions: each
        #    must hold this row before counting terminal siblings, so the count
        #    read happens-after every prior sibling's commit and exactly one
        #    transaction observes all-done (else a burst of N results each reads
        #    <N committed peers and none completes the step - step orphaned at
        #    WAITING_FOR_WEBHOOK).
        instance_step = await step_execution_repo.get_by_instance_and_key(
            instance_id, step_key, for_update=True
        )
        ...
        if iteration is None:
            ...
            # Invariant: terminal-COMPLETED is monotonic. A COMPLETED iteration
            # is final - a late/duplicate FAILED result (e.g. a stale dead-letter
            # replayed after the requeued retry already succeeded) must neither
            # transition the row nor trigger failure side effects. Drop it via a
            # no-op sentinel the caller turns into a None outcome. (A duplicate
            # COMPLETED is already idempotent in `complete()`; only the
            # contradicting FAILED needs guarding, since `fail()` raises on a
            # COMPLETED row and the old caller swallowed that into a forced FAIL.)
            if step_failed and iteration.status == IterationExecutionStatus.COMPLETED:
                logger.info(
                    "Dropping late FAILED result for already-COMPLETED iteration "
                    f"(terminal-COMPLETED is monotonic): step_id={instance_step.id} "
                    f"index={iteration_index} group={group_uuid}"
                )
                return {"terminal_noop": True}
