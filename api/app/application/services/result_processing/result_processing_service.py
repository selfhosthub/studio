...
class StepResultPayload:
    ...
    # Webhook completion mode: worker fired async + released. status is COMPLETED
    # but the step parks in WAITING_FOR_WEBHOOK until the provider callback(s) land.
    ...
    def from_dict(cls, payload: Dict[str, Any]) -> Optional["StepResultPayload"]:
        ...
        return cls(
            ...
            webhook_pending=payload.get("webhook_pending", False),
        )
...
class ResultProcessingService:
        ...
        *,
        ...
        if job.status in (
            ...
            if not parsed.is_iteration_job:
                ...
                return None
            # Webhook fire on a terminal (regen) iteration: step stays COMPLETED,
            # so stamp external_id here - the normal fire path below is unreachable.
            if parsed.webhook_pending and not parsed.is_failed:
                await self._record_webhook_fire(
                    parsed, instance_step, iteration_execution_repo
                )
                return ProcessingOutcome(
                    action=NextAction(action=OrchestrationAction.NOOP),
                    instance=instance,
                    job=job,
                    step_id=parsed.step_id,
        ...
        if parsed.webhook_pending and not parsed.is_failed:
            ...
            return ProcessingOutcome(
                action=NextAction(
                    action=OrchestrationAction.WAIT_WEBHOOK,
                    waiting_step_id=parsed.step_id,
                ),
                instance=instance,
                job=job,
                step_id=parsed.step_id,
        ...
        if parsed.is_failed:
            ...
            # `step_failed` here governs the NON-iteration parent path only
            # (orchestrator FAIL at step 13). For iteration jobs the parent
            # step's terminal status is decided by _complete_iteration_step on
            # the aggregate, so a single iteration's failure must not fail the
            # parent here, otherwise sibling iterations still in flight would be
            # abandoned and downstream steps would unwind prematurely. The
            # iteration's *own* failure is propagated to its IterationExecution
            # row separately via `parsed.is_failed` at the _handle_first_iteration
            # call below (Bug #1: this used to pass step_failed=False, silently
            # recording failed iterations as COMPLETED).
        ...
        if parsed.is_iteration_job:
            return await self._handle_first_iteration(
                ...
                instance_step=instance_step,
                job=job,
                # The iteration row records the iteration's OWN outcome, not the
                # parent-step suppression flag. Mirrors _handle_subsequent_iteration.
    ...
    async def _record_webhook_fire(
        self,
        parsed: StepResultPayload,
        instance_step: StepExecution,
        iteration_execution_repo: Optional[IterationExecutionRepository],
    ) -> None:
        """Write result.generation_id onto the iteration row's external_id (the
        callback match key, invariant I-15). No-op without an iteration row,
        generation id, or repo."""
        generation_id = (parsed.result_data or {}).get("generation_id")
        if not (
            iteration_execution_repo is not None
            and parsed.is_iteration_job
            and parsed.iteration_index is not None
            and generation_id
        ):
            return
        group_id = (
            UUID(parsed.iteration_group_id) if parsed.iteration_group_id else None
        )
        row = await iteration_execution_repo.get_by_step_and_index(
            step_id=instance_step.id,
            iteration_index=parsed.iteration_index,
            iteration_group_id=group_id,
            for_update=True,
        )
        if row is not None:
            row.external_id = str(generation_id)
            await iteration_execution_repo.update(row, commit=False)
    ...
    async def _handle_subsequent_iteration(
        ...
        # state. track_iteration_via_rows FOR UPDATE-locks the parent
        # StepExecution to serialize the all-done count across siblings.
        ...
        # Monotonic terminal-COMPLETED: a late FAILED result for an already-
        # COMPLETED iteration was dropped. No-op (None) - no side effects.
        if tracking.get("terminal_noop"):
            return None
    ...
    async def _handle_first_iteration(
        ...
        # Monotonic terminal-COMPLETED: the result contradicted an already-
        # COMPLETED iteration and was dropped. Return the side-effect-free no-op
        # outcome (None) - no transition, no sibling cancel, no instance fail,
        # no notification.
        if tracking.get("terminal_noop"):
            return None
    ...
    async def _complete_operation_iteration(
        ...
        Keep-variants: regen never deletes the original. On success the new
        variant is selected and the original deselected (at creation, in
        OrgFileCreator). On regen failure the step is restored to COMPLETED so
        downstream workflow state stays consistent.
        """
        step_id = parsed.step_id
        ...
        if is_failed and is_regeneration:
            ...
            logger.info(
                f"Regen failed for step {step_id}; originals preserved (kept variants)"
            )
        else:
            ...
            # Regen keeps the original (deselected at creation, not deleted) so the
            # user can compare/revert; only the new variant is selected downstream.
        ...
        # Regen skips the action executor; clear op tracking here (rerun/retry
        # clears at result_processor step 15). Multi-index regenerate keeps the
        # op until every regenerated index has landed. See lifecycle.md §6.10.
        if is_regeneration:
            step_entity = instance.step_entities[step_id]
            regenerated = active_op.get("regenerated_indices")
            if regenerated and parsed.iteration_index is not None:
                completed = list(active_op.get("completed_indices") or [])
                if parsed.iteration_index not in completed:
                    completed.append(parsed.iteration_index)
                if set(completed) >= set(regenerated):
                    step_entity.complete_operation_tracking()
                else:
                    active_op["completed_indices"] = completed
                    step_entity.active_operation = active_op
            else:
                step_entity.complete_operation_tracking()
            await instance_repo.update(instance)
            return ProcessingOutcome(
    ...
    async def _complete_iteration_step(
        ...
        # A step finalised out of a parked state resumes the run before the
        # resulting action completes or fails it - completion is illegal from a
        # waiting status. Idempotent: a no-op for an already-running instance.
