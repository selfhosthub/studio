...
class OutputExtractor:
    ...
    def _extract_with_jsonpath(
        ...
        # In webhook/async completion the result is a download payload, not the
        # synchronous response the result_schema declares - missing outputs are
        # expected there, so log them at debug rather than warning.
        is_download_result = isinstance(job.result, dict) and (
            "downloaded_files" in job.result or "assets" in job.result
        )
            ...
            try:
                ...
                if matches:
                    ...
                    if value is not None:
                        ...
                        log = logger.debug if is_download_result else logger.warning
                        log(
