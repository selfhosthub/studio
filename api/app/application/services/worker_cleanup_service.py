...
class WorkerCleanupService:
    ...
    async def cleanup_stale_workers(self) -> int:
        ...
        try:
            ...
            if not stale_workers:
                # Silenced: the common no-op case, fires every cycle. The
                # INFO line below logs when workers are actually deregistered.
                # logger.debug("No stale workers found")
                return 0

            worker_ids = [w.id for w in stale_workers]
            count = await self.worker_repository.mark_workers_as_deregistered(
                worker_ids
            )
            ...
            logger.error(f"Error cleaning up stale workers: {safe_error_message(e)}")
            raise
    ...
    async def requeue_orphaned_jobs(self) -> dict[str, int]:
        ...
        try:
            ...
            if r.requeued_step_ids:
                await self.step_execution_repository.reset_to_queued(
                    r.requeued_step_ids
                )
            ...
            logger.error(f"Error requeueing orphaned jobs: {safe_error_message(e)}")
            raise
    ...
    async def run_cleanup(self) -> dict[str, Any]:
        ...
        # almost always a no-op; only log below when it actually did work.
        ...
        # Only log when the cycle actually changed something; the all-zero
        # case is the steady state and not actionable.
        if stale_count or orphan_result["requeued"] or orphan_result["failed"] or deleted_count:
