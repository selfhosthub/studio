...
class OrchestrationAction(Enum):
    ...
    WAIT_WEBHOOK = "wait_webhook"  # park awaiting a provider callback
...
class NextAction:
    ...
    def __repr__(self) -> str:
        if self.action == OrchestrationAction.ENQUEUE_STEPS:
            ...
            OrchestrationAction.WAIT_WEBHOOK,
...
class WorkflowOrchestrator:
    ...
    def determine_next_action(
        ...
        if not auto_steps and manual_steps:
            # Arm EVERY ready manual step, not just the first - parallel manual
            # branches off one parent must all become independently triggerable.
            return NextAction(
