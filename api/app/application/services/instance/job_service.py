...
# A step-only rerun on a terminal instance reactivates the run: the user
# explicitly asked to run a step, so it resumes rather than orphaning a PENDING
# step on a dead instance (which can neither run nor be cancelled). Waiting and
# paused instances are left untouched - they were not asked to change.
_TERMINAL_REACTIVATE_STATUSES = frozenset(
    {
    ...
    async def rerun_step_only(
        ...
        if self.state_transition_service:
            ctx = await self.state_transition_service.prepare_for_reexecution(
                ...
                # Reactivate a terminal instance so the reset step can run and
                # the instance is cancellable again; leave waiting/paused as-is.
                skip_instance_transition=(
                    instance.status not in _TERMINAL_REACTIVATE_STATUSES
                ),
            ...
            for job in step_jobs:
                job.rerun()
                await self.step_execution_repository.update(job)
            if instance.status in _TERMINAL_REACTIVATE_STATUSES:
