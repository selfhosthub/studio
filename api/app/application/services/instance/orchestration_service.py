...
class OrchestrationService:
    ...
    async def _resolve_iteration_metadata_from_rows(
        ...
        if len(distinct_groups) > 1:
            raise BusinessRuleViolation(
                message=(
                    f"Step {step_execution_id} has rows spanning multiple "
                    f"iteration groups ({len(distinct_groups)}); expected one."
                ),
                code="ITERATION_MULTIPLE_GROUPS",
            )
    ...
    async def resume_from_waiting_state(
        ...
        # Allow a sibling manual trigger while the instance is already PROCESSING.
        pending_for_guard = instance.output_data.get(pending_key)
        step_still_armed = (
            isinstance(pending_for_guard, dict)
            and step_id in (pending_for_guard.get("step_ids") or [])
        )
        if instance.status != waiting_status and not (
            waiting_status == InstanceStatus.WAITING_FOR_MANUAL_TRIGGER
            and instance.status == InstanceStatus.PROCESSING
            and step_still_armed
        ):
        ...
        # Manual-trigger can arm several steps at once; consume only the one
        # triggered and keep the sentinel while siblings remain armed. Approval
        # (and the legacy single-step shape) clears the whole sentinel.
        pending = instance.output_data.get(pending_key)
        if isinstance(pending, dict) and isinstance(pending.get("step_ids"), list):
            pending["step_ids"] = [s for s in pending["step_ids"] if s != step_id]
            if not pending["step_ids"]:
                del instance.output_data[pending_key]
        elif pending_key in instance.output_data:
            del instance.output_data[pending_key]
    ...
    async def run_stopped_step(
        ...
        try:
            ...
            if self.state_transition_service:
                await self.state_transition_service.fail_on_enqueue_contract_error(
                    instance, step_id, e
                )
            raise
    ...
    async def regenerate_resources(
        ...
        # Keep-variants: regen never deletes the original. Resolve the selected
        # resources only to route by iteration_index and (for iterations) reset
        ...
        # Iteration resources reset their own row in place per-index; non-iteration falls through to whole-step rerun.
        resources_by_index: Dict[str, List[str]] = {}
        for r in resolved_resources:
            idx = (r.metadata or {}).get("iteration_index") if r.metadata else None
            if idx is not None:
                resources_by_index.setdefault(str(idx), []).append(str(r.id))

        if resources_by_index:
            return await self._regenerate_iteration_resources(
                instance=instance,
                step_id=step_id,
                job=job,
                resources_by_index=resources_by_index,
                parameter_overrides=parameter_overrides,
            )

        # --- Non-iteration step: whole-step rerun ---
        if self.state_transition_service:
        ...
        if self.job_enqueue_service and instance.workflow_snapshot:
            try:
                # Non-iteration: re-run the whole step through normal resolution.
                previous_results = {}
                if get_completed_step_results_fn:
                    previous_results = await get_completed_step_results_fn(instance)
        ...
        # Keep-variants: originals are never deleted. The whole-step re-run
        # recreates outputs; OrgFileCreator selects the new primary per slot and
        # deselects prior variants (kept for the user to compare/revert).
        logger.info(
            f"Regenerating step {step_id} (keep-variants); "
            f"{len(resolved_resources)} originals retained"
        )

        return StepExecutionResponse.from_domain(job)

    async def _regenerate_iteration_resources(
        self,
        *,
        instance: Instance,
        step_id: str,
        ...
        """Reset each affected iteration row in place and re-enqueue that index. Keep-variants: originals are retained, not deleted."""
        from app.domain.instance.models import OperationType

        regenerated_indices = sorted(int(k) for k in resources_by_index)
        single = len(regenerated_indices) == 1
        iter_count, iter_group = await self._resolve_iteration_metadata_from_rows(
            job.id
        )
        try:
            group_uuid: Optional[uuid.UUID] = (
                uuid.UUID(str(iter_group)) if iter_group else None
            )
        ...
        if self.state_transition_service:
            ctx = await self.state_transition_service.prepare_for_reexecution(
                instance=instance,
                operation=OperationType.REGENERATE_RESOURCES,
                step_ids=[],  # no wholesale row delete, no step rerun
                step_executions=[],
                operation_metadata={
                    "step_id": step_id,
                    "job_id": str(job.id),
        ...
        # Single index honors the UI override; multi-index uses each row's stored params.
        ...
        if self.iteration_execution_repository is not None:
            for idx in regenerated_indices:
                row = await self.iteration_execution_repository.get_by_step_and_index(
                    job.id, idx, group_uuid
                )
                if row is None:
                    logger.warning(
                        f"IterationExecution row not found for regeneration: "
                        f"step={step_id} index={idx} group={iter_group}"
                    )
                    continue
                row.reset_for_regeneration(
                    parameters=parameter_overrides if (single and parameter_overrides) else None
                )
        ...
        if self.job_enqueue_service and instance.workflow_snapshot:
            try:
                for idx in regenerated_indices:
                    ...
                    idx_params = (row.parameters if row else None) or {}
                    await self.job_enqueue_service.enqueue_step(
                        instance_id=instance.id,
                        step_id=step_id,
                        organization_id=instance.organization_id,
                        workflow_snapshot=instance.workflow_snapshot,
                        pre_resolved_parameters=idx_params,
                        iteration_metadata={
                            "iteration_index": idx,
                ...
                if self.state_transition_service:
                    await self.state_transition_service.commit_session()
                logger.info(
                    f"Re-enqueued step {step_id} for regeneration of iterations "
                    f"{regenerated_indices}",
                    extra={"instance_id": str(instance.id), "step_id": step_id},
                )
            except Exception as e:
                logger.error(
        ...
        logger.info(
            f"Reset iterations {regenerated_indices} in place for regeneration "
            f"(keep-variants) in step {step_id}"
        )
        return StepExecutionResponse.from_domain(job)
    ...
    async def regenerate_iteration(
        ...
        # iteration_count is row-derived (I-15) - canonical and only source.
        iteration_count, row_group_id = await self._resolve_iteration_metadata_from_rows(
            step_job.id
        )
        ...
        # Reset the iteration row to PENDING so the next result for this index is
        # tracked cleanly. The repo is guaranteed present here - count resolution
        # above (_resolve_iteration_metadata_from_rows) already requires it.
        assert self.iteration_execution_repository is not None
        instance_step = await self.step_execution_repository.get_by_instance_and_key(
            instance.id, step_id
        )
        if instance_step:
        ...
        # Keep-variants: the iteration's original resources are retained (kept +
        # deselected at creation); the regenerated variant becomes the selected one.
        logger.info(
            f"Reset iteration {iteration_index} in place for regeneration "
            f"(keep-variants) in step {step_id}"
        )
