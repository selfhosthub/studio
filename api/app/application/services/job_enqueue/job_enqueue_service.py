...
from app.application.services.job_enqueue.parameter_resolution import (
    collapse_file_source_to_local_path,
...
from app.domain.workflow.workflow_navigator import WorkflowNavigator
...
class JobEnqueueService:
    ...
    async def enqueue_step(
        ...
        # Scope the context to this step's transitive dependency closure. The
        # completed-step set is keyed in JSONB order and can include forward or
        # unrelated steps (e.g. a downstream step completed in a prior run);
        # resolving against those leaks the wrong outputs. A step sees only the
        # outputs of steps it depends on.
        closure = WorkflowNavigator.get_dependency_closure(steps, step_id)
        previous_step_results = {
            key: value
            for key, value in (previous_step_results or {}).items()
            if key in closure
        }
        ...
        if pre_resolved_parameters is not None:
            ...
            # service_id selects the inline-executor and image-preset paths below.
        ...
        # Webhook completion parks per-iteration rows (the callback token lives on
        # the row's external_id). A webhook-mode step that didn't already fan out
        # above runs as a single iteration so single + fan-out share one shape.
        webhook_svc_meta = endpoint.service_metadata or {}
        if (
            (step_config.get("client_metadata") or {}).get("completion_mode")
            == WEBHOOK_MODE
            and WEBHOOK_MODE in (webhook_svc_meta.get("completion_modes") or [])
            ...
            if iteration_metadata and iteration_metadata.get("iteration_index") is not None:
                return await self._iteration_enqueuer.enqueue_regen_iteration(
                    instance_id=instance_id,
                    step_id=step_id,
                    organization_id=organization_id,
                    workflow_snapshot=workflow_snapshot,
                    step_config=step_config,
                    resolved_step_config=resolved_step_config,
                    iteration_index=iteration_metadata["iteration_index"],
                    iteration_count=iteration_metadata["iteration_count"],
                    iteration_group_id=iteration_metadata["iteration_group_id"],
                    instance_parameters=instance_parameters,
                    previous_step_results=previous_step_results,
                )
            return await self._iteration_enqueuer.enqueue_iteration_jobs(
        ...
        # Apply image style presets when the resolved service declares support
        # (client_metadata.supports_image_presets) - never guessed from the id.
        if supports_image_presets(endpoint.service_metadata):
            preset_params = (resolved_step_config.get("job") or {}).get(
                "parameters"
            ) or {}
            if preset_params.get("styles"):
                resolved_step_config["job"]["parameters"] = apply_image_presets(
                    preset_params
                )
        ...
        # Webhook completion is handled exclusively by the iteration path: a
        # webhook-mode step is forced to a count-1 iteration above (so the
        # callback token has a row), and the iteration enqueuer attaches
        # completion_mode/webhook_completion + stamps can_check_status. This
        # single-job path therefore only runs get-mode steps - byte-for-byte
        # unchanged, with no completion keys on the payload.
        ...
        if isinstance(job_params, dict):
            resolved_params = resolve_step_parameters(
                job_params,
    ...
    async def enqueue_webhook_retrieve_job(
        self,
        *,
        instance_id: uuid.UUID,
        organization_id: uuid.UUID,
        ...
        """Enqueue a download-only sub-job carrying a pre-built result (the
        callback's asset URLs). The worker stores the assets via download_files
        and publishes a normal iteration result, so completion flows through the
        iteration-aggregation path. No outbound request fires. Returns the job id.
        """
        job_id = uuid.uuid4()

        # Route to the same queue the original step uses so the worker that
        # owns the seed-download capability claims it.
        seed_step_config = {
            "service_id": service_id,
            "provider_id": provider_id,
            "job": {"service_id": service_id, "provider_id": provider_id},
        }
        endpoint = await self._endpoint_resolver.resolve_step_endpoint(seed_step_config)
        queue_name = self.queue_router.get_queue_name(
            QueueRoutingContext(
                service_id=service_id,
                local_worker=endpoint.local_worker,
        ...
        payload: Dict[str, Any] = {
            ...
            "provider_id": provider_id,
            "credential_id": credential_id,
            # Pre-built result: the worker post-processes these URLs directly.
            "result_seed": {"assets": [{"url": url} for url in asset_urls]},
            "post_processing": {
                "download_files": {
                    "source_path": "$.assets[*].url",
                    "filename_pattern": f"{service_id.replace('.', '_')}_{{uuid}}",
                }
            },
            "instance_parameters": {},
            "previous_step_results": {},
    ...
    async def enqueue_webhook_poll_job(
        self,
        *,
        instance_id: uuid.UUID,
        organization_id: uuid.UUID,
        ...
        """Enqueue a reconcile-poll sub-job for an already-fired generation id
        whose provider callback never arrived.

        Carries the service's get-mode polling block (status endpoint + paths) +
        base_url + the known generation id, pinned to a single status check. The
        worker GETs the provider status (no fire): complete -> downloads via the
        service's post_processing and publishes a COMPLETED iteration result;
        provider-failed -> FAILED; not yet done -> re-parks. Returns the job id,
        or None if the service has no polling block to poll with.
        """
        seed_step_config = {
            "service_id": service_id,
            "provider_id": provider_id,
        ...
        polling = endpoint.polling
        if not polling:
            # No get-mode polling paths declared - nothing to poll with.
            return None

        # The status endpoint is relative to the provider base; the resolved url
        # is the create endpoint, so its parent is the base (mirrors the worker's
        # fire-then-poll base derivation).
        base_url = (endpoint.url or "").rsplit("/", 1)[0]
        generation_poll: Dict[str, Any] = {
            **polling,
            "base_url": base_url,
            "generation_id": generation_id,
            # Single-shot: the sweep re-polls on its own cadence, so the worker
            # checks once and re-parks rather than blocking on a long poll loop.
            "max_attempts": 1,
        }
