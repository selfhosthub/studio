...
class JobEnqueueService:
    ...
    async def enqueue_step(
        ...
        # Webhook completion mode (h): when the step opts in AND the service
        # declares the callback paths, carry both into the claim payload so the
        # worker's fire-and-release path activates. Absent either condition,
        # get-mode is byte-for-byte unchanged (the keys stay off the payload).
        ...
        if (
            ...
            # Persist whether the user-initiated "Check Status" poll can work for
            # this step: it can only poll if the service declares a polling block
            # (status endpoint + paths). Push-only webhook providers have none -
            # the UI hides Check Status and recovery is rerun. Stamped here (the
            # endpoint is resolved) so the instance-read path needs no catalog
            # lookup. See docs/contributing/provider-development.md.
            if self.step_execution_repository is not None:
                step_entity = (
                ...
                if step_entity is not None:
                    step_entity.execution_data["can_check_status"] = bool(
                        endpoint.polling
                    )
