...
def supports_image_presets(service_metadata: Optional[Dict[str, Any]]) -> bool:
    """True if the service declares it accepts image style preset parameters.

    Declared in the catalog as the service's
    ``client_metadata.supports_image_presets`` - never guessed from a provider
    or service-id prefix.
    """
    return bool(service_metadata and service_metadata.get("supports_image_presets"))
...
class IterationJobEnqueuer:
    ...
    async def enqueue_iteration_jobs(
        ...
        # Webhook mode: fire-and-release per iteration when the step opts in
        # and the service declares webhook_completion; else get-mode unchanged.
        svc_meta = endpoint.service_metadata or {}
        step_completion_mode = (step_config.get("client_metadata") or {}).get(
            "completion_mode"
        )
        webhook_completion_cfg = svc_meta.get("webhook_completion")
        use_webhook_completion = bool(
            step_completion_mode == WEBHOOK_MODE
            and WEBHOOK_MODE in (svc_meta.get("completion_modes") or [])
            and webhook_completion_cfg
        )
        routing = (webhook_completion_cfg or {}).get(
            "routing", GENERATION_ID_ROUTING
        )
        use_execution_token = (
            use_webhook_completion and routing == EXECUTION_TOKEN_ROUTING
        )

        # execution_token routing self-routes the callback by a per-iteration
        # token placed in the request body, so the provider POSTs to exactly
        # what we template - an absolute, publicly reachable base is required.
        ...
        if use_execution_token:
            ...
            for req in iteration_requests:
                req["callback_token"] = secrets.token_urlsafe(
                    settings.WEBHOOK_TOKEN_LENGTH
                )

        # Persist PENDING IterationExecution rows in the queued_jobs transaction.
        # For execution_token routing the row's external_id is the minted token,
        # so the inbound callback matches before any provider id exists.
        ...
        # Build the adapter once; reused across all iterations.
        iteration_adapter = build_adapter_for_endpoint(endpoint)

        # can_check_status drives the UI's Check Status button: pollable only.
        if use_webhook_completion and self.step_execution_repository is not None:
            step_entity = await self.step_execution_repository.get_by_instance_and_key(
                instance_id, step_id
            )
            if step_entity is not None:
                step_entity.execution_data["can_check_status"] = bool(endpoint.polling)
        ...
        for i in range(iteration_count):
            ...
            if use_webhook_completion:
                job_payload["completion_mode"] = WEBHOOK_MODE
                job_payload["webhook_completion"] = webhook_completion_cfg
            ...
            # Per-iteration wire envelope. In execution_token mode the minted
            # token becomes the callback URL injected into the request body.
            ...
            iter_http_request = try_build_http_request(
                ...
                callback_url=iter_callback_url,
            )
    ...
    async def enqueue_regen_iteration(
        self,
        instance_id: uuid.UUID,
        step_id: str,
        organization_id: uuid.UUID,
        ...
        try:
            ...
            logger.exception(f"Step {step_id}: queue routing error (regen)")
            safe_msg = safe_error_message(e)
            await self.status_publisher.publish_status(
                {
                    "instance_id": str(instance_id),
        ...
        svc_meta = endpoint.service_metadata or {}
        webhook_completion_cfg = svc_meta.get("webhook_completion")
        routing = (webhook_completion_cfg or {}).get("routing", GENERATION_ID_ROUTING)
        use_execution_token = routing == EXECUTION_TOKEN_ROUTING

        # execution_token routing self-routes the callback by a per-iteration
        # token. Mint a fresh one and stamp it onto the existing (reset) row so
        ...
        # [*] → [iteration_index] for the single targeted slot.
        iter_step_config = copy.deepcopy(resolved_step_config)

        def replace_star(value: Any, index: int) -> Any:
            if isinstance(value, str):
                return value.replace("[*]", f"[{index}]")
            elif isinstance(value, dict):
                return {k: replace_star(v, index) for k, v in value.items()}
        ...
        resolved_iter_config = self.variable_resolver.resolve_variables_in_config(
            replace_star(iter_step_config, iteration_index),
            previous_step_results or {},
            instance_parameters or {},
        )

        iter_job_cfg = resolved_iter_config.get("job") or {}
        iter_params = iter_job_cfg.get("parameters") or {}
        if supports_image_presets(endpoint.service_metadata) and iter_params.get(
            "styles"
        ):
            resolved_iter_config["job"]["parameters"] = apply_image_presets(iter_params)
        iter_params = resolved_iter_config.get("job", {}).get("parameters")
        if isinstance(iter_params, dict):
            resolved_iter_config.setdefault("job", {})["parameters"] = (
                resolve_step_parameters(
                    iter_params,
                    org_id=str(organization_id),
                    instance_id=str(instance_id),
        ...
        job_payload: Dict[str, Any] = {
            "job_id": job_id,
            "step_id": step_id,
            "step_config": resolved_iter_config,
            "input_data": input_data,
            "credential_id": credential_id,
            "provider_id": provider_id_for_worker,
            "service_id": service_id_for_worker,
            ...
            "iteration_count": iteration_count,
            "iteration_group_id": iteration_group_id,
            "completion_mode": WEBHOOK_MODE,
            "webhook_completion": webhook_completion_cfg,
        }

        iter_http_request = try_build_http_request(
            endpoint=endpoint,
            resolved_step_config=resolved_iter_config,
            organization_id=organization_id,
            step_id=step_id,
            adapter=iteration_adapter,
            callback_url=iter_callback_url,
        ...
        request_body = iter_job_cfg.get("parameters") or {}
        await self.status_publisher.publish_status(
            {
                "instance_id": str(instance_id),
                "step_id": step_id,
    ...
    async def _stamp_external_id_on_row(
        self,
        *,
        instance_id: uuid.UUID,
        step_key: str,
        ...
        """Write a freshly-minted callback token onto the existing reset row.

        The regen reuses the row the API already reset (external_id cleared); the
        new token is the inbound match key for this fire. No new row is created.
        """
        if (
        ...
        step = await self.step_execution_repository.get_by_instance_and_key(
            instance_id=instance_id, step_key=step_key
        )
        if step is None:
            return
        try:
            group_uuid: Optional[uuid.UUID] = uuid.UUID(iteration_group_id)
        except (ValueError, AttributeError, TypeError):
            group_uuid = None
        row = await self.iteration_execution_repository.get_by_step_and_index(
            step.id, iteration_index, group_uuid
        )
        if row is None:
            return
        row.external_id = external_id
        await self.iteration_execution_repository.update(row, commit=False)
    ...
    async def _create_iteration_execution_rows(
        ...
        iterations = [
            IterationExecution.create(
                ...
                # execution_token routing: the minted callback token is the
                # inbound match key. None for get / generation_id routing.
                external_id=req.get("callback_token"),
            )
            for req in iteration_requests
        ]
