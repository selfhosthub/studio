...
class ServiceNotFoundError(BusinessRuleViolation):
    """Raised when a step references a service_id that doesn't resolve to a
    catalog service.

    No fallback: the step fails at enqueue with an actionable message so the
    operator can install the missing package (or fix the misconfigured
    service_id) and rerun the step. A package can be removed out from under an
    existing workflow, so this must surface, not silently route to the provider
    base URL.
    """
...
class StepEndpointResolver:
    ...
    async def resolve_step_endpoint(
        ...
        Raises:
            ...
            ServiceNotFoundError: If the step references a service/provider that
                doesn't resolve and has no raw URL to fall back to.
        """
        job = step_config.get("job") or {}
        ...
        if not provider_id_str or not service_id_str:
            # A step with no service to call cannot be routed. Fail it now with
            # an actionable message rather than returning an empty endpoint that
            # dies downstream in queue routing as service ''.
            raise ServiceNotFoundError(
                f"Step references no service "
                f"(service_id={service_id_str!r}, provider_id={provider_id_str!r}). "
                f"Set a valid service_id, then rerun the step."
            )
        ...
        try:
            ...
            # If provider not found, a raw-URL step is still valid; otherwise the
            # referenced provider/package is gone - fail the step now.
            if not provider:
                if direct_url:
                ...
                raise ServiceNotFoundError(
                    f"Service '{service_id_str}' not found "
                    f"(provider '{provider_id_str}' is not installed). "
                    f"Install the package that provides it, then rerun the step."
                )
            ...
            # No operational hard-kill at runtime: an org that already configured
            # credentials keeps running against an operationally-inactive provider
            # (Phase 8 / catalog-visibility plan decision #9). `operational_status
            # == inactive` gates new installs + new credential setup only; it does
            # not stop in-flight or existing workflows here.
            ...
            if not service:
                # No catalog service for this service_id. A raw-URL step (direct
                # url in params, or a provider base URL to POST against) is still
                # valid; route to it. But if there's nothing to call, fail the
                # step now with an actionable message instead of returning an
                # empty endpoint that dies later in queue routing as service ''.
        ...
        except (ServiceInactiveError, ServiceNotFoundError):
