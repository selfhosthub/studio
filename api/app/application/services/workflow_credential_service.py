...
class WorkflowCredentialService:
    ...
    async def check_workflow_credentials(
        ...
        for step_id, step_config in steps.items():
            ...
            # Check for missing/disabled AI prompts
            if prompt_repo:
    ...
    async def _check_step(
        ...
        # Check if provider is uninstalled (operationally inactive). This is the
        # install/credential-setup gate (pre-run advisory), not a runtime block.
        if provider and provider.operational_status == OperationalStatus.INACTIVE:
    ...
    async def _check_step_prompts(
        ...
        """Return CredentialIssues for missing or disabled AI prompts in a step."""
            ...
            if not prompt_id_str:
                issues.append(
                    CredentialIssue(
                        ...
                        step_name=step_name,
                        provider_id="",
                        provider_name="AI Prompt",
                        status="prompt_missing",
                        message="No AI prompt selected",
                        action_url=f"/workflows/{workflow_id}/edit",
                    )
                )
                continue
            ...
            try:
                ...
                issues.append(
                    CredentialIssue(
                        ...
                        provider_name="AI Prompt",
                        status="prompt_missing",
                        message="AI prompt has an invalid ID",
                        action_url=f"/workflows/{workflow_id}/edit",
                    )
                )
                continue
            ...
            if not prompt:
                issues.append(
                    CredentialIssue(
                        ...
                        provider_name="AI Prompt",
                        status="prompt_missing",
                        message="AI prompt not found (may have been deleted)",
                        action_url=f"/workflows/{workflow_id}/edit",
                    )
                )
                ...
                issues.append(
                    CredentialIssue(
                        ...
                        status="prompt_missing",
                        message=f'AI prompt "{prompt.name}" has been uninstalled',
                ...
                issues.append(
                    CredentialIssue(
                        ...
                        provider_name=f"Prompt: {prompt.name}",
                        status="prompt_missing",
                        message=f'AI prompt "{prompt.name}" is disabled',
                        action_url=f"/workflows/{workflow_id}/edit",
                    )
                )
