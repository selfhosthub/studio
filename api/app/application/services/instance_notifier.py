...
class InstanceNotifier:
    ...
    WAITING_ACTIONS = frozenset(
        {
            "waiting_approval",
            "waiting_manual_trigger",
            "waiting_webhook",
            "waiting_webhook_reminder",
        }
    )
    ...
    _ACTION_TO_STATUS = {
        ...
        "waiting_webhook": InstanceStatus.WAITING_FOR_WEBHOOK,
        "waiting_webhook_reminder": InstanceStatus.WAITING_FOR_WEBHOOK,
    ...
    def __init__(
        ...
        terminal_hook: Optional[
            Callable[[Instance, str, AsyncSession], Awaitable[None]]
        ] = None,
    ):
        ...
        # Fired once per terminal (completed/failed) announce, after the bell
        # commit — used to chain EVENT-trigger workflows. Best-effort.
    ...
    async def create_notification(
        ...
        title_override/message_override let a caller distinguish two notifications
        that share a status (e.g. the initial WFW announce vs. the once-per-step
        "still awaiting" reminder, both WAITING_FOR_WEBHOOK)."""
        try:
            ...
            elif status == InstanceStatus.WAITING_FOR_WEBHOOK:
                title = "Awaiting Provider Callback"
                message = f"{display_name} fired its request and is awaiting the provider callback."
                priority = NotificationPriority.LOW
                tags = ["workflow", "webhook"]
    ...
    async def announce_schedule_fired(
        self,
        session: AsyncSession,
        organization_id: UUID,
        instance_id: UUID,
        ...
        """Bell notification that a SCHEDULE trigger fired and created a run.

        Distinct from the run's own lifecycle announces (started/completed):
        this confirms the *schedule* ticked. Lives here so the create_notification
        pairing contract stays satisfied. Best-effort — never raises; a failure
        must not stall the schedule advance."""
        display_name = workflow_name or "Workflow"
        try:
            await self.create_notification(
                ...
                title_override="Scheduled Run Triggered",
                message_override=f"{display_name} was triggered by its schedule.",
            )
            await session.commit()
        except Exception as e:
            logger.warning(
                f"Schedule-fired notification failed for instance "
                f"{instance_id}: {e}"
            )
    ...
    async def announce_state_change(
        ...
        title_override/message_override customize the bell text for actions that
        share a status (the WFW reminder reuses WAITING_FOR_WEBHOOK).
        """
        ...
        # EVENT-trigger fan-out: only on terminal completed/failed, only when a
        # session is available. Best-effort — never let it break the announce.
        if (
            self._terminal_hook is not None
            and session is not None
            and action_type in ("completed", "failed")
        ):
            try:
                await self._terminal_hook(instance, action_type, session)
            except Exception as e:
                logger.warning(f"Terminal event-trigger hook failed: {e}")
