...
class UnresolvedMappingError(Exception):
    """A declared 'mapped' input reference resolved to nothing."""
...
class MappingResolver:
    ...
    def resolve_mappings(
        ...
        for param_name, mapping_expr in input_mappings.items():
            try:
                ...
                elif (ref := self._required_step_ref(mapping_expr)) is not None:
                    raise UnresolvedMappingError(
                        f"Unresolved mapped reference for parameter '{param_name}': "
                        f"'{ref}' produced no value"
                    )
                else:
                    logger.warning(
    ...
    @staticmethod
    def _required_step_ref(mapping_expr: Any) -> Optional[str]:
        """Return 'stepId.outputField' for a dict 'mapped' reference to a
        completed step (not a form field), else None.

        These reference an upstream step output the workflow declares as
        required; an unresolved one is a wiring bug, not an optional value.
        """
        if not isinstance(mapping_expr, dict):
            return None
        step_id = mapping_expr.get("stepId")
        output_field = mapping_expr.get("outputField")
        if (
            mapping_expr.get("mappingType") == "mapped"
            and step_id
            and step_id != "__instance_form__"
            and output_field
        ):
            return f"{step_id}.{output_field}"
        return None
