...
class OrgService:
    ...
    async def update_organization(
        ...
        # Handle is_staging changes (super_admin only). Folded into the
        # generic update path so super-admin UIs can flip it alongside other
        # org settings; no dedicated endpoint required.
        if (
            command.is_staging is not None
            and command.is_staging != organization.is_staging
        ):
            if user.role != Role.SUPER_ADMIN:
                raise PermissionDeniedError(
                    message="Only super admins can change staging-tenant status",
                    code="INSUFFICIENT_PERMISSIONS",
                )
            organization.set_staging(command.is_staging)
