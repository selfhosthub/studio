...
class PackageManagementService:
    ...
    async def get_provider_usage(
        ...
        # Steps live on workflow_versions (the immutable per-save snapshot), not
        # on workflows. Scan each workflow's current (latest) version's steps.
        result = await self._session.execute(
            text(
                """
                ...
                WHERE cur.steps::text LIKE :pattern
            """
            ),
    ...
    async def get_provider_by_slug(self, slug: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "name": provider.name,
            "slug": provider.slug,
            # `status` key carries the operational axis (kept for call-site compat).
    ...
    async def reactivate_provider(
        ...
        await self._session.execute(
            update(ProviderModel)
            .where(ProviderModel.id == provider_id)
            .values(operational_status=OperationalStatus.ACTIVE)
        )
