...
class StepExecutionResponse(BaseModel):
    ...
    # WFW steps only: the user-initiated "Check Status" poll can work (the
    # service has a polling block). Stamped at enqueue time into execution_data;
    # push-only webhook providers leave it False and the UI hides the button.
    ...
    def from_domain(
        ...
        return cls(
            ...
            can_check_status=(
                status == StepExecutionStatus.WAITING_FOR_WEBHOOK
                and bool((step.execution_data or {}).get("can_check_status"))
            ),
