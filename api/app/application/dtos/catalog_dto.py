...
class CatalogTier(str, Enum):
    ...
    ``community`` <-> the Community bucket, ``plus`` <-> the Plus bucket. The
    tier->subscription lookup is future work; this only names the values the
    bucket predicates read.
    """
...
class CatalogEntry(BaseModel):
    ...
    def is_community(self) -> bool:
        return (
            ...
            and self.tier is CatalogTier.COMMUNITY
        )
    ...
    def is_plus(self) -> bool:
        return (
            self.origin is CatalogOrigin.CATALOG_REFERENCE
            and self.tier is CatalogTier.PLUS
        )
