...
# Resolve imports from absolute script location, independent of cwd or whether
# this is run as a script or `python -m`. `_HERE` (the scripts dir) lets the
# sibling modules import as top-level; `_API_ROOT` lets `app.*` resolve.
...
from bootstrap_database import validate_password
...
def prompt_for_new_password() -> str:
    ...
    while True:
        ...
        if not is_valid:
            print(f"   ❌ {message}")  # codeql[py/clear-text-logging-sensitive-data]
            continue
...
async def main():
    ...
    try:
        ...
        if admin_password:
            ...
            if not is_valid:
                logger.error(f"❌ Invalid SHS_ADMIN_PASSWORD: {error}")  # codeql[py/clear-text-logging-sensitive-data]
                sys.exit(1)
