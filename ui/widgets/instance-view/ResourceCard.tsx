...
interface ResourceCardProps {
  ...
  variantActive?: boolean; // Keep-variants: this is the selected variant flowing downstream
  onUseVariant?: (resourceId: string) => void; // Keep-variants: make this the selected variant
}
...
const ResourceCard = React.memo(function ResourceCard({ resource, onDownload, onDelete, onRegenerate, onView, showMetadata = false, size = 'medium', showThumbnails = true, selectable = false, selected = false, onSelect, variantActive = false, onUseVariant }: ResourceCardProps) {
  ...
  // Show thumbnail only for image resources with a loaded blob URL
  const showThumbnail = isImageType && thumbnailBlobUrl && !thumbnailError;
  ...
  return (
    <div className={`border rounded-lg bg-card hover:shadow-md transition-shadow ${containerPadding} ${
      variantActive
        ? 'border-success ring-2 ring-[var(--theme-primary)]/40'
        : selected
      ...
      >
        ...
        {resource.status === 'available' && (
          <div className="absolute top-1.5 left-1.5 flex gap-1">
            ...
            {onUseVariant && !variantActive && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onUseVariant(resource.id);
                }}
                className={`${buttonSize} bg-black/60 hover:bg-[var(--theme-success)] rounded transition-colors`}
                title="Use this image"
              >
                <Check className={`${buttonIconSize} text-white`} />
              </button>
            )}
            {onDelete && (
              <button
        ...
        {/* Selected-variant ribbon - bottom right corner */}
        {variantActive && (
          <div className="absolute bottom-1.5 right-1.5">
            <span className="text-xs px-2 py-1 rounded bg-success-subtle text-success flex items-center gap-1">
