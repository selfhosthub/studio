...
export default function MediaViewerModal({
  ...
  // Reset view state synchronously when the resource changes (during render, not
  // in an effect) so the spinner shows immediately and stale media is cleared.
  const [loadedResourceId, setLoadedResourceId] = useState(resource?.id);
  if (resource?.id !== loadedResourceId) {
    setLoadedResourceId(resource?.id);
    setLoading(true);
    setError(null);
    setMediaUrl(null);
  }
