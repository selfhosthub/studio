...
export default function InstanceSimpleView({
  ...
  // a developer-debug concern not useful to end users. Exception: when a step's
  // iterations produce >1 file each (e.g. Leonardo, shs-video), grouped view is
  // the default because flat-view drag must stay inside iteration boundaries to
  // keep the prompt/audio/images bundle intact.
  // Per-step view state. Each step's flat/grouped choice is independent so
  // navigating from a multi-file iteration step (defaults to grouped) to a
  // single-file step (defaults to flat) shows the correct default for each.
  // User toggles persist for the step they were made on but don't bleed
  // across steps.
  const [perStepFlatten, setPerStepFlatten] = useState<Record<string, boolean>>({});
  ...
  // Per-step view derivation. The base default is `simpleMode` (flat for
  // simple mode, grouped for technical mode). A step that produces >1 file
  // per iteration auto-flips to grouped so the prompt/audio/images bundle
  // stays intact during reorder. User toggles are recorded in
  // `perStepFlatten` and take precedence over the auto-default.
  const hasMultiFileIteration = selectedStep
    ? Object.values(
        stepResources.reduce<Record<number, number>>((acc, r) => {
          const idx = r.metadata?.iteration_index;
          if (typeof idx === "number") acc[idx] = (acc[idx] ?? 0) + 1;
          return acc;
        }, {})
      ).some((n) => n > 1)
    : false;
  ...
  const flattenIterations =
    selectedStepId && selectedStepId in perStepFlatten
      ? perStepFlatten[selectedStepId]
      : hasMultiFileIteration
        ? false
        : simpleMode;

  const setFlattenIterations = (value: boolean) => {
    if (selectedStepId) {
      setPerStepFlatten((prev) => ({ ...prev, [selectedStepId]: value }));
    }
  };
        ...
        <div className="flex-1 overflow-y-auto bg-surface">
          ...
          {selectedItem.type === "step" && selectedStep && (
            <StepPanel
              ...
              stageStepReorder={stageStepReorder}
              flushStepReorder={flushStepReorder}
              discardStepReorder={discardStepReorder}
