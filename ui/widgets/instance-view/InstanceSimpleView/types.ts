...
export interface WorkflowStep {
  ...
  provider_id?: string;
  service_type?: string;
  status: 'pending' | 'queued' | 'running' | 'completed' | 'failed' | 'cancelled' | 'waiting_for_approval' | 'waiting_for_manual_trigger' | 'waiting_for_webhook' | 'stopped';
  can_check_status?: boolean;  // WFW steps only: provider has a pollable status endpoint
...
export interface InstanceSimpleViewProps {
  ...
  onCheckWebhookStatus: (instanceId: string, stepId: string) => void;
  ...
  onSelectVariant: (jobId: string, resourceId: string) => void;
