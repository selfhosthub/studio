...
export function normalizeConfig(config: any): FormFieldConfig {
  return {
    label: config.label,
...
/**
 * Calculate human-readable duration between two timestamps, full units (e.g. 1h 5m 12s).
 */
...
export function groupResourcesByIteration(
  ...
  // Sort each iteration's resources by display_order so persisted reorders
  // are reflected in the rendered grid (input array is concatenated across
  // jobs and has no inherent display_order sort).
