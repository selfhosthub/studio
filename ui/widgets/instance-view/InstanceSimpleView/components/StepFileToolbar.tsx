    ...
    <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
      ...
      <div className="flex items-center gap-2">
        ...
        {stepResources.some((r: OrgFile) => r.metadata?.iteration_index !== undefined) && (() => {
          const perIter = stepResources.reduce<Record<number, number>>((acc, r) => {
            const idx = r.metadata?.iteration_index;
            if (typeof idx === "number") acc[idx] = (acc[idx] ?? 0) + 1;
            return acc;
          }, {});
          const hasMultiFileIteration = Object.values(perIter).some((n) => n > 1);
          const groupTitle = hasMultiFileIteration
            ? "Group by iteration (recommended — keeps prompt, audio, and images together)"
            : "Group by iteration";
          const flatTitle = hasMultiFileIteration
            ? "Flat view (read-only — switch to grouped view to reorder iterations)"
          ...
          return (
            <button
              ...
              title={flattenIterations ? groupTitle : flatTitle}
            >
