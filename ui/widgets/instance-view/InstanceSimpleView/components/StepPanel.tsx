...
export function StepPanel({
  ...
  // Apply optimistic local order (from in-flight reorders) before grouping so
  // the user sees their drag persist immediately instead of waiting on refetch.
  const stepOrderKey = selectedStep.instance_step_id
    ? `step:${selectedStep.instance_step_id}`
    : null;
  const localOrder = stepOrderKey ? localResourceOrder[stepOrderKey] : undefined;
  const orderedStepResources = localOrder
    ? [...stepResources].sort((a, b) => {
        const aIdx = localOrder.indexOf(a.id);
        const bIdx = localOrder.indexOf(b.id);
        return (aIdx === -1 ? Infinity : aIdx) - (bIdx === -1 ? Infinity : bIdx);
      }).map((r, idx) => localOrder.includes(r.id) ? { ...r, display_order: idx } : r)
    : stepResources;
  const { hasIterations, groups } = groupResourcesByIteration(orderedStepResources, iterationCount, filesPerIteration);
  ...
  // Before any Run/Rerun fires, flush any locally-staged reorder so the
  // step's display_order is up-to-date before the execution kicks off.
  // If the flush fails we abort the run so the user can retry/inspect
  // instead of silently running with the stale order.
  const flushPendingReorderBeforeRun = async (): Promise<boolean> => {
    const stepId = selectedStep.instance_step_id;
    if (!stepId || !hasPendingReorder(stepId)) return true;
    try {
      await flushStepReorder(stepId);
      return true;
    } catch (e) {
      console.error("Failed to persist reorder before run; aborting run.", e);
      return false;
    }
  };

  const wrappedOnRunStoppedStep = async (stepId: string) => {
    if (await flushPendingReorderBeforeRun()) onRunStoppedStep(stepId);
  };
  const wrappedOnRerunJobOnly = async (jobId: string) => {
    if (await flushPendingReorderBeforeRun()) onRerunJobOnly(jobId);
  };
  const wrappedOnRerunAndContinue = async (jobId: string) => {
    if (await flushPendingReorderBeforeRun()) onRerunAndContinue(jobId);
  };
  const wrappedOnRerunStepOnly = async (instanceId: string, stepId: string) => {
    if (await flushPendingReorderBeforeRun()) onRerunStepOnly(instanceId, stepId);
  };

  const pendingReorderForThisStep =
    !!selectedStep.instance_step_id && hasPendingReorder(selectedStep.instance_step_id);

  // Regenerate/delete invalidate the staged reorder (IDs change or disappear),
  // so drop the pending order at click time. Doesn't affect run actions.
  const discardIfPending = () => {
    if (selectedStep.instance_step_id) discardStepReorder(selectedStep.instance_step_id);
  };
  const wrappedOnRegenerateResources = (stepId: string, jobId: string, resourceIds: string[]) => {
    discardIfPending();
    onRegenerateResources(stepId, jobId, resourceIds);
  };
  const wrappedOnRegenerateIteration = (stepId: string, jobId: string, iterationIndex: number) => {
    discardIfPending();
    onRegenerateIteration(stepId, jobId, iterationIndex);
  };
  const wrappedOnDeleteResources = (stepId: string, jobId: string, resourceIds: string[]) => {
    discardIfPending();
    onDeleteResources(stepId, jobId, resourceIds);
  };
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <StepHeaderActions
        ...
        onApprove={onApprove}
        onTrigger={onTrigger}
        onRunStoppedStep={wrappedOnRunStoppedStep}
        onRerunJobOnly={wrappedOnRerunJobOnly}
        onRerunAndContinue={wrappedOnRerunAndContinue}
        onRerunStepOnly={wrappedOnRerunStepOnly}
        onCheckWebhookStatus={onCheckWebhookStatus}
      />
      ...
      {showFiles && (
        <div className="bg-card rounded-lg border border-primary p-4">
          {pendingReorderForThisStep && (
            <div className="mb-3 flex items-center justify-between gap-2 px-3 py-2 bg-warning-subtle border border-warning rounded text-xs text-warning">
              <span>Reorder not saved</span>
              <div className="flex items-center gap-3">
                <button
                  onClick={async () => {
                    if (!selectedStep.instance_step_id) return;
                    try {
                      await flushStepReorder(selectedStep.instance_step_id);
                    } catch (e) {
                ...
                <button
                  onClick={() => selectedStep.instance_step_id && discardStepReorder(selectedStep.instance_step_id)}
          ...
          <StepFileToolbar
            ...
            onRegenerateResources={wrappedOnRegenerateResources}
            onDeleteResources={wrappedOnDeleteResources}
          ...
          {showFullPlaceholders && !hasIterations ? (
            ...
            <div className="space-y-2">
              {groups.map(({ iterationIndex, resources, expectedCount: groupExpected, isComplete }: { iterationIndex: number | null; resources: OrgFile[]; expectedCount: number; isComplete: boolean }) => {
                ...
                const handleReorderWithinIteration = (newIterationOrder: string[]) => {
                  const stepId = selectedStep.instance_step_id;
                  if (!stepId) return;
                  const fullOrder: string[] = [];
                  for (const g of groups) {
                    if (g.iterationIndex === iterationIndex) {
                      fullOrder.push(...newIterationOrder);
                    } else {
                      fullOrder.push(...g.resources.map((r) => r.id));
                    }
                  }
                  // Stage locally only — persists on Run/Rerun click.
                  stageStepReorder(stepId, fullOrder);
                };

                return (
                  <UnifiedIterationBlock
                    key={iterationIndex ?? "ungrouped"} // defaults-ok -- React key fallback, not a user-facing default
                    ...
                    onRegenerateIteration={(idx: number) =>
                      wrappedOnRegenerateIteration(selectedStep.step_id, selectedStepExecution?.id || "", idx)
                    }
                    onRegenerateSelected={(rids: string[]) =>
                      wrappedOnRegenerateResources(selectedStep.step_id, selectedStepExecution?.id || "", rids)
                    }
                    onDeleteSelected={(rids: string[]) =>
                      wrappedOnDeleteResources(selectedStep.step_id, selectedStepExecution?.id || "", rids)
                    }
                    onSelectVariant={(rid: string) =>
                      onSelectVariant(selectedStepExecution?.id || "", rid)
                    }
                    gridClass={gridClass}
                    orgSettings={orgSettings}
                    ...
                    sensors={sensors}
                    onReorderWithinIteration={handleReorderWithinIteration}
                  />
                );
              })}
            </div>
