...
export function ResourceGrid({
  ...
  const handleRegenerate = useCallback((resourceId: string) => {
    if (stepId && jobId && onRegenerateResources) {
      onRegenerateResources(stepId, jobId, [resourceId]);
    }
  }, [stepId, jobId, onRegenerateResources]);

  const canAct = selectedStep?.status === "completed";
  ...
  // Flat view never reorders iteration steps — iterations are the unit of
  // order, reordered only in grouped view. Drag is enabled only for
  // non-iteration steps (a plain N-file batch).
  const canReorder = selectedStep?.status === "completed"
    && !hasIterations
    ...
    return (
      <DndContext
        ...
        onDragEnd={(e: DragEndEvent) => {
          // Only non-iteration steps are draggable here (canReorder gates the
          // handle); reorder is job-scoped.
          if (!hasIterations && selectedStepExecution?.id) {
          ...
          <div className={gridClass}>
            {orderedResources.map((resource: OrgFile) => (
              <SortableResourceCard
                ...
                onRegenerate={canAct && onRegenerateResources ? handleRegenerate : undefined}
                onView={handleView}
