...
export function useFormValidation(formSchema: WorkflowFormSchema): UseFormValidationReturn {
  ...
  const validate = (formValues: Record<string, any>): boolean => {
    ...
    formSchema.fields.forEach((field) => {
      ...
      // Number validation
      if (config.fieldType === 'number' && value !== '' && value != null) { // form-field-switch-ok - validator, not a render path
      ...
      if ((config.fieldType === 'text' || config.fieldType === 'textarea') && value) { // form-field-switch-ok - validator
      ...
      // JSON validation
      if (config.fieldType === 'json' && value) { // form-field-switch-ok - validator
        try {
          JSON.parse(value);
        } catch {
