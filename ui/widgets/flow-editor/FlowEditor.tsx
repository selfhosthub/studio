...
const FlowEditor = ({
      ...
      const updatedNodes = remainingNodes.map(node => {
        ...
        return {
          ...
          data: {
            ...
            isManualTrigger: step.trigger_type === 'manual',
          },
        };
      });
  ...
  // Sync connections → edges. Done during render (not in an effect) to avoid a
  // cascading commit. Re-runs whenever `connections` or the converter changes
  // (the converter's identity changes on edge-style/selection/mapping changes),
  // matching the previous effect's [connections, connectionsToEdges] deps.
  const [edgeSync, setEdgeSync] = useState<{
    connections: typeof connections;
    toEdges: typeof connectionsToEdges;
  }>(() => ({ connections, toEdges: connectionsToEdges }));
  if (edgeSync.connections !== connections || edgeSync.toEdges !== connectionsToEdges) {
    setEdgeSync({ connections, toEdges: connectionsToEdges });
    setEdges(connectionsToEdges(connections));
  }
  ...
  // Source handle id captured on connect-start, consumed by onConnectEnd for drag-to-create.
  ...
  const onConnectEnd = useCallback(
    (event: MouseEvent | TouchEvent, connectionState: FinalConnectionState) => {
      ...
      if (!sourceId || !onCreateConnectedStep) return;

      // Only create a step when the drag ended on empty canvas, not on a target node.
      // onConnect already wired the edge for a valid drop; firing here too would double-create.
      if (connectionState.isValid || connectionState.toNode) return;

      const point = 'changedTouches' in event ? event.changedTouches[0] : event;
      const position = screenToFlowPosition({ x: point.clientX, y: point.clientY });
      onCreateConnectedStep(sourceId, position);
    },
    [onCreateConnectedStep, screenToFlowPosition],
  );
