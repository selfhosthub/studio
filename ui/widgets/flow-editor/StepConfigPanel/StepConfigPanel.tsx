...
import { BREAKPOINTS, STORAGE_KEYS } from '@/shared/lib/constants';
...
export function StepConfigPanel({
  ...
  const [panelWidth, setPanelWidth] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(STORAGE_KEYS.STEP_CONFIG_PANEL_WIDTH);
  ...
  const [outputsPanelWidth, setOutputsPanelWidth] = useState(() => {
    ...
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(STORAGE_KEYS.STEP_CONFIG_OUTPUTS_PANEL_WIDTH);
  ...
  const [showOutputsPanel, setShowOutputsPanel] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(STORAGE_KEYS.STEP_CONFIG_SHOW_OUTPUTS_PANEL);
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem(STORAGE_KEYS.STEP_CONFIG_PANEL_WIDTH, String(panelWidth));
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      // Save to localStorage
      if (typeof window !== 'undefined') {
        localStorage.setItem(STORAGE_KEYS.STEP_CONFIG_OUTPUTS_PANEL_WIDTH, String(outputsPanelWidth));
      }
    };
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(STORAGE_KEYS.STEP_CONFIG_SHOW_OUTPUTS_PANEL, String(showOutputsPanel));
    }
  }, [showOutputsPanel]);
        ...
        <div className="flex-shrink-0 bg-card border-b border-primary px-4 py-2 flex items-center justify-between">
          ...
          {onRun && (
            <button
              onClick={onRun}
              disabled={isRunning}
              className="p-1.5 rounded-md text-white bg-[var(--theme-btn-primary-bg)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
