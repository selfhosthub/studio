...
export function AutoSaveDropdown({
  ...
  const [localInterval, setLocalInterval] = useState<number | null>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(STORAGE_KEYS.WORKFLOW_AUTO_SAVE_INTERVAL);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      if (localInterval) {
        localStorage.setItem(STORAGE_KEYS.WORKFLOW_AUTO_SAVE_INTERVAL, String(localInterval));
      } else {
