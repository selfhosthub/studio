...
export function useAutoLayout({ connections, preferences }: UseAutoLayoutProps) {
  const calculateLayout = useCallback((steps: Step[]): Step[] => {
    ...
    // Process levels ascending so a node's dependencies are placed before it,
    // letting a lone node inherit its branch's vertical lane.
    const positionedSteps: Array<{ step: Step; position: { x: number; y: number } }> = [];
    const placedCenterY: Record<string, number> = {};

    // Average center-Y of a step's already-placed dependencies, or null if none placed.
    const inheritedCenterY = (stepId: string): number | null => {
      const deps = (dependencyMap[stepId] || []).filter(depId => depId in placedCenterY);
      if (deps.length === 0) return null;
      return deps.reduce((sum, depId) => sum + placedCenterY[depId], 0) / deps.length;
    };

    const ascendingLevels = Object.keys(stepsByLevel)
      .map(Number)
      .sort((a, b) => a - b);

    ascendingLevels.forEach(level => {
      const stepsAtThisLevel = stepsByLevel[level];
      ...
      stepsAtThisLevel.forEach((step, indexInLevel) => {
        const height = estimateNodeHeight(step, preferences.nodeWidth);
        let y: number;

        if (countAtLevel === 1) {
          // Single step - keep it on its branch lane by inheriting its parents'
          // center, falling back to the canvas center for roots.
          const parentCenter = inheritedCenterY(step.id);
          y = (parentCenter ?? targetCenterY) - height / 2;
          // If this level is in the path of a data-jump, offset it UP to clear the connection lines
          if (needsOffset) {
            y = targetCenterY - dataJumpOffset - height;
          }
        } else {
        ...
        placedCenterY[step.id] = y + height / 2;
