...
export type Visibility = 'private' | 'staging' | 'public';
...
// Reserved first-party namespaces; mirrors backend RESERVED_NAMESPACES.
export const RESERVED_NAMESPACES: ReadonlySet<string> = new Set(['shs']);

export function isReservedSlug(slug: string | undefined | null): boolean {
  if (!slug || !slug.includes('/')) return false;
  return RESERVED_NAMESPACES.has(slug.split('/', 1)[0]);
}

export function isCustomOriginSlug(slug: string | undefined | null): boolean {
...
export interface ProviderService {
  ...
  created_at?: string;
  updated_at?: string;
  /** Whether the service requires a credential (defaults to true when absent) */
  requires_credentials?: boolean;
  /** Cross-provider dispatch targets; non-empty enables cross-provider credentials */
  dispatch?: string[];
  /** Renders a custom editor instead of the generic parameter form */
  custom_ui?: boolean;
  /** Manages its own output (suppresses the output schema section) */
  custom_output?: boolean;
  /** Custom parameter-section title (falls back to 'Service Parameters') */
  parameter_title?: string | null;
