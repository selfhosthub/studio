...
export function OrgSettingsProvider({ children }: { children: ReactNode }) {
  ...
  // Capture the primitive so the callback's inferred dep (orgId) matches its
  ...
  useEffect(() => {
    // IIFE so the deferred setState inside fetchSettings isn't read as a synchronous set.
