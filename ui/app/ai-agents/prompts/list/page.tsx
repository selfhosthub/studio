...
function PromptsListPageContent() {
  ...
  useEffect(() => {
    ...
    // IIFE so the deferred setState inside fetch functions isn't read as a synchronous set.
    void (async () => { await fetchOrgPrompts(); })();
    if (!isSuperAdmin) void (async () => { await fetchMyPrompts(); })();
  ...
  const orgFilterKey = `${orgSearchTerm}|${categoryFilter}`;
