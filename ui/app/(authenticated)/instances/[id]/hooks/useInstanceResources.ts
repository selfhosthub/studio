// ui/app/(authenticated)/instances/[id]/hooks/useInstanceResources.ts
...
export function useInstanceResources({
  ...
  const handleDeleteResourcesWithIds = useCallback(async (stepId: string, jobId: string, resourceIds: string[]) => {
    ...
    try {
      ...
      // Refetch instance + jobs: regen moves surviving files to a new job_execution_id.
      setJobResources(prev => {
        const next = { ...prev };
        delete next[jobId];
        return next;
  ...
  }, [instanceId, setInstance, setJobs, setJobResources, toast]);
  ...
  // --- Select variant (keep-variants) ---

  const handleSelectVariant = useCallback(async (jobId: string, resourceId: string) => {
    const slot = (jobResources[jobId] || []).find(r => r.id === resourceId)?.metadata?.iteration_index;
    const sceneLabel = typeof slot === 'number' ? `scene ${slot + 1}` : undefined;

    // Optimistic: swap the selected ribbon within this slot immediately so the
    // user sees their choice land before the round-trip + refetch resolves.
    setJobResources(prev => {
      ...
      return {
        ...prev,
        [jobId]: list.map(r =>
          r.metadata?.iteration_index === slot ? { ...r, is_selected: r.id === resourceId } : r
        ),
      };
    });
    ...
    try {
      ...
      const updated = asOutputResources(await getJobResources(jobId));
      setJobResources(prev => ({ ...prev, [jobId]: updated }));
      toast({ title: sceneLabel ? `Selected image for ${sceneLabel}` : 'Image selected', variant: 'success' });
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : 'Unknown error';
      toast({ title: `Failed to select image: ${message}`, variant: 'destructive' });
      // Reconcile the optimistic swap back to server truth.
      try {
        const reverted = asOutputResources(await getJobResources(jobId));
        setJobResources(prev => ({ ...prev, [jobId]: reverted }));
      } catch { /* leave optimistic state; next WS refetch will correct it */ }
    }
  }, [jobResources, setJobResources, toast]);
