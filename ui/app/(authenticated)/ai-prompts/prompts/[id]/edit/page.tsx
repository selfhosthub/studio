// ui/app/(authenticated)/ai-prompts/prompts/[id]/edit/page.tsx
...
function EditPromptPageContent() {
  ...
  const { returnTo, goBack } = useReturnTo("/ai-prompts/prompts/list");
  const { user } = useUser();
  ...
  useEffect(() => {
    ...
    getPrompt(promptId)
      ...
      .catch((err) => {
        setError(err?.message || "Failed to load prompt"); // defaults-ok
      })
  ...
  if (error || !prompt) {
    return (
      <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-3xl mx-auto">
        <ErrorState
          title="Error"
          message={error || "Prompt not found"} // defaults-ok
