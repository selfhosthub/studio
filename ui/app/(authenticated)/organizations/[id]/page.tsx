// ui/app/(authenticated)/organizations/[id]/page.tsx
...
export default function OrganizationDetailsPage() {
  ...
  // Toggle staging-tenant flag (super_admin only). Staging Organizations see
  // staging-visibility catalog packages in addition to public ones.
  const handleToggleStagingTenant = async () => {
    const newValue = !organization.is_staging;
    try {
      const updated = await updateOrganization(orgId, { is_staging: newValue });
      setOrganization(updated);
      toast({
        title: newValue ? 'Marked as staging organization' : 'Unmarked as staging organization',
        variant: 'success',
      });
    } catch (err: unknown) {
      toast({
        title: 'Failed to update staging-tenant flag',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg shadow-md p-6 border border-primary">
        ...
        <div className="space-y-4">
          ...
          {/* Staging organization (super_admin only) */}
          {isSuperAdmin && (
            <div>
              <p className="block text-sm font-medium text-secondary mb-2">
              ...
              <div className="flex items-center space-x-3 px-4 py-2 bg-card rounded-md border border-primary">
                <StatusBadge
                  status={organization.is_staging ? 'staging' : 'standard'}
                  variant={organization.is_staging ? 'warning' : 'default'}
                />
                <ActionButton
                  variant={organization.is_staging ? 'warning' : 'active'}
                  onClick={handleToggleStagingTenant}
                >
                  {organization.is_staging ? 'Revoke Access' : 'Grant Access'}
                </ActionButton>
                <p className="text-sm text-secondary">
                  Staging Organizations see staging-visibility catalog packages.
                </p>
              </div>
            </div>
          )}
