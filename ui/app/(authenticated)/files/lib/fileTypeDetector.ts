// ui/app/(authenticated)/files/lib/fileTypeDetector.ts
...
export function getSourceDescription(source: string): string {
  switch (source) {
    case 'job_generated': return 'Created by a Studio worker (e.g. ComfyUI, TTS, video render).';
    case 'job_download': return 'Retrieved by a Studio worker from an external provider (e.g. Pexels, stock library).';
    case 'user_upload': return 'Uploaded directly by a user.';
    default: return source;
  }
}

export function getSourceProvenance(resource: OrgFile): string | null {
  if (resource.source === 'job_download' && resource.provider_url) {
    try {
      return new URL(resource.provider_url).hostname;
    } catch {
      return resource.provider_resource_id;
    }
  }
  if (resource.source === 'job_generated' && resource.instance_id) {
    return `instance ${resource.instance_id.slice(0, 8)}`;
  }
  return null;
}
