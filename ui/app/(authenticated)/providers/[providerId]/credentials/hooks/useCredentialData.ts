// ui/app/(authenticated)/providers/[providerId]/credentials/hooks/useCredentialData.ts
...
export function useCredentialData({ providerId }: UseCredentialDataOptions) {
  ...
  // Fetch provider, credentials, and OAuth providers (Pattern A: async IIFE)
  useEffect(() => {
    void (async () => {
      ...
      try {
        const [providerData, creds] = await Promise.all([
