// ui/app/(authenticated)/providers/[providerId]/credentials/components/CredentialListItem.tsx
...
import type { Credential } from '@/features/providers';
  ...
  return (
    <div className="bg-card border border-primary rounded-lg overflow-hidden">
      ...
      <div className="px-6 py-4">
        ...
        {/* Webhook callback URL - present once this credential is webhook-capable */}
        {cred.webhook_callback_token && <CallbackUrlDisplay token={cred.webhook_callback_token} />}
      </div>
    </div>
  );
}
...
/* -------------------------------------------------------------------------- */
/*  Internal sub-component: callback URL display                               */
/* -------------------------------------------------------------------------- */

function CallbackUrlDisplay({ token }: { token: string }) {
  const [copied, setCopied] = useState(false);
  const url = getApiEndpoint(`/webhooks/incoming/${token}`);
    ...
    <div className="mt-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          readOnly
          value={url}
          className="flex-1 min-w-0 bg-input border border-primary rounded-lg p-3 font-mono text-xs text-primary"
        />
        <button
          type="button"
          onClick={handleCopy}
          aria-label="Copy callback URL"
          className="px-3 py-2 bg-input text-secondary rounded hover:opacity-80 transition-colors"
        >
          {copied ? <Check className="w-4 h-4 text-success" /> : <Copy className="w-4 h-4" />}
        </button>
      </div>
      <p className="text-xs text-muted mt-1 italic">
        Paste into the provider&apos;s dashboard when creating its API key (webhook mode only).
      </p>
    </div>
  );
}
