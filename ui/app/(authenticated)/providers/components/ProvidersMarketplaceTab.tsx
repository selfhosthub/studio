// ui/app/(authenticated)/providers/components/ProvidersMarketplaceTab.tsx
...
// Hoisted out of the parent (react-hooks/static-components): a component defined
// inside another remounts every render. Sort state is passed as props.
...
export function ProvidersMarketplaceTab({ isSuperAdmin }: ProvidersMarketplaceTabProps) {
  ...
  useEffect(() => {
    // IIFE so the deferred setState inside fetchCatalog isn't read as a synchronous set.
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'category':
          ...
          break;
        case 'tier':
          const tierOrder: Record<ProviderTier, number> = { 'community': 0, 'plus': 1 };
  ...
  // Reset page when filters change (adjust during render, not in an effect).
  const filterKey = `${searchQuery}|${tierFilter}|${categoryFilter}|${statusFilter}`;
  ...
  const canInstall = (pkg: MarketplacePackage) => {
    const hasDownload = (pkg.versions && pkg.versions.length > 0) || pkg.download_url || pkg.latest_url;
    if (!hasDownload) return false;
    if (pkg.tier === 'plus' && !tokenConfigured) return false;
    return true;
  };
  ...
  return (
    <>
      ...
      <div className="mb-6 flex flex-wrap gap-3 items-center">
        ...
        {isSuperAdmin && (
          <>
            {(packages.some(p => p.tier === 'community' && p.status === 'available' && canInstall(p)) ||
              (tokenConfigured && packages.some(p => p.tier === 'plus' && p.status === 'available' && canInstall(p)))) && (
              <InstallAllDropdown
                hasCommunity={packages.some(p => p.tier === 'community' && p.status === 'available' && canInstall(p))}
                hasPlus={tokenConfigured && packages.some(p => p.tier === 'plus' && p.status === 'available' && canInstall(p))}
            ...
            <TableHeader>
              <TableRow>
                <TableHeaderCell onClick={() => handleSort('display_name')}>
                  Package
                  <SortIndicator field="display_name" sortField={sortField} sortDirection={sortDirection} />
                </TableHeaderCell>
                <TableHeaderCell align="center" onClick={() => handleSort('category')}>
                  Category
                  <SortIndicator field="category" sortField={sortField} sortDirection={sortDirection} />
                </TableHeaderCell>
                ...
                <TableHeaderCell align="center" onClick={() => handleSort('tier')}>
                  Tier
                  <SortIndicator field="tier" sortField={sortField} sortDirection={sortDirection} />
                </TableHeaderCell>
                <TableHeaderCell align="center">
                  Services
                </TableHeaderCell>
                        ...
                        <>
                          {canInstall(pkg) ? (
                            ...
                            <div className="flex flex-col items-center gap-1">
                              <span
                                className="action-btn-locked"
                                title="Plus package - requires entitlement token"
                              >
                                <Lock className="w-3 h-3 mr-1" />
                                Plus
                              </span>
