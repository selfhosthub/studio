// ui/app/(authenticated)/workflows/[id]/edit/hooks/useWorkflowLoader.ts
...
export function useWorkflowLoader(workflowId: string, options?: UseWorkflowLoaderOptions) {
  ...
  // Ref prevents effect re-runs when the callback identity changes. Updated in an
  // effect (not during render) per react-hooks/refs; it's only read async after the
  // fetch resolves, so post-commit timing is equivalent.
