// ui/app/(authenticated)/workflows/[id]/edit/hooks/useCredentialCheck.ts
...
export function useCredentialCheck(workflowId: string | undefined) {
  ...
  useEffect(() => {
    // IIFE so the deferred setState inside refreshCredentialCheck isn't read as a synchronous set.
    void (async () => { await refreshCredentialCheck(); })();
