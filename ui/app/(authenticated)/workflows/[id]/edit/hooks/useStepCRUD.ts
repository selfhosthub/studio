// ui/app/(authenticated)/workflows/[id]/edit/hooks/useStepCRUD.ts
...
export function useStepCRUD(
  ...
  const addStep = () => {
    ...
    const newStep = buildNewStep(newPosition);
  ...
  const addConnectedStep = (sourceStepId: string, position: { x: number; y: number }) => {
    if (!workflow.steps.some((s: Step) => s.id === sourceStepId)) return;
    const newStep = buildNewStep(position, [sourceStepId]);

    const newConnection = {
      id: `conn-${sourceStepId}-${newStep.id}`,
    ...
    setWorkflow((prevWorkflow: any) => ({
      ...prevWorkflow,
      steps: topologicalSortSteps([...prevWorkflow.steps, newStep]),
