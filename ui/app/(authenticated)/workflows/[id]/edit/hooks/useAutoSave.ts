// ui/app/(authenticated)/workflows/[id]/edit/hooks/useAutoSave.ts
...
export function useAutoSave({ onSave, hasUnsavedChanges, enabled = true }: UseAutoSaveOptions) {
  const [autoSaveInterval, setAutoSaveInterval] = useState<number | null>(() => {
    ...
    const saved = window.localStorage.getItem(STORAGE_KEYS.WORKFLOW_AUTO_SAVE_INTERVAL);
  ...
  useEffect(() => {
    if (autoSaveInterval) {
      localStorage.setItem(STORAGE_KEYS.WORKFLOW_AUTO_SAVE_INTERVAL, String(autoSaveInterval));
    } else {
      localStorage.removeItem(STORAGE_KEYS.WORKFLOW_AUTO_SAVE_INTERVAL);
    }
  }, [autoSaveInterval]);
