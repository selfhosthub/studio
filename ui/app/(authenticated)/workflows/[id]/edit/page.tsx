// ui/app/(authenticated)/workflows/[id]/edit/page.tsx
...
import { validateDependencies, topologicalSortSteps, findDisconnectedSteps } from '@/shared/lib/step-utils';
import { updateWorkflow, exportWorkflow } from '@/shared/api';
import { useRouter } from 'next/navigation';
import { StepConfigPanel } from '@/widgets/flow-editor';
import { WorkflowStepConfig } from '@/app/(authenticated)/workflows/components/WorkflowStepConfig';
...
export default function WorkflowEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Suspense>
      <WorkflowEditPageContent id={id} />
    </Suspense>
  );
}

function WorkflowEditPageContent({ id }: { id: string }) {
  const router = useRouter();
  const { returnTo } = useReturnTo('/workflows/list');
  ...
  const handleTriggerChange = async (newTriggerType: string) => {
    ...
    // Switching to webhook no longer mints a token/secret automatically - the
    // user generates it explicitly (so selecting the type doesn't create an
    // org secret prematurely, mirroring the API-key flow).
  ...
  const handleScheduleSaved = (result: { enabled: boolean; next_run_at: string | null; rrule: string | null; dtstart: string | null; timezone: string }) => {
    setWorkflow((prev: any) => prev && ({
      ...prev,
      schedule_rrule: result.rrule,
      schedule_dtstart: result.dtstart,
      schedule_timezone: result.timezone,
      schedule_enabled: result.enabled,
      schedule_next_run_at: result.next_run_at,
    }));
  };
  ...
  const handleScheduleCleared = () => {
    setWorkflow((prev: any) => prev && ({
      ...prev,
      schedule_rrule: null,
      schedule_dtstart: null,
      schedule_enabled: false,
      schedule_next_run_at: null,
    }));
  };

  // API-key trigger handlers (key value itself is never stored client-side beyond the one-time reveal)
  const handleApiKeyChanged = (hasApiKey: boolean) => {
    setWorkflow((prev: any) => prev && ({ ...prev, api_key_set: hasApiKey }));
  };

  // Event trigger handlers
  const handleEventSaved = (sourceWorkflowId: string, on: 'completed' | 'failed' | 'terminal') => {
    setWorkflow((prev: any) => prev && ({ ...prev, event_source_workflow_id: sourceWorkflowId, event_on: on }));
  };

  const handleEventCleared = () => {
    setWorkflow((prev: any) => prev && ({ ...prev, event_source_workflow_id: null, event_on: null }));
  };
  ...
  const saveWorkflow = async (e: React.FormEvent) => {
    ...
    const disconnected = findDisconnectedSteps(stepsRecord);
    if (disconnected.length > 0) {
      const names = disconnected.map((id) => stepsRecord[id].name || id).join(', ');
      if (!confirm(`These steps aren't connected to the workflow and won't run:\n\n${names}\n\nSave anyway?`)) {
        return;
      }
    }
  ...
  // Run is only available for active workflows, and never to super-admins;
  // missing status defaults to active.
  const canRun = !isSuperAdmin && (workflow?.status ?? 'active') === 'active'; // defaults-ok
    ...
    return (
      <div className="p-6">
        <div className="alert alert-error p-6">
          ...
          <div className="mt-4">
            <Link href={returnTo} className="link">Back to workflow list</Link>
          </div>
        </div>
      </div>
    );
    ...
    <div className={`p-2 ${widthClass} mx-auto`}>
      ...
      <form onSubmit={saveWorkflow}>
        ...
        <WorkflowSettingsCollapsible
          workflow={workflow}
          webhook={webhook}
          canManageTriggers={user?.role === 'admin' || user?.role === 'super_admin'}
          onPropertyChange={handleWorkflowPropertyChange}
          onTriggerChange={handleTriggerChange}
          onMethodChange={handleWebhookMethodChange}
          onGenerateToken={handleGenerateWebhookToken}
          ...
          onEventCleared={handleEventCleared}
        />
        ...
        <StepConfigPanel
          ...
          stepConfigContent={steps.selectedStep ? (
            <WorkflowStepConfig
              ...
              reauthorize={(() => {
                const issue = credentialCheck?.issues?.find(
                  (i: any) => i.step_id === steps.selectedStep!.id && i.status === 'reauthorize_required'
                );
                return issue ? { message: issue.message, actionUrl: issue.action_url } : undefined;
              })()}
            />
          ) : null}
        />
