// ui/app/(authenticated)/workflows/[id]/edit/components/WebhookConfigSection.tsx
...
export function WebhookConfigSection({
  ...
  return (
    <div className="md:col-span-2 p-3 bg-surface rounded-md border border-primary">
      {!webhookToken ? (
        // No URL yet - generate it explicitly (selecting the webhook trigger no
        // longer mints a secret on its own).
        <div className="space-y-3">
          <p className="text-muted text-xs">
            Generate a webhook URL to trigger this workflow over HTTP.
          </p>
          <button
            type="button"
          ...
          >
            {webhook.webhookTokenLoading ? 'Generating...' : 'Generate webhook URL'}
          </button>
        </div>
      ) : (
        <div className="space-y-3">
