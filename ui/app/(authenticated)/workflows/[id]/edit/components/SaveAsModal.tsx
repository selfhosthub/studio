// ui/app/(authenticated)/workflows/[id]/edit/components/SaveAsModal.tsx
...
export function SaveAsModal({ isOpen, onClose, workflow, onSuccess }: SaveAsModalProps) {
  ...
  // Seed form fields when the modal opens (or the workflow changes). The setStates
  // bail when the value is unchanged, so this is loop-safe; the IIFE keeps the sets
  // off the synchronous effect path (react-hooks/set-state-in-effect).
  useEffect(() => {
    if (isOpen && workflow) {
      void (async () => {
