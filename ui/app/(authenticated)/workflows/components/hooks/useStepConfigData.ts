// ui/app/(authenticated)/workflows/components/hooks/useStepConfigData.ts
...
export function useStepConfigData({ step, onUpdate, allSteps }: UseStepConfigDataArgs) {
  ...
  // localStorage isn't available during SSR, so read it once on mount behind a ref
  // guard and set state directly (compute-then-set, not buried in a branch).
  ...
  useEffect(() => {
    ...
    // Unconditional set after the guard; null falls back to the default (equal to
    // the initial state, so it's a no-op) which keeps this off the conditional path.
    const next = savedState === null ? STEP_CONFIG_DEFAULTS.serviceParametersExpanded : savedState !== 'false';
    setServiceParametersExpanded(next);
  }, []);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // IIFE so the synchronous clear-to-[] isn't read as a synchronous set within
    // the effect body. The no-prompt branch still runs synchronously.
    void (async () => {
      if (!promptId) {
        setPromptVarNames([]);
        return;
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      ...
      const requiresCreds = selectedService?.requires_credentials;
  ...
  // Materialize schema defaults - untouched fields won't get saved unless written to
  // state. Adjust during render (not in an effect) when paramSchema changes; existing
  // parameters win via {...defaults, ...prev}.
  const [prevParamSchema, setPrevParamSchema] = useState(paramSchema);
  if (paramSchema !== prevParamSchema) {
    setPrevParamSchema(paramSchema);
  ...
  // Reset local state when the user switches steps. Keyed on step.id only to avoid
  // update loops. Adjust during render (not in an effect) so the reset lands before
  // commit; the onUpdate effect below still skips once via its syncedStepIdRef guard.
