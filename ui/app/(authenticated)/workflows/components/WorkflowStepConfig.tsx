// ui/app/(authenticated)/workflows/components/WorkflowStepConfig.tsx
...
import { usePromptSystemOverride } from '@/features/step-config/usePromptSystemOverride';
...
import CompletionModeSection from '@/features/step-config/sections/CompletionModeSection';
import {
  SERVICE_TYPES,
  SERVICE_TYPE_LABELS,
} from '@/entities/provider';
import { CredentialSelector } from '@/features/providers';
import { getProviderCredential } from '@/shared/api/providers';
...
interface WorkflowStepConfigProps {
  ...
  /** When set, the step's OAuth credential needs re-authorization; links to the provider secrets page. */
  reauthorize?: { message: string; actionUrl: string };
}
...
export function WorkflowStepConfig({
  ...
  // ── System-slot override ────────────────────────────────────
  // When a bound AI Prompt carries system content, fields marked
  // ui.system_slot (e.g. claude `system`) render disabled - the dispatch
  // shaper gives the prompt's system precedence.
  const systemProvidedByPrompt = usePromptSystemOverride(inputMappings);

  // ── Provider docs slide-over ────────────────────────────────
  const [isDocsOpen, setIsDocsOpen] = useState(false);
  const [docsSlug, setDocsSlug] = useState<string | null>(null);

  // ── Webhook-callback-key presence (for CompletionModeSection's warning) ──
  // undefined => unknown (no credential selected / not webhook-capable).
  const [credentialHasWebhookKey, setCredentialHasWebhookKey] = useState<
    boolean | undefined
  >(undefined);
  const supportsWebhookMode = Array.isArray(serviceMetadata?.completion_modes)
    && serviceMetadata.completion_modes.includes('webhook');
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!supportsWebhookMode || !credentialId) {
        if (!cancelled) setCredentialHasWebhookKey(undefined);
        return;
      }
      try {
        const cred = await getProviderCredential(credentialId);
        if (!cancelled) {
          setCredentialHasWebhookKey(Boolean(cred.has_webhook_callback_key));
        }
      } catch {
        if (!cancelled) setCredentialHasWebhookKey(undefined);
      }
  ...
  }, [supportsWebhookMode, credentialId]);
  ...
  const handleServiceChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (newServiceId) {
      const newSelectedService = services.find((s: Record<string, any>) => s.service_id === newServiceId);
      const requiresCreds = newSelectedService?.requires_credentials;
    ...
    return (
      <div key={key} id={`param-field-${key}`} className="p-3 bg-info-subtle border border-info rounded-md transition-all">
        <MappableParameterField
          ...
          systemProvidedByPrompt={systemProvidedByPrompt}
          instanceFormFields={instanceFormFields}
          currentStepId={step.id}
          arrayMappingNotice={serviceMetadata?.array_mapping_notice}
        />
    ...
    <div>
      <div className="bg-card shadow-sm rounded-lg border p-6">
        <div className="space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="@container bg-card shadow-sm rounded-b-lg p-4 border border-t-0 border-primary">
              <div className="space-y-4">
                ...
                {fieldRules.showProvider && providerId && (selectedService?.requires_credentials !== false || !!selectedService?.dispatch?.length) && (
                  <CredentialSelector
                    ...
                    allowCrossProvider={!!selectedService?.dispatch?.length}
                ...
                {reauthorize && (
                  <div className="bg-warning-subtle border border-warning rounded-md p-3 flex items-start gap-2">
                    ...
                    <p className="text-sm text-warning">
                      {reauthorize.message}{' '}
                      <Link href={reauthorize.actionUrl} className="underline hover:no-underline font-medium">
                        Re-authorize →
                      </Link>
                    </p>
                  </div>
          ...
          {serviceId && (
            <div className="mt-6">
              <button type="button" className="w-full flex justify-between items-center cursor-pointer p-3 bg-blue-800 rounded-t-md" onClick={() => { const newState = !serviceParametersExpanded; localStorage.setItem(STORAGE_KEYS.SERVICE_PARAMETERS_EXPANDED, String(newState)); setServiceParametersExpanded(newState); }}> {/* css-check-ignore: no semantic token */}
                <div className="flex items-center gap-2">
                  <h3 className="text-md font-medium text-white">{selectedService?.parameter_title ?? STEP_CONFIG_DEFAULTS.parameterSectionTitle}</h3>
              ...
              {serviceParametersExpanded && (
                <div className="p-4 bg-info-subtle rounded-b-lg">
                  ...
                  {/* Completion Mode - boxed like the other sub-sections; under Run-once-per-item. Renders only for services that declare webhook support. */}
                  <CompletionModeSection
                    serviceMetadata={serviceMetadata}
                    credentialProviderId={credentialProviderId || providerId}
                    credentialHasWebhookKey={credentialHasWebhookKey}
                    completionMode={step.client_metadata?.completion_mode}
                    onCompletionModeChange={(m) =>
                      onUpdate({
                        ...step,
                        client_metadata: { ...(step.client_metadata || {}), completion_mode: m },
                      })
                    }
                  />
                  {serviceId === 'shs/core.set_fields' ? (
                    <SetFieldsEditor parameters={parameters} onParametersChange={setParameters} onOutputFieldsChange={setOutputFields} />
                  ) : serviceId === 'shs/core.webhook_wait' && workflowId ? (
          ...
          {serviceId && !selectedService?.custom_output && (
            <div className="mt-6">
