...
function ProvidersDocsContent() {
  ...
  // (Pattern C: adjust during render keyed on providers identity; fires same times as the old effect)
  const [prevProviders, setPrevProviders] = useState(providers);
  if (providers !== prevProviders) {
    setPrevProviders(providers);
    if (providers.length > 0 && (!selectedSlug || !providers.some(p => p.id === selectedSlug))) {
