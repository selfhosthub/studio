// ui/app/docs/super-admin/page.tsx
...
import { ArrowLeft, Menu, X, Lock, ChevronRight, ChevronDown, Lightbulb, AlertTriangle, Info, Link as LinkIcon } from 'lucide-react';
...
export default function SuperAdminDocPage() {
  ...
  useEffect(() => {
    ...
    const fetchDoc = async () => {
      try {
        const data = await apiRequest<DocContent>('/docs/super-admin/content');
        setDoc(data);
      } catch (err) {
        ...
        if (status === 403) {
          ...
          setError('Super Admin documentation not found');
        } else {
    ...
    return (
      <main className="flex min-h-screen flex-col bg-card">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="animate-pulse text-secondary">Checking access...</div>
        </div>
        <Footer />
      </main>
    );
  ...
  // Show access denied (useRoleAccess will redirect)
  if (!hasAccess) {
    return (
      <main className="flex min-h-screen flex-col bg-card">
        <Navbar />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-secondary mb-4">
              This documentation is only available to Super Administrators.
            </p>
            <Link
              href="/docs"
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-4">
            <a
              ...
              className="text-sm font-semibold text-primary hover:text-info transition-colors"
            >
              {doc.title}
            </a>
            <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300"> {/* css-check-ignore: no semantic token */}
              Super Admin
            </span>
          </div>
