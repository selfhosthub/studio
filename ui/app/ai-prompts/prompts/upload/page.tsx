// ui/app/ai-prompts/prompts/upload/page.tsx

"use client";

import React, { useState, useRef } from 'react';
import { useRouter } from 'next/navigation';
import { importPrompt } from '@/shared/api';
...
export default function UploadPromptPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFile) {
      setError('Please select a prompt file');
      return;
    }
    ...
    try {
      const result = await importPrompt(selectedFile);
      router.push(`/ai-prompts/prompts/${result.prompt.id}/edit`);
    } catch (err: unknown) {
      console.error('Failed to upload prompt:', err);
      setError(err instanceof Error ? err.message : 'Failed to upload prompt');
      setLoading(false);
    }
  };

  // No DashboardLayout wrapper — ui/app/ai-prompts/layout.tsx already provides it
  // (double-wrapping renders the navbar twice).
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-sm mt-1 text-secondary">
          Install a prompt from a single JSON file
        </p>
      </div>
        ...
        >
          ...
          {selectedFile ? (
            ...
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-primary">
                Drop your prompt file here, or click to browse
              </p>
              <p className="mt-1 text-xs text-secondary">
                JSON files only
              </p>
        ...
        <div className="mt-6 bg-surface rounded-lg p-4">
          ...
          <ul className="text-xs text-secondary space-y-1">
            <li className="flex items-center gap-2">
              ...
              <code className="bg-card px-1 rounded">name</code>, <code className="bg-card px-1 rounded">chunks</code>
            </li>
          </ul>
        </div>
