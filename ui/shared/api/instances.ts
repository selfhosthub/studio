...
/** Result of a user-initiated provider poll on a step awaiting a webhook callback. */
export interface CheckWebhookStatusResponse {
  polls_enqueued: number;
  iterations_pending: number;
}

/** User-initiated provider poll for a step parked awaiting a webhook callback. */
export async function checkWebhookStatus(
  instanceId: string,
  stepId: string,
): Promise<CheckWebhookStatusResponse> {
  return apiRequest<CheckWebhookStatusResponse>(
    `/instances/${instanceId}/step_executions/${stepId}/check-webhook-status`,
    { method: 'POST' },
  );
}
