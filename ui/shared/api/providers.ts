...
/** Identity-only provider list ({id, slug}) for readiness/membership checks.
 * Far smaller than getProviders() - skips services/config/metadata. */
export async function getProvidersSlim(): Promise<Array<{ id: string; slug: string }>> {
  return apiRequest<Array<{ id: string; slug: string }>>('/providers/?slim=true');
}
...
export async function updateProviderCredential(credentialId: string, updates: {
  ...
  if (updates.webhook_callback_token !== undefined) {
    apiPayload.webhook_callback_token = updates.webhook_callback_token;
  }
...
export interface OrganizationSecretUsage {
  count: number;
  workflow_names: string[];
}

// How many workflows trigger off this secret — used to warn before deletion that
// confirming resets those workflows to a manual trigger.
export async function getOrganizationSecretUsage(secretId: string): Promise<OrganizationSecretUsage> {
  return apiRequest<OrganizationSecretUsage>(`/organizations/secrets/${secretId}/usage`);
}
