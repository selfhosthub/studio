...
export async function setWorkflowVisibility(
  ...
  return apiRequest<WorkflowResponse>(`/workflows/${workflowId}/visibility`, {
    method: 'POST',
...
export async function setWorkflowSchedule(
  ...
  return apiRequest<WorkflowScheduleResponse>(`/workflows/${workflowId}/schedule`, {
    method: 'PUT',
...
export async function clearWorkflowSchedule(workflowId: string): Promise<void> {
  await apiRequest(`/workflows/${workflowId}/schedule`, { method: 'DELETE' });
}

// --- API-key trigger ---

export interface WorkflowApiKeyResponse {
  api_key: string;
  trigger_url: string;
  /** Workflows sharing this key's trigger secret (incl. this one). >1 → the
   *  workflow UI defers regeneration to the Secrets page. */
  shared_by_count?: number;
}

/** api_key is returned ONCE on create. */
export async function generateWorkflowApiKey(workflowId: string): Promise<WorkflowApiKeyResponse> {
  return apiRequest<WorkflowApiKeyResponse>(`/workflows/${workflowId}/api-key`, {
    method: 'POST',
  });
}

/** Regenerate invalidates the old key immediately; api_key returned ONCE. */
export async function regenerateWorkflowApiKey(workflowId: string): Promise<WorkflowApiKeyResponse> {
  return apiRequest<WorkflowApiKeyResponse>(`/workflows/${workflowId}/api-key/regenerate`, {
    method: 'POST',
  });
}

export async function clearWorkflowApiKey(workflowId: string): Promise<void> {
  await apiRequest(`/workflows/${workflowId}/api-key`, { method: 'DELETE' });
}

/** Recall the existing API key (admin only; stored encrypted-at-rest). 404 if none. */
export async function recallWorkflowApiKey(workflowId: string): Promise<WorkflowApiKeyResponse> {
  return apiRequest<WorkflowApiKeyResponse>(`/workflows/${workflowId}/api-key`);
}

/** A reusable trigger secret the admin can share across workflows. */
export interface TriggerSecretOption {
  id: string;
  name: string;
  shared_by_count: number;
}
 ...
 *  Each credential lives in its own typed secret, so this filters on the column. */
export async function listWorkflowTriggerSecretOptions(
  workflowId: string,
  secretType: TriggerSecretType,
): Promise<TriggerSecretOption[]> {
  return apiRequest<TriggerSecretOption[]>(
    `/workflows/${workflowId}/trigger-secret/options?secret_type=${secretType}`,
  );
}

/** Point a workflow at an existing trigger secret (share it; admin only). */
export async function setWorkflowTriggerSecret(
  workflowId: string,
  secretId: string,
): Promise<WorkflowResponse> {
  return apiRequest<WorkflowResponse>(`/workflows/${workflowId}/trigger-secret`, {
    method: 'PUT',
    body: JSON.stringify({ secret_id: secretId }),
  });
}

export interface RotateTriggerSecretKeyResponse {
  api_key: string;
  shared_by_count: number;
}

/** Deliberately rotate a shared `workflow_trigger` secret's API key from the
 *  Secrets page (admin only). Every workflow sharing it gets the new key. */
export async function regenerateTriggerSecretKey(
  secretId: string,
): Promise<RotateTriggerSecretKeyResponse> {
  return apiRequest<RotateTriggerSecretKeyResponse>(
    `/workflows/trigger-secrets/${secretId}/regenerate-key`,
    { method: 'POST' },
  );
}
...
export interface WorkflowEventBody {
  ...
  on: 'completed' | 'failed' | 'terminal';
}

export async function setWorkflowEventTrigger(
  workflowId: string,
  body: WorkflowEventBody,
): Promise<WorkflowResponse> {
  return apiRequest<WorkflowResponse>(`/workflows/${workflowId}/event`, {
    method: 'PUT',
    body: JSON.stringify(body),
  });
}

export async function clearWorkflowEventTrigger(workflowId: string): Promise<void> {
  await apiRequest(`/workflows/${workflowId}/event`, { method: 'DELETE' });
}
...
export interface WorkflowConnectionPreview {
  provider: string;
  services: string[];
}
...
export interface MarketplaceWorkflowDetail extends MarketplaceWorkflow {
  steps: WorkflowStepPreview[];
  connections: WorkflowConnectionPreview[];
  credentials: string[];
...
/** Super-admin: full pre-install detail (step DAG, connections, credentials,
 * I/O) for one catalog workflow. Fetched lazily on modal-open. */
export async function getWorkflowDetail(workflowId: string): Promise<MarketplaceWorkflowDetail> {
  return apiRequest<MarketplaceWorkflowDetail>(`/workflows/marketplace/catalog/${workflowId}`);
}

export async function installWorkflowFromMarketplace(
  workflowId: string,
  opts?: { force?: boolean },
): Promise<WorkflowInstallResponse> {
  const qs = opts?.force ? '?force=true' : '';
  return apiRequest<WorkflowInstallResponse>(`/workflows/marketplace/install/${workflowId}${qs}`, {
    method: 'POST',
  });
}
