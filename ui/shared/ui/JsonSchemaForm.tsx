...
export default function JsonSchemaForm({
  ...
  // The merge runs during render (not in an effect) when schema/exampleParameters
  // change, so seeded defaults don't require a cascading commit. Existing formData
  // (including user edits) always wins.
  const [seededFrom, setSeededFrom] = useState<{ schema: ParameterSchema; example: Record<string, any> } | null>(null);
  const [seedToNotify, setSeedToNotify] = useState<Record<string, any> | null>(null);
  if (schema?.properties && (!seededFrom || seededFrom.schema !== schema || seededFrom.example !== exampleParameters)) {
    setSeededFrom({ schema, example: exampleParameters });
    ...
    if (Object.keys(combinedData).length > 0) {
      ...
      setFormData(newData);
      setSeedToNotify(newData);
    }
  }

  // Notify the parent of seeded values. Kept in an effect because calling onChange
  // (a parent setState) during render is illegal. Fires once per seed event.
  useEffect(() => {
    if (seedToNotify) {
      onChange(seedToNotify);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- fire only when a new seed is produced, not on onChange identity change
  }, [seedToNotify]);
  ...
  if (!schema || !schema.properties) {
    return <div className="p-4 border border-danger bg-danger-subtle rounded">Invalid schema: no properties defined</div>;
  }
