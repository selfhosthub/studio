...
/**
 * Egress sentinel the API returns in place of a configured-but-not-revealed
 * secret-class value (webhook secret / auth-header value / JWT secret). The UI
 * must not pre-fill it into an editable field and must not send it back as a
 * real value; the API ignores it on save.
 */
export const CONFIGURED_SENTINEL = '[CONFIGURED]';
...
export const TIMEOUTS = {
  ...
  COPY_FEEDBACK: 2000,
  /** Shorter copy-to-clipboard feedback */
  COPY_FEEDBACK_SHORT: 1500,
  ...
  HIGHLIGHT_RING: 2000,
  /** One paint tick before triggering a mount transition */
  MOUNT_TICK: 10,
  /** Delay before closing hover-expanded sidebar submenus */
  SUBMENU_CLOSE: 50,
  /** Brief delay before a post-action redirect */
  REDIRECT_DELAY: 1500,
} as const;
...
export const PAGINATION = {
  ...
  COMPACT_PAGE_SIZE: envInt('NEXT_PUBLIC_PAGE_SIZE_COMPACT', 10),
...
export const STORAGE_KEYS = {
  ...
  FILES_VIEW_MODE: 'files-view-mode',
  PROVIDERS_VIEW_MODE: 'providers-view-mode',
  STEP_CONFIG_PANEL_WIDTH: 'stepConfigPanelWidth',
  STEP_CONFIG_OUTPUTS_PANEL_WIDTH: 'stepConfigOutputsPanelWidth',
  STEP_CONFIG_SHOW_OUTPUTS_PANEL: 'stepConfigShowOutputsPanel',
  WORKFLOW_AUTO_SAVE_INTERVAL: 'workflowAutoSaveInterval',
  GLOBAL_WORKFLOW_SETTINGS_EXPANDED: 'globalWorkflowSettingsExpanded',
} as const;
