...
/** Step ids that neither depend on a step nor are depended on, in a workflow with more than one step. */
export function findDisconnectedSteps(
  steps: Record<string, { depends_on?: string[] }>
): string[] {
  ...
  for (const step of Object.values(steps)) {
    for (const depId of step.depends_on || []) {
      if (depId !== '__instance_form__') hasDependents.add(depId);
    }
  }

  return ids.filter(id => {
    const deps = (steps[id].depends_on || []).filter(d => d !== '__instance_form__');
    return deps.length === 0 && !hasDependents.has(id);
  });
}
...
type OrderableStep = { id: string; depends_on?: string[]; ui_config?: { position?: { y?: number } } };

/**
 * Canonical step display order — the TS twin of the backend `order_steps_topologically`.
 * Sorts by DAG depth (longest path from a root), then canvas Y, then insertion. Cycle-safe.
 */
export function orderStepsByDepth<T extends OrderableStep>(steps: T[]): T[] {
  if (steps.length <= 1) return steps;

  const byId = new Map(steps.map(s => [s.id, s]));
  const depsOf = (s: T): string[] => (s.depends_on || []).filter(d => byId.has(d));
  ...
  const depthOf = (id: string): number => {
    ...
    if (computing.has(id)) return 0; // cycle: treat as a root to break recursion
    computing.add(id);
    const step = byId.get(id)!;
    const deps = depsOf(step);
    const result = deps.length === 0 ? 0 : 1 + Math.max(...deps.map(depthOf));
    computing.delete(id);
    depth.set(id, result);
    return result;
  };

  const yOf = (s: T): number => {
    const y = s.ui_config?.position?.y;
    return typeof y === 'number' ? y : Infinity;
  };

  const insertion = new Map(steps.map((s, i) => [s.id, i]));
  return [...steps].sort((a, b) =>
    depthOf(a.id) - depthOf(b.id) ||
    yOf(a) - yOf(b) ||
    insertion.get(a.id)! - insertion.get(b.id)!
  );
}

/** @deprecated Use {@link orderStepsByDepth}. Retained as an alias for call-site compatibility. */
export function topologicalSortSteps<T extends OrderableStep>(steps: T[]): T[] {
  return orderStepsByDepth(steps);
}

export function getUpstreamSteps<T extends OrderableStep>(
  stepId: string,
  allSteps: T[]
): T[] {
  ...
  // Order the closure by depth so the picker reads in execution order regardless of input order.
  return orderStepsByDepth(allSteps.filter((s) => upstreamStepIds.has(s.id)));
}
