...
export interface StepConfigDefaults {
  ...
  /** Async completion mode for a step that hasn't set one - synchronous polling. */
  completionMode: 'get' | 'webhook';
  /** Parameter-section title when the service declares no custom parameter_title. */
...
export const STEP_CONFIG_DEFAULTS: StepConfigDefaults = {
  ...
  filesPerIteration: 1,
  completionMode: 'get',
  parameterSectionTitle: 'Service Parameters',
};
