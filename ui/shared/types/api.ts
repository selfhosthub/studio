...
export interface WorkflowResponse {
  ...
  webhook_token?: string | null; // per-workflow URL capability token
  webhook_secret?: string | null; // null or "[CONFIGURED]" on read (never the raw secret)
  webhook_auth_type?: string;
  webhook_auth_header_value?: string; // null or "[CONFIGURED]" on read (never the raw secret)
  webhook_jwt_secret?: string; // null or "[CONFIGURED]" on read (never the raw secret)
  // Schedule trigger (read fields; api_key is never returned on GET)
  schedule_rrule?: string | null;
  schedule_dtstart?: string | null;
  schedule_timezone?: string | null;
  schedule_enabled?: boolean | null;
  schedule_next_run_at?: string | null;
  // Event trigger
  event_source_workflow_id?: string | null;
  event_on?: 'completed' | 'failed' | 'terminal' | null;
  created_by?: string; // UUID
  scope?: string; // 'personal' | 'organization'
  publish_status?: string | null; // null | 'pending' | 'rejected'
  visibility?: string; // 'private' | 'staging' | 'public' (cross-org marketplace)
  instance_count?: number; // times this workflow has executed
  can_be_deleted?: boolean; // false while live instances exist
...
export interface StepExecutionResponse {
  ...
  can_approve: boolean;
  can_trigger: boolean;
  can_check_status: boolean; // WFW steps: provider has a pollable status endpoint
  is_terminal: boolean;
...
export interface OrgFile {
  ...
  is_selected: boolean; // keep-variants: the chosen variant for its (job_execution_id, iteration_index) slot; only selected variants flow downstream
