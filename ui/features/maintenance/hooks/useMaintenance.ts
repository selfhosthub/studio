...
export function useMaintenance(options?: UseMaintenanceOptions): MaintenanceStatus {
  ...
  // Holds the latest connectWebSocket() so the reconnect timer can call it without
  // connectWebSocket referencing itself (react-hooks/immutability). Set in an effect.
  ...
  // Keep connectWebSocketRef pointing at the latest connectWebSocket.
  useEffect(() => {
    connectWebSocketRef.current = connectWebSocket;
  }, [connectWebSocket]);

  // Handle maintenance events from authenticated WebSocket (for logged-in users).
  // Applied during render when the event changes (handleMaintenanceUpdate is stable).
  const [prevMaintenanceEvent, setPrevMaintenanceEvent] = useState(maintenanceEvent);
  if (maintenanceEvent !== prevMaintenanceEvent) {
    setPrevMaintenanceEvent(maintenanceEvent);
    if (maintenanceEvent) {
  ...
  useEffect(() => {
    // IIFE so the deferred setState inside checkMaintenance isn't read as a synchronous set.
    void (async () => { await checkMaintenance(); })();
  }, [checkMaintenance]);
