...
export function useNotificationWebSocket() {
  const { user } = useUser();
  // Capture the primitive so the connect callback's inferred dep (userId) matches
  // its dependency array (react-hooks/preserve-manual-memoization).
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const url = `${wsUrl}/ws/user/${userId}`;
  ...
  }, [userId, maxReconnectAttempts, reconnectDelay]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- connect/disconnect are useCallbacks keyed on userId (already listed); intentionally reconnect only when userId changes
  }, [userId]);
