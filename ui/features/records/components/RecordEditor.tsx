...
export default function RecordEditor({
  ...
  useEffect(() => {
    // IIFE so the deferred setState inside fetchSchema isn't read as a synchronous set.
    void (async () => { await fetchSchema(); })();
  }, [fetchSchema]);

  // Update JSON value when switching to JSON mode (adjust during render, not in an effect).
  const jsonSyncKey = mode === 'json' ? `${mode}|${JSON.stringify(records)}` : mode;
  const [prevJsonSyncKey, setPrevJsonSyncKey] = useState(jsonSyncKey);
  if (jsonSyncKey !== prevJsonSyncKey) {
    setPrevJsonSyncKey(jsonSyncKey);
    if (mode === 'json') {
