...
export default function BaseServiceSection({ title = 'Service' }: BaseServiceSectionProps) {
  ...
  // Initialize collapsed state from section configs (adjust during render, not in
  // an effect) when the configs change. Existing user state wins via {...,...prev}.
  const [prevSectionConfigs, setPrevSectionConfigs] = useState(sectionConfigs);
  if (sectionConfigs !== prevSectionConfigs) {
    setPrevSectionConfigs(sectionConfigs);
      ...
      <div className="space-y-2">
        ...
        <div className="min-h-[42px] w-full px-2 py-1.5 border border-primary rounded-md shadow-sm focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-info flex flex-wrap gap-1.5 items-center">
          ...
          <input
            ...
            onBlur={addTag}
            className="flex-1 min-w-[80px] bg-transparent border-none outline-none focus-visible:ring-2 focus-visible:ring-primary rounded text-sm py-0.5"
