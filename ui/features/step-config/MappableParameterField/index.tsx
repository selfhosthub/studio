...
export function MappableParameterField({
  ...
  // The bound AI Prompt's System box already fills this service's system
  // slot; the dispatch shaper gives it precedence, so anything typed here
  // would be silently ignored. Disable the field and say why.
  if (schema.ui?.system_slot && systemProvidedByPrompt) {
    return (
      <div className="param-field-wrapper">
        <label htmlFor={`param-${paramKey}`} className="block text-sm font-medium text-secondary mb-1">
          {label}
        </label>
        <textarea
          id={`param-${paramKey}`}
          value={value ?? ''}
          disabled
          rows={schema.ui?.rows ?? 3} // defaults-ok: matches FieldInput's default-size textareaDefaultRows
          className="form-textarea w-full text-sm opacity-60"
        />
        <p className="mt-1 text-xs text-danger">
          System content is provided by the selected AI Prompt. To edit it,
          change the prompt&apos;s System field - this value is ignored.
        </p>
      </div>
    );
  }
      ...
      <div className="param-field-input-row" data-testid={`param-control-${paramKey}`}>
        {isPrompt ? (
          ...
          <MappedFieldSelector
            ...
            arrayMappingNotice={arrayMappingNotice}
          />
