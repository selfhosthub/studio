...
export interface PropertySchema {
  ...
  ui?: {
    ...
    /** Marks the service's system-slot parameter (e.g. claude `system`,
     *  gemini `systemInstruction`). When a bound AI Prompt already carries
     *  system content, this field is disabled - the prompt wins at dispatch. */
    system_slot?: boolean;
...
export interface MappableParameterFieldProps {
  ...
  /** True when a sibling parameter has an AI Prompt bound whose System box is
   *  non-empty. Fields marked ui.system_slot render disabled with a notice -
   *  the dispatch shaper gives the prompt's system content precedence. */
  systemProvidedByPrompt?: boolean;
