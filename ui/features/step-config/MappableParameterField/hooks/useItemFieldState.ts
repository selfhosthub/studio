...
export function useItemFieldState(
  ...
  // The setStates bail out when nothing changed, so this is loop-safe even when
  // `value` is a fresh reference each render; the IIFE keeps the bail-out sets off
  // the synchronous effect path (react-hooks/set-state-in-effect).
  useEffect(() => {
    void (async () => {
