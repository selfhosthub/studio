...
interface ArrayItemsPanelProps {
  ...
  /** Service-authored message shown when an array is mapped but the service is not iterable. */
  arrayMappingNotice?: string;
}
...
export function ArrayItemsPanel({
  ...
  // Stable IDs for array items - prevents React DOM reuse on reorder. Held in state
  // (not a render-mutated ref, per react-hooks/refs) and reconciled to the array
  // length during render; add/remove/reorder update the id list to track items.
  const syncIds = (ids: string[], next: number, length: number): { ids: string[]; next: number } => {
    const out = ids.slice(0, length);
    let n = next;
    while (out.length < length) out.push(`item-${n++}`);
    return { ids: out, next: n };
  };
  const [idState, setIdState] = useState<{ ids: string[]; next: number }>(
    () => syncIds([], 0, arrayValue.length)
  );
  const itemIds = idState.ids.length === arrayValue.length
    ? idState.ids
    : syncIds(idState.ids, idState.next, arrayValue.length).ids;
  if (idState.ids.length !== arrayValue.length) {
    setIdState(s => syncIds(s.ids, s.next, arrayValue.length));
  }
  ...
  useEffect(() => {
    if (schema.type === 'array' && schema.minItems && schema.minItems > 0) {
      ...
      if (currentLength < schema.minItems) {
        ...
        // IIFE so this sync set isn't read as a synchronous set within the effect body.
        void (async () => { setExpandedArrayItems({ 0: true }); })();
      }
    }
  ...
  const addItem = () => {
    ...
    onValueChange(paramKey, [...arrayValue, newItem]);
    // The render-time reconcile will assign this id to the new last item.
    const newId = `item-${idState.next}`;
    setExpandedArrayItems(prev => ({ ...prev, [newId]: true }));
  };
  const removeItem = (index: number) => {
    if (schema.minItems && arrayValue.length <= schema.minItems) return;
    onRemoveItemMappings?.(paramKey, index, arrayValue.length);
    setIdState(s => { const ids = s.ids.slice(); ids.splice(index, 1); return { ...s, ids }; });
  ...
  const renderCollapsibleGroupDivider = (groupTitle: string, itemIndex: number, groupId: string, isCollapsed: boolean, hasModified: boolean) => (
    <div
      ...
      className="group-header-warning -mx-3 mt-3 mb-1 focus-visible:ring-2 focus-visible:ring-primary"
      onClick={() => toggleSceneGroupCollapsed(itemIndex, groupId)}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          toggleSceneGroupCollapsed(itemIndex, groupId);
        }
      }}
    >
  ...
  const renderItemField = (
    ...
    // The mapped output is an array - the service will combine all its items.
    const mappedBaseField = (fieldMapping.outputField || '').split('[')[0];
    const isArrayMapped = (outputs[mappedBaseField] as { type?: string } | undefined)?.type === 'array';
    ...
    return (
      ...
      >
            ...
            <div className="relative">
              ...
              {isDropdownOpen && (
                <div data-step-config-dropdown className="absolute right-0 mt-1 w-36 bg-card border border-primary rounded shadow-lg z-50">
                  <button type="button" onClick={(e) => { e.stopPropagation(); setItemFieldMode(itemIndex, fieldKey, 'static', keyPrefix, onMappingChange, () => onFieldChange(fieldKey, '')); }} className={`w-full flex items-center gap-2 px-2 py-1.5 text-xs text-left hover:bg-card ${fieldMode === 'static' ? 'bg-surface' : ''}`}><Type className="h-3 w-3 text-secondary" /><span>Static</span></button>
                  ...
                  <button type="button" onClick={(e) => { e.stopPropagation(); setItemFieldMode(itemIndex, fieldKey, 'form', keyPrefix, onMappingChange, () => onFieldChange(fieldKey, '')); }} className={`w-full flex items-center gap-2 px-2 py-1.5 text-xs text-left hover:bg-card ${fieldMode === 'form' ? 'bg-surface' : ''}`}><FileInput className="h-3 w-3 text-info" /><span>Form</span></button>
                  {fieldSchema.ui?.prompt && <button type="button" onClick={(e) => { e.stopPropagation(); setItemFieldMode(itemIndex, fieldKey, 'prompt', keyPrefix, onMappingChange, () => onFieldChange(fieldKey, '')); }} className={`w-full flex items-center gap-2 px-2 py-1.5 text-xs text-left hover:bg-card ${fieldMode === 'prompt' ? 'bg-surface' : ''}`}><Sparkles className="h-3 w-3 text-success" /><span>Prompt</span></button>}
                </div>
              )}
            </div>
          )}
        ...
        {fieldMode === 'prompt' ? (() => {
          ...
          <>
            {renderMappedFieldInput()}
            {arrayMappingNotice && isArrayMapped && (
              <div className="p-2 bg-info-subtle border border-info rounded-md">
                <p className="text-xs text-info">{arrayMappingNotice}</p>
              </div>
            )}
          </>
