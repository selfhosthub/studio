...
export function SchemaView({
  ...
  const [prevStepsForExpand, setPrevStepsForExpand] = useState(previousSteps);
  if (previousSteps !== prevStepsForExpand) {
    setPrevStepsForExpand(previousSteps);
  ...
  // Auto-expand new arrays (both native and forwarded). Runs during render when
  // previousSteps or seenArrays change (matching the original effect's deps);
  // self-terminates because once keys are added they're no longer "new".
  const [prevArrayCheck, setPrevArrayCheck] = useState<{ steps: Step[]; seen: Set<string> }>(
    () => ({ steps: previousSteps, seen: seenArrays })
  );
  if (prevArrayCheck.steps !== previousSteps || prevArrayCheck.seen !== seenArrays) {
    setPrevArrayCheck({ steps: previousSteps, seen: seenArrays });
  ...
  const getFilteredOutputs = (stepId: string, outputs: Record<string, any>) => {
    return Object.entries(outputs).filter(([fieldName, fieldDef]) => {
      ...
      if (typeFilter) {
        const matchesType = typeFilter === 'number' ? (fieldType === 'number' || fieldType === 'integer') : fieldType === typeFilter; // form-field-switch-ok - output-type filter
    ...
    return Object.entries(instanceFormFields)
      .filter(([fieldName, def]) => {
        if (typeFilter) {
          ...
          const matchesType = typeFilter === 'number' ? (fieldType === 'number' || fieldType === 'integer') : fieldType === typeFilter; // form-field-switch-ok - output-type filter // form-field-switch-ok - output-type filter
          if (!matchesType) return false;
        }
        if (showUnmappedOnly) {
            ...
            {isExpanded && (filteredOutputs.length > 0 || filteredPromptOutputs.length > 0 || filteredForwardedOutputs.length > 0) && (
              <div className={`ml-3 border-l ${isImmediatePredecessor ? 'border-info' : 'border-primary'}`}>
                ...
                {filteredOutputs.map(([fieldName, fieldDef]) => {
                  ...
                  const hasNested = fieldType === 'array' && typedField.items?.properties; // form-field-switch-ok - output-type check
                  const arrayKey = `${step.id}:${fieldName}`;
                    ...
                    {!isForwardedCollapsed && (
                      <div className="ml-3 border-l border-purple-200 dark:border-purple-800"> {/* css-check-ignore: no semantic token */}
                        {filteredForwardedOutputs.map(([fieldName, fieldDef]) => {
                          ...
                          const fieldType = typedField.type || 'string';
                          const isMapped = isFieldMapped(step.id, fieldName, inputMappings);
                          const hasNested = fieldType === 'array' && typedField.items?.properties; // form-field-switch-ok - output-type check // form-field-switch-ok - output-type check
                          const arrayKey = `${step.id}:forwarded:${fieldName}`;
                          const isArrayExpanded = expandedArrays.has(arrayKey);
