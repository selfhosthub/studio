...
import { getProviderService, getAllServices, getPrompt } from '@/shared/api';
...
export function useEnhancedStepOutputs(
  ...
  useEffect(() => {
    const fetchMissingSchemas = async () => {
      ...
      // service_id is a namespaced slug (e.g. "shs/openai.openai_chat"); the API is keyed by UUID.
      const allServices = await getAllServices();
      const idByServiceId = new Map(allServices.map(s => [s.service_id, s.id]));

      await Promise.all(
        servicesToFetch.map(async ({ serviceId }) => {
          try {
            const uuid = idByServiceId.get(serviceId);
            if (!uuid) return;
            const data = await getProviderService(uuid);
  ...
  const typeCounts = useMemo(() => {
    ...
    enhancedSteps.forEach(step => {
      Object.values(step.outputs || {}).forEach((fieldDef: any) => {
        ...
        if (fieldType === 'integer') { // form-field-switch-ok - JSON-schema output type, not a form-field render
          counts.number++;
