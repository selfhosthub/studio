...
For how variables flow between env files and running processes, see [env-inheritance.md](../development/env-inheritance.md) (architectural context; see here for the current variable list).
...
| Variable | Description |
|----------|-------------|
| `SHS_REPO_ROOT` | Path to a local `studio-dev` checkout. Only needed when building images from source rather than pulling from GHCR. |
