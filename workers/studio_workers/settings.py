# workers/studio_workers/settings.py
...
class SharedSettings(BaseSettings):
    ...
    # Per-cycle sleep randomization (±fraction) to desync workers that boot
    # together. Applied AFTER the backoff cap, so the effective max sleep is
    # JOB_POLL_BACKOFF_MAX_S * (1 + JOB_POLL_JITTER_FRAC). Capping after jitter
    # would re-synchronize all workers at exactly the cap, defeating the point.
    JOB_POLL_JITTER_FRAC: float = 0.25
    ...
    # TCP connect + pool-acquire timeout for httpx transfer requests.
    HTTP_CONNECT_TIMEOUT_S: float = 30
