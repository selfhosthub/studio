...
class JobClaimContract(BaseModel):
    ...
    service_type: str
    service_id: str
    # Surfaced on the claim so storage_mode=local workers can compute the
    # canonical /workspace/orgs/{org}/instances/{inst}/{file} write path.
    # Optional for backward compat with old claim payloads predating the
    # write-side direct-path optimization.
