# workers/studio_workers/engines/general/handler.py
...
class StepExecutor(WorkerBase):
    ...
    def _process_step_job(self, job: Dict[str, Any]):
        ...
        try:
            ...
            # Per-step completion mode + the service's declared callback paths.
            # When the step opts into "webhook", the request fires and releases
            # without polling; the API parks the step for a provider callback.
            # Absent → today's get-mode behavior.
            completion_mode = job.get("completion_mode")
            webhook_completion = job.get("webhook_completion")
            # A pre-built result whose asset URLs are post-processed directly;
            # no outbound request fires for such a job.
            result_seed = job.get("result_seed")
            # Reconcile poll of an already-fired generation id (no fire); the
            # block carries the service's polling paths + base_url + the id.
            generation_poll = job.get("generation_poll")
            ...
            result, _ = self.executor.execute(
                ...
                webhook_completion=webhook_completion,
            ...
            # Fire-and-released steps report the generation id with a pending flag;
            # the marker is internal and must not reach the published result.
            webhook_pending = bool(
                isinstance(result, dict) and result.pop("_webhook_pending", False)
            )
            ...
            # logs. Raw str(e) leaks infra detail (SQL, paths, upstream bodies) to UI
            # clients. safe_error_message lives in api/, not importable here, so
            # workers inline the classifier (transfer-worker idiom).
            step_duration_ms = int((time.time() - step_start_time) * 1000)
            logger.exception(
                f"Step failed (job {job_id}, {step_duration_ms}ms): {e}",
                extra={
                    "job_id": job_id,
                    "service_id": service_id,
                    "duration_ms": step_duration_ms,
