# workers/studio_workers/engines/general/executor.py
...
def _bare_service_name(service_id: str) -> str:
    """Strip the provider namespace from a catalog service_id.

    All catalog items are namespaced ``{provider_slug}.{service_name}``
    (e.g. ``shs/core.poll_service`` -> ``poll_service``,
    ``openai.chat`` -> ``chat``). The provider slug may itself contain a
    ``/`` path segment (``shs/core``). The service name is the segment
    after the final ``.``. Mirrors the API inline executor's
    ``service_id.split(".")[-1]`` so worker-side core dispatch matches
    the same contract.
    """
    return service_id.rsplit(".", 1)[-1] if "." in service_id else service_id


def _is_core_service(service_id: str) -> bool:
    """True when the service_id resolves to a core (``.../core``) provider.

    Core services carry a provider slug whose final path segment is
    ``core`` (``shs/core.poll_service``). Bare ``core.poll_service`` (used
    in unit tests and any non-namespaced caller) is also accepted.
    """
    if not service_id or "." not in service_id:
        return False
    provider_slug = service_id.rsplit(".", 1)[0]
    return provider_slug == "core" or provider_slug.endswith("/core")
...
class JobExecutor:
    ...
    def __init__(
        ...
        token_getter=None,
        storage_mode_getter=None,
    ):
        self.http_client = http_client
        self.credential_client = credential_client
        if token_getter is None:
            raise ValueError("token_getter is required")
        self._upload_client = FileUploadClient(
            token_getter=token_getter,
            storage_mode_getter=storage_mode_getter,
        )
        # Set per-job in execute(); enables the local-write upload path.
    ...
    def execute(
        ...
        webhook_completion: Optional[Dict[str, Any]] = None,
        result_seed: Optional[Dict[str, Any]] = None,
        generation_poll: Optional[Dict[str, Any]] = None,
        organization_id: Optional[str] = None,
        instance_id: Optional[str] = None,
    ) -> Tuple[Dict[str, Any], Optional[Dict[str, Any]]]:
        """Execute a single workflow step. Returns (result, request_body)."""
        # Stashed per-job so the upload helpers can take the bytes-free local
        # write path when this worker shares the API's /workspace mount.
        self._organization_id = organization_id
        self._instance_id = instance_id
        job_config = step_config.get("job") or {}
        if not service_id:
            service_id = step_config.get("service_id") or job_config.get("service_id")
        if not provider_id:
            provider_id = step_config.get("provider_id") or job_config.get(
                "provider_id"
            )

        # service_id selects the execution path (_is_core_service / local_worker /
        # HTTP). A missing one is unroutable - fail loud instead of substituting ""
        # and silently falling through to the wrong branch.
        if not service_id:
            raise ValueError("step is missing service_id; cannot route execution")
        ...
        seeded = False
        if _is_core_service(service_id):
            ...
            # Webhook mode: fire once and release the worker, capturing the
            # provider's generation id; the provider calls back when the asset
            # renders. Only when the step opted in and the service declares the
            # paths - otherwise the request runs to completion as today.
            webhook_mode = completion_mode == "webhook" and bool(webhook_completion)
            result, actual_request = self._fire_prebuilt_request(
                http_request=http_request,
                credential_id=credential_id,
                auth_config=auth_config,
                polling_config=polling_config,
                webhook_completion=webhook_completion if webhook_mode else None,
            )
            request_body = actual_request

        elif result_seed is not None:
            # A job carrying a pre-built result skips the request and
            # post-processes the result directly. There is no outbound call;
            # the asset URLs are already present in the seed.
            result = result_seed
            request_body = {}
            seeded = True

        elif generation_poll is not None:
            # Reconcile poll: a previously-fired generation whose callback never
            # arrived. Poll the provider's status endpoint for the known id (no
            # fire). Complete -> result flows through normal projection +
            # download post-processing; provider-failed -> raise -> step FAILED;
            # still pending -> re-park (the marker keeps the step WAITING).
        ...
        # Skip for QUEUED results (local worker routing) and fire-and-released
        # results (the asset isn't ready yet - it arrives via callback).
        if post_processing_config and not (
            isinstance(result, dict)
            and (result.get("status") == "QUEUED" or result.get("_webhook_pending"))
        ):
    ...
    def _execute_core_service(
        ...
        if _bare_service_name(service_id) == "poll_service":
    ...
    def _fire_prebuilt_request(
        ...
        if webhook_completion:
            result = self._capture_webhook_fire(result, webhook_completion)
        elif polling_config and polling_config.get("enabled"):
    ...
    def _capture_webhook_fire(
        self,
        create_response: Dict[str, Any],
        webhook_completion: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Flag a fired result for fire-and-release, returning without polling.

        generation_id routing pulls the provider's id out of the create response
        (the inbound demux key). execution_token routing already carries the match
        key on the row (external_id), so it captures no id - it only asserts the
        create was accepted (when id_response_path is declared) and parks. NOT
        returning a generation_id is deliberate: result processing writes any
        generation_id onto external_id, which would clobber the minted token.

        A missing/absent id (when a path is declared) means the request shape
        doesn't match - fail loud rather than release a step that would park
        forever waiting on a callback it can't match.
        """
        id_path = webhook_completion.get("id_response_path")
        routing = webhook_completion.get("routing", "generation_id")

        if routing == EXECUTION_TOKEN_ROUTING:
            # Optional create-acceptance assert; the id is not used for routing.
            if id_path and not jsonpath_parse(id_path).find(create_response):
                raise RuntimeError(
                    f"Create response has no id at {id_path}; not parking "
                    f"(provider likely rejected the request)"
                )
            return {"_webhook_pending": True}

        if not id_path:
            raise RuntimeError("webhook completion requires an id_response_path")

        try:
            jsonpath_expr = jsonpath_parse(id_path)
        except Exception as e:
            raise RuntimeError(
                f"Invalid webhook id_response_path {id_path!r}: {e}"
            ) from e

        matches = jsonpath_expr.find(create_response)
        if not matches:
            raise RuntimeError(
                f"No generation id at {id_path} in the create response"
            )

        return {"generation_id": matches[0].value, "_webhook_pending": True}
    ...
    def _poll_generation_status(
        self,
        generation_poll: Dict[str, Any],
        credential_id: Optional[str],
        auth_config: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        """Single-shot reconcile poll of an already-fired generation id.

        ``generation_poll`` is the service's get-mode polling block plus
        ``base_url`` and ``generation_id`` (the parked step's external id). Auth
        is merged from the same credential/auth_config the fire used. Reuses
        ``_poll_for_completion`` (with max_attempts pinned by the API to keep
        this a quick check): complete -> the extracted result; provider-failed
        -> RuntimeError (step FAILED); not yet done -> TimeoutError, caught here
        and turned into a fire-and-release marker so the step stays parked for a
        later reconcile.
        """
        headers: Dict[str, str] = {}
        if credential_id:
            ...
            if credentials:
                self._merge_auth_into_envelope(headers, {}, credentials, auth_config)
            else:
                logger.warning(
                    f"Reconcile poll: no credentials for {credential_id}"
                )

        generation_id = generation_poll.get("generation_id")
        try:
            return self._poll_for_completion(
                initial_result={},
                polling_config=generation_poll,
                base_url=generation_poll.get("base_url", ""),
                headers=headers,
                generation_id=generation_id,
            )
        except TimeoutError:
            logger.debug(
                f"Reconcile poll: generation {generation_id} not ready; "
                "leaving the step parked"
            )
            return {"_webhook_pending": True, "generation_id": generation_id}
    ...
    def _poll_for_completion(
        ...
        """Poll for async-API completion.

        The generation id normally comes from the initial fire response via
        ``generation_id_path``. A caller that already knows the id (e.g. a
        reconcile poll of a previously-fired generation that never called back)
        passes ``generation_id`` directly, which skips the extraction.
        """
        import time
        ...
        if generation_id is None:
            generation_id_path = polling_config.get(
                "generation_id_path", "$.generationId"
            )
            try:
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                # A non-retryable client error (expired/invalid key, forbidden,
                # unknown generation) will never resolve by waiting. Fail fast
                # with the real status + a bounded body preview instead of
                # looping to a generic "timed out" that masks the cause.
                # 408 (request timeout) and 429 (rate-limit) are transient -
                # keep polling within the budget.
                if 400 <= status_code < 500 and status_code not in (408, 429):
                    body_preview = " ".join(e.response.text.split())[:200]
                    raise RuntimeError(
                        f"Polling status check failed with HTTP {status_code}: "
                        f"{body_preview}"
                    ) from e
                logger.warning(
                    f" Poll attempt {attempt} got retryable HTTP "
                    f"{status_code}; will retry"
                )
                continue
            except Exception as e:
                logger.warning(f" Poll attempt {attempt} failed: {e}")
    ...
    def _post_process_result(
        ...
        decode_base64_config = post_processing_config.get("decode_base64")
        if decode_base64_config:
            return self._decode_base64_result(
                result=result,
                config=decode_base64_config,
                iteration_index=iteration_index,
                job_id=job_id,
            )
        ...
        try:
            for index, url in enumerate(urls):
                try:
                    ...
                    # Flatten path separators so the name can't escape temp_dir.
    ...
    def _decode_base64_result(
        ...
        """Decode base64 strings from a JSON field, upload as files, set downloaded_files."""
        import base64
        import tempfile

        source_path = config.get("source_path")
        if not source_path:
            logger.warning("decode_base64 config missing source_path")
            return result

        try:
            jsonpath_expr = jsonpath_parse(source_path)
            matches = jsonpath_expr.find(result)
            encoded = [match.value for match in matches if match.value]
        except Exception as e:
            logger.warning(f" Failed to parse JSONPath '{source_path}': {e}")
            return result

        if not encoded:
            logger.debug(f"→ No base64 data found at {source_path}")
            return result

        filename_pattern = config.get("filename_pattern", "{uuid}_{index}.png")
        max_file_size_mb = config.get("max_file_size_mb", FILE_DOWNLOAD_MAX_MB)
        max_file_size_bytes = max_file_size_mb * 1024 * 1024

        iter_suffix = f"_{iteration_index}" if iteration_index is not None else ""
        temp_dir = Path(tempfile.mkdtemp(prefix=f"studio_b64{iter_suffix}_"))
        temp_dir.mkdir(parents=True, exist_ok=True)
            ...
            for index, b64 in enumerate(encoded):
                try:
                    ...
                    if len(raw) > max_file_size_bytes:
                        logger.warning(
                            f"Decoded file too large (>{max_file_size_bytes} bytes), skipping"
                        )
                        continue

                    file_uuid = str(uuid.uuid4())[:8]
                    filename = filename_pattern.format(index=index, uuid=file_uuid)
                    filename = filename.replace("/", "_").replace("\\", "_")
                    ...
                    logger.debug(f"  ✓ Saved: {filename} ({len(raw)} bytes)")

                except Exception as e:
                    logger.warning(f"  ✗ Failed to decode base64 at index {index}: {e}")
                    continue
        ...
        # Partial results break downstream steps - fail loud.
        failed_count = len(encoded) - len(decoded_files)
        if failed_count > 0:
            raise RuntimeError(
                f"{failed_count} of {len(encoded)} base64 decodes failed"
            )

        result["downloaded_files"] = decoded_files
        logger.debug(f"✓ Decoded {len(decoded_files)} file(s)")
        ...
        try:
            ...
            if config.get("compute_duration"):
                if duration is not None:
                    ...
                    logger.warning(
                        f"compute_duration requested but duration probe failed for {filename}. "
