# workers/studio_workers/engines/video/handler.py
...
class VideoWorker(WorkerBase):
    ...
    def _process_job(self, job: Dict[str, Any]):
        ...
        try:
            ...
            if output_path and os.path.exists(output_path):
                # Local-write path consumes output_path; capture size first.
            ...
            # logs. Raw str(e) leaks infra detail (paths, ffmpeg/upstream output) to
            # UI clients. safe_error_message lives in api/, not importable here, so
            # workers inline the classifier (transfer-worker idiom).
            duration_ms = int((time.time() - job_start_time) * 1000)
            logger.exception(
                f"Video job {job_id} failed ({duration_ms}ms): {e}",
    ...
    def _render_video_clip(
        ...
        from studio_workers.engines.video.common import get_media_duration
    ...
    def _resolve_media_path(
        self, src: Any, cache_dir: Optional[str], default_ext: str = ".mp3"
    ) -> str:
        """Resolve a media reference to a local filesystem path.

        Accepts a dual-view ``{virtual_path, url, filename}`` dict from
        enqueue, an external URL string, or a bare local path. Workspace
        sources go to ``resolve_media_source`` (local first, URL second,
        classified error if both fail); bare local paths short-circuit and
        are returned as-is for back-compat with internal callers.
        """
        from studio_workers.engines.video.common import download_file, create_http_client
        from studio_workers.utils.security import resolve_media_source

        if isinstance(src, dict):
            virtual_path: Optional[str] = src.get("virtual_path")
            url: Optional[str] = src.get("url")
        elif isinstance(src, str) and src.startswith("/orgs/"):
            virtual_path = src
            url = None
            ...
            # Bare local path (test fixtures / internal callers passing an
            # already-resolved path). Return unchanged; downstream consumers
            # (ffmpeg, PIL) handle existence checking.
            return src

        ext_src = url or virtual_path or ""
        ext = os.path.splitext(ext_src.split("?")[0])[-1] or default_ext

        def _download(target_url: str) -> str:
            local_path = os.path.join(
                cache_dir or self.cache_dir, f"media_{uuid.uuid4().hex[:8]}{ext}"
            )
            os.makedirs(os.path.dirname(local_path), exist_ok=True)
            client = create_http_client(
                timeout=float(settings.HTTP_DOWNLOAD_TIMEOUT_S)
            )
            try:
                download_file(client, target_url, local_path)
            finally:
                client.close()
            return local_path
    ...
    def _concatenate_group_videos(
        ...
        from studio_workers.engines.video.utils import generate_random_filename
    ...
    def _upload_to_storage(
        ...
        For video outputs, extracts a JPEG poster frame and sends it alongside
        as the OrgFile thumbnail - the API can't extract video thumbnails itself.
        """
        ...
        if ext in (".mp4", ".mov", ".webm", ".mkv", ".avi"):
            from studio_workers.engines.video.common import extract_poster_thumbnail
            thumb_local = os.path.join(
                os.path.dirname(local_path), f"{os.path.basename(local_path)}.thumb.jpg"
            )
            thumbnail_path = extract_poster_thumbnail(local_path, thumb_local)
    ...
    def _merge_audio_tracks(
        ...
        # url may be a string (external CDN) or a dual-view media-ref dict
        # ({virtual_path, url, filename}) after enqueue-side resolution.
        # merge_video_audio handles both shapes.
