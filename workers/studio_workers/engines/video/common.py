# workers/studio_workers/engines/video/common.py
...
def extract_poster_thumbnail(
    video_path: str,
    output_path: str,
    max_dim: int = 256,
    quality: int = 85,
) -> Optional[str]:
    """Extract a single JPEG poster frame from a video; returns output_path on success or None.

    Seeks to ~10% in (capped at 2s) - a stable visual frame, not a black opener.
    Scales the longest side to max_dim, preserving aspect ratio. Failures are
    logged and swallowed; thumbnails are best-effort and never fail a render.
    """
    try:
        duration = get_media_duration(video_path)
    except Exception as exc:
        logger.warning(f"Cannot probe duration for thumbnail extract on {video_path}: {exc}")
        duration = 0.0

    seek = min(2.0, max(0.0, duration * 0.1))
    cmd = [
        "ffmpeg",
        "-y",
        "-loglevel",
    ...
    try:
        run_ffmpeg(
            cmd,
            "Thumbnail extract",
            timeout=video_settings.THUMBNAIL_EXTRACT_TIMEOUT_S,
        )
        return output_path
    except Exception as exc:
        logger.warning(f"Thumbnail extract failed for {video_path}: {exc}")
        return None
