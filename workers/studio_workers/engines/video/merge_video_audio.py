# workers/studio_workers/engines/video/merge_video_audio.py
...
def merge_video_audio(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Enqueue-side ships file refs as dual-view {virtual_path, url, filename}
    # dicts; legacy callers (and external CDN audio URLs) may still pass a
    # plain URL string. Unpack into the resolve_media_source pair.
    audio_url_raw = params.get("audio_url")
    if isinstance(audio_url_raw, dict):
        audio_virtual_path = audio_url_raw.get("virtual_path")
        audio_url = audio_url_raw.get("url")
    else:
        audio_virtual_path = None
    ...
    try:
        ...
        # Uniform policy: local first, URL fallback, classified error if
        # both fail. Replaces the pre-Group-2 stat-and-fallback block whose
        # URL fallback was broken (called httpx with the /orgs/ virtual
        # path, never a real URL).
        if not audio_path and (audio_virtual_path or audio_url):
            ...
            def _download_audio(url: str) -> str:
                ext = get_url_extension(url, ".mp3")
                dest = temp.create_path(f"input_audio{ext}")
                download_file(http_client, url, dest)
                return dest

            audio_path = resolve_media_source(
                virtual_path=audio_virtual_path,
                url=audio_url,
                workspace_root=_settings.WORKSPACE_ROOT,
                download_fn=_download_audio,
            )
            logger.debug(f"Resolved audio source -> {audio_path}")
...
def _process_subtitles_auto(
    ...
    transcription = transcribe_audio(
        ...
        backend=sub_config.get("backend"),
    )
