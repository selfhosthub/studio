# workers/studio_workers/engines/video/create_video.py
...
from studio_workers.utils.security import validate_padding_color
...
def create_video(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from studio_workers.settings import settings as _settings
    from studio_workers.engines.video.settings import settings as _video_settings
...
def _process_image_to_clip(
    ...
    # Resolve via the uniform policy: local first, URL fallback, classified
    # error if both fail. image_url may be a dual-view dict from enqueue, an
    # external URL string, or a /orgs/ virtual path string (back-compat).
    if isinstance(image_url, dict):
        image_virtual_path = image_url.get("virtual_path")
        image_url_str = image_url.get("url")
    elif isinstance(image_url, str) and image_url.startswith("/orgs/"):
        image_virtual_path = image_url
        image_url_str = None
    elif isinstance(image_url, str) and image_url.startswith(("http://", "https://")):
        image_virtual_path = None
        image_url_str = image_url
    else:
        # Legacy bare-string callers (test fixtures or pre-resolved paths).
        # The unified helper now handles workspace + URL cases above; this
        # branch keeps internal callers that pass an already-local path
        # working without going through download.
        image_path = image_url
        image_virtual_path = None
        image_url_str = None

    if image_virtual_path or image_url_str:
        from studio_workers.settings import settings as _s
        from studio_workers.utils.security import resolve_media_source

        def _download_image(target_url: str) -> str:
            ext = get_url_extension(target_url, ".jpg")
            dest = temp.create_path(f"image_{idx}{ext}")
            download_file(http_client, target_url, dest)
            return dest

        image_path = resolve_media_source(
            virtual_path=image_virtual_path,
            url=image_url_str,
            workspace_root=_s.WORKSPACE_ROOT,
            download_fn=_download_image,
        )
