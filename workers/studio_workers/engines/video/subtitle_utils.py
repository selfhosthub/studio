# workers/studio_workers/engines/video/subtitle_utils.py
...
try:
    from faster_whisper import WhisperModel  # pragma: no cover

    FASTER_WHISPER_AVAILABLE = True  # pragma: no cover
...
def transcribe_audio(
    ...
    """Transcribe audio; returns result dict with word-level timestamps.

    backend: "openai" or "faster". Falls back to settings.WHISPER_BACKEND.
    """
    ...
    backend = backend or video_settings.WHISPER_BACKEND
    if backend == "faster":
        return _transcribe_faster(audio_path, language, model_name)
    return _transcribe_openai(audio_path, language, model_name)
...
def _transcribe_faster(
    ...
    """faster-whisper backend; normalized to the openai-whisper result shape."""
    if not FASTER_WHISPER_AVAILABLE:
        raise RuntimeError(
            "faster-whisper not installed. Run: pip install faster-whisper"
        )

    # CTranslate2 has no MPS; int8 on CPU is the fast path on Apple Silicon.
    device, compute_type = "cpu", "int8"
    if TORCH_AVAILABLE and torch.cuda.is_available():
        device, compute_type = "cuda", "float16"

    cache_key = ("faster", model_name, device, compute_type)
    if cache_key in _whisper_model_cache:
        logger.debug(f"Using cached faster-whisper model: {model_name} ({device})")
        model = _whisper_model_cache[cache_key]
    else:
        logger.debug(f"Loading faster-whisper model: {model_name} ({device})")
    ...
    # faster-whisper streams Segment objects with .words[].{word,start,end};
    # downstream expects {"segments": [{"words": [{word,start,end}], "text"}], "text"}.
    out_segments = []
    full_text = []
    for seg in segments:
        words = [
