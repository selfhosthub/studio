# workers/studio_workers/utils/http_job_client.py
...
class HTTPJobClient:
    ...
    def __init__(
        ...
        self.jitter_frac = JOB_POLL_JITTER_FRAC
    ...
    def claim_job(self, queue_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from studio_workers.utils.logging_config import clear_job_correlation_id
            ...
            if response.status_code == 200:
                ...
                from studio_workers.utils.logging_config import set_job_correlation_id
    ...
    def get_sleep_duration(self) -> float:
        # Apply ±JITTER_FRAC randomization so workers that boot and back off in
        # lockstep desync within a cycle or two. Jitter is applied AFTER the cap
        # (see settings.JOB_POLL_JITTER_FRAC for why), so the sleep may exceed
        # backoff_max by up to the jitter fraction.
        jitter = 1.0 + random.uniform(-self.jitter_frac, self.jitter_frac)
        return self.current_backoff * jitter
...
class JobClient:
    ...
    def startup_jitter(self) -> None:
        """Sleep a random fraction of one poll interval before the first claim.

        When multiple workers boot simultaneously (e.g. 6 containers via
        compose) they would otherwise all hit the claim endpoint on the same
        tick - a thundering herd. Spreading the first volley over [0, interval)
        smooths the initial load; per-cycle jitter (get_sleep_duration) keeps
        them desynced thereafter.
        """
        delay = random.uniform(0, JOB_POLL_INTERVAL)
        logger.debug(f"Startup poll jitter: sleeping {delay:.2f}s before first claim")
        time.sleep(delay)
