...
class JobExecutor:
    ...
    def _capture_webhook_fire(
        ...
        """Pull the generation id out of the create response and flag the result
        for fire-and-release, returning without polling.

        A missing id_response_path or an id that isn't in the response means the
        request shape doesn't match the declared path - fail loud rather than
        release a step that would park forever waiting on a callback it can't match.
        """
